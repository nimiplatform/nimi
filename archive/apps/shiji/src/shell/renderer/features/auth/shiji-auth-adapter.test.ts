/**
 * ShiJi auth adapter regression tests (SJ-SHELL-010 / SJ-SHELL-011 /
 * SJ-SHELL-002). Locks the RuntimeAccountService boundary:
 *
 * - `applyToken`/`persistSession` MUST throw SHIJI_TOKEN_PROXY_FORBIDDEN.
...
const ensureShiJiBootstrapReadyMock = vi.fn(async () => undefined);
...
vi.mock('@renderer/app-shell/bootstrap.js', async () => {
  const browser = await import('@nimiplatform/sdk/runtime/browser');
  return {
    ensureShiJiBootstrapReady: ensureShiJiBootstrapReadyMock,
    ...
    loadShiJiRuntimeAccountUser: async () => {
...
const {
  ...
  logoutShiJiRuntimeAccount,
} = await import('./shiji-auth-adapter.js');

describe('shiji-auth-adapter (SJ-SHELL-010 / SJ-SHELL-011 / SJ-SHELL-002)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  // -------------------------------------------------------------------------
  // SJ-SHELL-011: applyToken / persistSession MUST fail-close
  // -------------------------------------------------------------------------

  it('applyToken throws SHIJI_TOKEN_PROXY_FORBIDDEN — runtime owns custody', async () => {
    const adapter = createShiJiDesktopBrowserAuthAdapter();
    await expect(adapter.applyToken!('access-token', 'refresh-token')).rejects.toThrow(/SJ-SHELL-011|K-ACCSVC-008|token custody/);
  });

  it('persistSession throws SHIJI_TOKEN_PROXY_FORBIDDEN', async () => {
    const adapter = createShiJiDesktopBrowserAuthAdapter();
    await expect(
      adapter.persistSession!({ accessToken: 'a', refreshToken: 'r', user: { id: 'u' } } as any),
    ).rejects.toThrow(/SJ-SHELL-011|K-ACCSVC-008|token custody/);
  });

  // -------------------------------------------------------------------------
  // SJ-SHELL-011: every embedded auth path is unsupported
  // -------------------------------------------------------------------------
  ...
  // -------------------------------------------------------------------------
  // SJ-SHELL-002: loadCurrentUser comes from runtime projection
  // -------------------------------------------------------------------------
  ...
  it('loadCurrentUser projects from runtime.account.getAccountSessionStatus', async () => {
    getAccountSessionStatusMock.mockResolvedValue({
      ...
      accountProjection: { accountId: 'shiji-user', displayName: 'ShiJi User' },
    });
    await expect(loadShiJiCurrentUser()).resolves.toEqual({
      id: 'shiji-user',
      displayName: 'ShiJi User',
    });
    expect(ensureShiJiBootstrapReadyMock).toHaveBeenCalled();
  });
  ...
  it('loadCurrentUser returns null when runtime account is ANONYMOUS', async () => {
    ...
    await expect(loadShiJiCurrentUser()).resolves.toBeNull();
  });

  // -------------------------------------------------------------------------
  // SJ-SHELL-002: clearPersistedSession routes to runtime logout
  // -------------------------------------------------------------------------
  ...
  it('logoutShiJiRuntimeAccount() invokes runtime.account.logout once', async () => {
    await logoutShiJiRuntimeAccount();
    expect(logoutMock).toHaveBeenCalledTimes(1);
  });

  // -------------------------------------------------------------------------
  // SJ-SHELL-010: broker.begin uses the realm OAuth authorize URL from runtime
  // -------------------------------------------------------------------------

  it('broker.begin calls runtime.account.beginLogin with the fixed ShiJi caller', async () => {
    ...
    const broker = createShiJiRuntimeAccountBrowserBroker();
    const result = await broker.begin({
      callbackUrl: 'http://127.0.0.1:9200/oauth/callback',
      timeoutMs: 60_000,
    });
    ...
    expect(beginCall.caller).toEqual(
      expect.objectContaining({ appId: 'app.nimi.shiji' }),
    );
    expect(beginCall.redirectUri).toBe('http://127.0.0.1:9200/oauth/callback');
    expect(beginCall.callbackOrigin).toBe('http://127.0.0.1:9200');
  ...
  it('broker.begin throws when runtime rejects the request', async () => {
    ...
    const broker = createShiJiRuntimeAccountBrowserBroker();
    await expect(
      broker.begin({ callbackUrl: 'http://127.0.0.1:9200/oauth/callback', timeoutMs: 60_000 }),
    ).rejects.toThrow(/Runtime account login could not start/);
  });

  // -------------------------------------------------------------------------
  // SJ-SHELL-011 / R-OAUTH-008: broker.complete is code-only, refreshToken=''
  // -------------------------------------------------------------------------
  ...
  it('broker.complete sends an empty refreshToken (R-OAUTH-008 / K-ACCSVC-008)', async () => {
    ...
    expect(completeCall.redirectUri).toBe('http://127.0.0.1:9200/oauth/callback');
    expect(completeCall.callbackOrigin).toBe('http://127.0.0.1:9200');
    expect(result).toEqual({
      user: { id: 'shiji-user', displayName: 'ShiJi User' },
    });
  });
  ...
  // -------------------------------------------------------------------------
  // SJ-SHELL-011 source-text static lock: no legacy auth paths
  // -------------------------------------------------------------------------
  ...
  it('auth adapter module does not import legacy shared desktop auth-session helpers', async () => {
    ...
    const source = fs.readFileSync(path.join(here, 'shiji-auth-adapter.ts'), 'utf8');
