...
test('prepareProfileRuntimeDescriptor is write-classified and receives idempotency metadata', async () => {
  assert.equal(
    RuntimeWriteMethodIds.includes(RuntimeMethodIds.local.prepareProfileRuntimeDescriptor),
    true,
    'descriptor preparation writes Runtime-private materialization state',
  );
  assert.equal(isRuntimeWriteMethod(RuntimeMethodIds.local.prepareProfileRuntimeDescriptor), true);

  let captured: RuntimeUnaryCall<RuntimeWireMessage> | null = null;
  installNodeGrpcBridge({
    invokeUnary: async (_config, input) => {
      captured = input;
      return PrepareProfileRuntimeDescriptorResponse.toBinary(
        PrepareProfileRuntimeDescriptorResponse.create({
  ...
  try {
    const client = createRuntimeClient(runtimeConfig);
    const response = await client.local.prepareProfileRuntimeDescriptor({
      descriptorJson: new TextEncoder().encode('{"schema_version":1}'),
    });
    assert.equal(response.descriptorId, 'descriptor:test');
    assert.ok(captured);
    assert.equal(captured.metadata.appId, APP_ID);
    assert.equal(typeof captured.metadata.idempotencyKey, 'string');
    assert.ok((captured.metadata.idempotencyKey || '').length > 0);
