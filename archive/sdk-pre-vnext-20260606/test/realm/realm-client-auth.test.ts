...
test('Realm runtime_account auth only accepts RuntimeAccountService-backed token providers', () => {
  assert.throws(
    ...
    (error: unknown) => {
      const nimiError = asNimiError(error, { source: 'sdk' });
      assert.equal(nimiError.reasonCode, ReasonCode.SDK_REALM_CONFIG_INVALID);
      assert.equal(nimiError.actionHint, 'provide_runtime_account_service_access_token_provider');
      return true;
    },
  );
...
test('Realm static refresh requires explicit external_principal mode', async () => {
  await assert.rejects(
    ...
    (error: unknown) => {
      const nimiError = asNimiError(error, { source: 'sdk' });
      assert.equal(nimiError.reasonCode, ReasonCode.SDK_REALM_CONFIG_INVALID);
      assert.equal(nimiError.actionHint, 'set_external_principal_mode_for_realm_refresh');
      return true;
    },
  );
});
...
test('Realm refresh requests are bound through the generated AuthService operation', () => {
  ...
  const authSource = readSdkSource('src/realm/client-auth.ts');
  const refreshSource = readSdkSource('src/realm/client-refresh.ts');

  assert.doesNotMatch(clientSource, /\/api\/auth\/refresh/);
  assert.match(authSource, /executeGeneratedRealmRefreshToken/);
  assert.doesNotMatch(authSource, /\/api\/auth\/refresh/);
...
test('Realm.decodeTokenExpiryUnsafe parses valid and invalid JWTs', () => {
  ...
  try {
    const result = Realm.decodeTokenExpiryUnsafe(validJwt);
    ...
    assert.equal(Realm.decodeTokenExpiryUnsafe('not-a-jwt'), null);
    ...
    assert.equal(Realm.decodeTokenExpiryUnsafe(noExpJwt), null);

    // Malformed base64
    const malformedJwt = 'header.!!!invalid-base64!!!.signature';
    assert.equal(Realm.decodeTokenExpiryUnsafe(malformedJwt), null);
    ...
    assert.equal(Realm.decodeTokenExpiryUnsafe(base64UrlJwt)?.expiresAt, 1700000001 * 1000);
  } finally {
