...
test('product subprocess timeouts consume the registry Journey budget', () => {
  ...
  const driver = fs.readFileSync(path.join(repoRoot, 'tests/local-agent-product/conversation-report/product-driver.mjs'), 'utf8');
  assert.match(driver, /NIMI_LOCAL_AGENT_PRODUCT_JOURNEY_TIME_BUDGET_MS:\s*String\(journey\.time_budget_ms\)/u);
