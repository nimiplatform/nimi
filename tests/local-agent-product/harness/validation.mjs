...
export function validateArchitecture({ catalog, journeys, policy }) {
  const failures = [];
  if (catalog?.schema_version !== 'nimi.local-agent-product-acceptance-points/v2') failures.push('acceptance catalog schema_version must be v2');
  ...
  const points = array(catalog?.acceptance_points);
  const pointById = new Map();
  const layerCounts = Object.fromEntries(layerIds.map((layer) => [layer, 0]));
  let i7Count = 0;
  let i8Count = 0;
  for (const [index, point] of points.entries()) {
    const label = `acceptance point ${point?.leaf_id || index}`;
    checkRequiredFields(failures, label, point, [
      'leaf_id', 'owner_iteration', 'group', 'product_requirement', 'requirement_refs', 'spec_refs', 'source_kinds',
      'assertion_ids', 'minimum_sufficient_layer', 'evidence_class', 'execution_binding',
    ]);
    const id = text(point?.leaf_id);
    if (!id) continue;
    if (pointById.has(id)) failures.push(`duplicate or conflicting leaf owner for ${id}`);
    else pointById.set(id, point);
    for (const field of oldSchedulingFields) {
      if (Object.hasOwn(point || {}, field)) failures.push(`${id} acceptance catalog mixes old scheduling field ${field}`);
    }
    ...
    if (array(point?.assertion_ids).length === 0 || new Set(point.assertion_ids).size !== point.assertion_ids.length) failures.push(`${id} assertion_ids must be non-empty and unique`);
    if (point?.owner_iteration === 'I7') {
      i7Count += 1;
      if (point.group === 'P' || layer === 'L5') failures.push(`${id} I7 point cannot be P/L5`);
    } else if (point?.owner_iteration === 'I8') {
      i8Count += 1;
      if (point.group !== 'P' || layer !== 'L5') failures.push(`${id} I8 point must be P/L5`);
      if (point.execution_binding?.journey_id) failures.push(`${id} I8 point cannot bind an I7 Journey`);
      if (point.execution_binding?.suite_id !== 'live-behavior-i8') failures.push(`${id} I8 point must remain bound to live-behavior-i8`);
    } else failures.push(`${id} has invalid owner iteration ${point?.owner_iteration}`);
  }
  if (points.length !== 169 || pointById.size !== 169 || catalog?.leaf_count !== 169) failures.push(`acceptance catalog must contain exactly 169 unique leaves, got rows=${points.length} unique=${pointById.size}`);
  if (i7Count !== 145 || i8Count !== 24) failures.push(`acceptance owner counts must be I7=145/I8=24, got I7=${i7Count}/I8=${i8Count}`);
  const expectedLayerCounts = { L0: 28, L1: 43, L2: 44, L3: 30, L4: 0, L5: 24 };
  for (const [layer, expected] of Object.entries(expectedLayerCounts)) {
    if (layerCounts[layer] !== expected) failures.push(`minimum layer ${layer} expected ${expected}, got ${layerCounts[layer]}`);
  }
  ...
  for (const journey of journeyRows) {
    ...
    if (!id) {
      failures.push('journey row missing journey_id');
      continue;
    }
    ...
    if (!['L2', 'L3'].includes(journey.applicable_layer)) failures.push(`${id} applicable_layer must be L2 or L3`);
    ...
    for (const processId of ['provider', 'realm', 'runtime', 'desktop', 'zhiyu']) {
      if (!Number.isInteger(environment?.start_limits?.[processId]) || environment.start_limits[processId] < 0) failures.push(`${id} start limit ${processId} must be a non-negative integer`);
    }
    const checkpointMap = new Map();
    for (const checkpoint of array(journey.checkpoints)) {
      const checkpointId = text(checkpoint?.checkpoint_id);
      if (!checkpointId) {
        failures.push(`${id} checkpoint missing checkpoint_id`);
        continue;
      }
      ...
      if (!Array.isArray(checkpoint.prerequisite_ids) || !Array.isArray(checkpoint.covered_leaf_ids)) failures.push(`${id}/${checkpointId} must declare prerequisite_ids and covered_leaf_ids arrays`);
    }
    for (const checkpoint of checkpointMap.values()) {
      ...
      for (const leafId of array(checkpoint.covered_leaf_ids)) {
        const point = pointById.get(leafId);
        if (!point) failures.push(`${id}/${checkpoint.checkpoint_id} covers orphan leaf ${leafId}`);
        else if (point.owner_iteration !== 'I7' || point.minimum_sufficient_layer !== journey.applicable_layer) failures.push(`${id}/${checkpoint.checkpoint_id} covers leaf ${leafId} at conflicting owner/layer`);
        const rows = coverage.get(leafId) ?? [];
        rows.push({ journeyId: id, checkpointId: checkpoint.checkpoint_id });
        coverage.set(leafId, rows);
      }
    }
    checkCycles(failures, journey, checkpointMap);
  }
  if (!journeyById.has('full-chain-core')) failures.push('journey registry must contain full-chain-core');

  for (const point of points) {
    const id = point.leaf_id;
    const layer = point.minimum_sufficient_layer;
    const binding = point.execution_binding;
    if (!object(binding) || array(binding.checkpoint_ids).length === 0) {
      failures.push(`${id} execution binding must contain checkpoint_ids`);
      continue;
    }
    ...
    if (!journey) {
      failures.push(`${id} references missing journey ${binding.journey_id}`);
      continue;
    }
    ...
    for (const checkpointId of binding.checkpoint_ids) {
      ...
      else if (!checkpoint.covered_leaf_ids.includes(id)) failures.push(`${id} checkpoint ${checkpointId} has no reciprocal covered mapping`);
    }
    const covered = array(coverage.get(id)).filter((row) => row.journeyId === journey.journey_id).map((row) => row.checkpointId);
    if (!sameSet(covered, binding.checkpoint_ids)) failures.push(`${id} Journey covered mapping conflicts with catalog binding`);
  ...
  const coreLeafIds = new Set(array(coreJourney?.checkpoints).flatMap((checkpoint) => checkpoint.covered_leaf_ids));
  if (coreLeafIds.size !== 44) failures.push(`full-chain-core must cover 44 unique L2 leaves, got ${coreLeafIds.size}`);
  const extendedLeafIds = new Set(journeyRows.filter((journey) => journey.applicable_layer === 'L3').flatMap((journey) => journey.checkpoints.flatMap((checkpoint) => checkpoint.covered_leaf_ids)));
  if (extendedLeafIds.size !== 30) failures.push(`extended journeys must cover 30 unique L3 leaves, got ${extendedLeafIds.size}`);
  if (!array(coreJourney?.checkpoints).some((checkpoint) => checkpoint.covered_leaf_ids.length > 1)) failures.push('a Journey checkpoint must be able to cover multiple leaves');
  ...
  for (const path of ['tests/local-agent-product/harness/run-tier.mjs', 'tests/local-agent-product/harness/orchestrator.mjs', 'leaf_per_process', 'full_environment_starts_equals_leaf_count_times_repeat']) {
    if (!forbiddenPaths.includes(path)) failures.push(`execution policy must forbid old runner/path ${path}`);
  }
  ...
  for (const [gateId, command] of Object.entries(expectedGateCommands)) {
    if (policy?.gates?.[gateId]?.command !== command) failures.push(`execution policy gate ${gateId} must use ${command}`);
  }
  if (policy?.repeat_policies?.core?.stability !== 3
    || policy?.repeat_policies?.core?.clean_root_per_repeat !== true
    || policy?.repeat_policies?.core?.fresh_logical_identity_per_repeat !== true) failures.push('core stability must use three clean Journey repeats with fresh logical identities');
  ...
  if (policy?.gate_budgets_ms?.local_pr_l0_l1_l2_target !== 1500000
    || policy?.gate_budgets_ms?.required_pr_job_hard !== 1800000
    || policy?.gate_budgets_ms?.core_stability_hard !== 3600000
    || policy?.gate_budgets_ms?.extended_hard !== 2700000
    || policy?.gate_budgets_ms?.exhaustive_hard !== 5400000) failures.push('execution policy gate budgets drifted from P-TEST');
...
export function validateJourneyResult({ architecture, journey, result, expectedSourceState }) {
  ...
  const points = array(architecture?.catalog?.acceptance_points).filter((point) => point.execution_binding?.journey_id === journey?.journey_id);
  const pointById = new Map(points.map((point) => [point.leaf_id, point]));
  ...
  for (const leaf of array(result?.leafResults)) {
    ...
    for (const assertionId of array(leaf.assertionIds)) {
      const assertion = assertions.find((candidate) => candidate.assertionId === assertionId);
      if (!assertion || assertion.outcome !== 'passed') failures.push(`${id} assertion ${assertionId} lacks passing checkpoint evidence`);
    }
  ...
  for (const point of points) if (!leafMap.has(point.leaf_id)) failures.push(`missing leaf result ${point.leaf_id}`);
...
export function validateJourneyRepeatIsolation(results) {
  ...
  for (const [journeyId, rows] of byJourney) {
    ...
    const seenIdentity = new Map(['rootId', ...logicalIdentityFields].map((field) => [field, new Set()]));
    for (const result of rows) {
      ...
      for (const field of ['rootId', ...logicalIdentityFields]) {
...
export function validateSuiteResult({ architecture, result, expectedSourceState }) {
  ...
  const points = array(architecture?.catalog?.acceptance_points).filter((point) => ['L0', 'L1'].includes(point.minimum_sufficient_layer));
  const pointById = new Map(points.map((point) => [point.leaf_id, point]));
  const checkpointMap = new Map();
  for (const checkpoint of array(result?.checkpoints)) {
    const id = text(checkpoint?.checkpointId);
  ...
  for (const point of points) {
    if (!leafMap.has(point.leaf_id)) failures.push(`missing suite leaf result ${point.leaf_id}`);
    for (const checkpointId of point.execution_binding.checkpoint_ids) {
      ...
      else if (!array(checkpoint.leafIds).includes(point.leaf_id)) failures.push(`${checkpointId} does not reciprocally bind ${point.leaf_id}`);
    }
  }
