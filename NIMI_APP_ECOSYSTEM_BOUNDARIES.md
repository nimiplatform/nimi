...
- Product-control existing-record schema gate: the product-control owner must reject corrupted `~/.nimi/nimi.json` records whose `firstRun.installLevel` is outside `minimal | recommended`, and must route read-for-entry to `repair_required` without overwriting the file. This was first closed in the Desktop backend while that backend still carried the writer; after the hard-cut decision, the same invariant belongs to the Runtime product-control service.
...
- Runtime OpenApp readiness verifier: `OpenApp` consumes an admitted `OpenAppReadinessVerifier` for account-library and permission gates. Production Runtime wires the verifier to Runtime AccountService authenticated projection plus the spec-owned account-scoped `~/.nimi/accounts/<account-id>/apps/library.json` and `~/.nimi/accounts/<account-id>/permissions/grants.json` projections. Runtime must not treat package verification as account-library success and must not treat structurally valid `permission_scope_ref` rows as granted.
- SDK S-PERM permission client surface: SDK may own the typed app-facing permission client and transport contract for S-PERM logical operations `list`, `get`, `request`, `revoke`, `subscribe`, and `status`, including canonical grant-state / permission-scope validation and mock transport ergonomics. It must require an explicit `AIScopeRef` for every operation and must not infer permission grant truth. Tester Settings consumes this surface with a fixture transport as second-app proof; the concrete Runtime / Realm / Cognition authority behind the transport remains separate.
- Permission taxonomy/state vocabulary alignment: Platform `P-PERM-007` owns the canonical `permission_scope_ref.scopeFamily` set, including `ai_profile`; SDK `S-PERM-008` must mirror that spelling and must not revive `default_experience`. Platform `P-PERM-003` owns the canonical grant states, including `superseded`; Desktop account grants projection readers may accept `superseded` as a typed projection state, but this does not make Desktop a grant lifecycle authority.
...
- SDK speech stream routeDecision alignment: `runtimeStreamSpeechSynthesis` must not fall back from missing Runtime `started.routeDecision` to request `head.routePolicy`; artifact chunks remain `UNSPECIFIED` unless Runtime emits `CLOUD` or `LOCAL`. This is SDK projection cleanup over Runtime evidence, not provider/model route authority.
...
- Desktop runtime bridge generated shape / unary codec residue: generated Runtime bridge proto/method shapes, method-id constants, Rust build-time proto generation, and reusable unary payload encode/decode helpers are owned by `kit/shell/tauri`. Desktop must consume `nimi_shell_tauri::runtime_bridge` and must not keep `apps/desktop/src-tauri/src/runtime_bridge/**` generated files, `.gitignore` masks for that directory, a local `mod runtime_bridge`, Desktop `build.rs` proto generation, hardcoded Runtime method ids, or app-local generic base64/prost unary bridge helpers. Tester Tauri consumes the same Kit unary codec helper as the second-app proof.
...
- Kit chat runtime model boundary: Kit chat/headless helpers may assemble UI messages, history windows, metadata, and stream events, but must not synthesize executable model or route tokens. Missing model/selected binding must fail closed before Runtime execution; `model: "auto"` is not a Kit fallback. Apps may pass an explicit model only when it came from Runtime/SDK selected-binding evidence or explicit user/caller selection.
- Desktop HTTP proxy authorization boundary: Desktop `http_request` is a bounded OS/network helper, not a generic Realm/Runtime bypass. Renderer-supplied Authorization is allowed only for admitted Runtime/Realm origins from the host allow-list; arbitrary HTTPS proxying must not carry Authorization. SDK/Realm transports and managed connector auth must use typed SDK surfaces and named host adapters rather than treating `proxyHttp` as an app-owned REST escape hatch.

Closed hard-cut status that is often misread:

- Account app-library / grants projection boundary: `.nimi/spec/platform/kernel/tables/local-config-file-registry.yaml` admits the account-scoped `library_json` and `grants_json` paths. Runtime app lifecycle writes `library.json`, Runtime production OpenApp consumes `library.json` and `grants.json` for launch readiness, and Runtime `GetAccountAppLibrary` is the public authenticated account-library read surface. Desktop `account_app_library_get` is only a bridge adapter to that Runtime RPC; Desktop must not resolve account-scoped library paths, read `~/.nimi/accounts/<account-id>/apps/library.json`, expose `account_app_library_apply`, or observe lifecycle jobs to mutate renderer library truth. The remaining Desktop-side grants reader is a fail-closed projection consumer only until the deferred permission fabric replaces it; do not reclassify it as a canonical permission authority.
- Product-control Runtime hard-cut status: `RuntimeLocalService` now exposes the product-control record RPC seam for record read, selected-data-root read, record creation, data-root selection, install-level selection, device-scan completion, host-evidence recording, setup-state reconciliation, local-AI-ready evidence recording, and `ready_for_use` admission. Desktop Tauri commands for those operations are bridge adapters and no longer write `~/.nimi/nimi.json` directly in production. Runtime writes `productVersion` from its explicit startup version and must fail closed before record creation if that version is missing. Desktop production paths that need the selected `nimi_data` root, including `desktop_paths::resolve_nimi_data_dir`, support log export, and `nimi_data` cleanup, must consume Runtime selected-data-root projection rather than reading the product-control file. The old Desktop product-control file reader/writer/projection helpers are test-fixture-only and must not be reintroduced into production command paths. For Desktop-owned host evidence (`accountDefaultProfileRef`, `builtInAiConfigRefs`), Desktop may submit explicit backend-verified evidence JSON to Runtime; Runtime owns authenticated account verification, Runtime baseline/execution re-resolution, failure routing, intermediate state writes, and the atomic product-control record write. Remaining Desktop host-evidence slice: Account Default Profile materialization/read and built-in Desktop AIConfig materialization/read. Do not reclassify the completed Runtime product-control core/admission seam as Desktop-owned in future reviews.
- Built-in Desktop AIConfig host-evidence boundary: `builtInAiConfigRefs` remain Desktop chat feature-scope host evidence under `D-AIPC-013` / `P-AISC-006`, while the first-run baseline consumer set (`llama.cpp.cpu`, `speech.qwen3-asr.python`, `speech.qwen3-tts.python`) is Runtime-owned K-LENV-ACT-011 readiness/execution evidence. Desktop may use those consumer ids only as lookup keys into a verified `runtimeBaselineRef`; if any required consumer or `bound_asset_id` is absent, built-in AIConfig materialization must fail closed. Do not classify those consumer ids as Desktop-owned provider/model routing truth, and do not move Runtime baseline consumer policy into SDK or Desktop.

Known legal residues / watchlist:

- Desktop Avatar / Agent Center local resource residue: command execution currently admits `desktop_agent_center_*` and `desktop_agent_center_avatar_*` bounded resource commands, while `agent-avatar-surface-contract.md` forbids reviving desktop-local avatar carrier truth. Current audit found no active carrier drift: local Avatar asset refs are private configuration/import/validation evidence, Desktop launch handoff remains the D-LLM-072 triple, tests guard that no package/path/profile/token/account/binding/carrier/config is copied into handoff payloads, and the ordinary chat overlay carrier path is not mounted. Treat this as legal residue only while it remains private configuration/import evidence and unreachable as carrier truth; any re-mount or launch-ready use requires a spec fork or deletion/fencing. `start_with_chat` condition 6 must fail closed as `unknown` until a typed Runtime authorization projection exists; do not infer authorization from account readiness, Tauri handoff readiness, config presence, local asset validity, or prior same-agent traffic.
- Offline/group-chat legal residue: Desktop offline cache/outbox is a transport/cache facility, not Realm commit truth; entries are deleted only after Realm accepts them. Runtime-group-agent-to-Realm commit handoff stays split: SDK Runtime host surface creates the typed candidate commit payload from Runtime evidence, and SDK Realm group-chat extension submits it to Realm. Desktop must not collapse this into a generate-and-commit helper, direct REST bypass, renderer-local committed truth, or synthetic message success. Tester now proves the same SDK Runtime Realm group-message candidate surface as a materially different consumer; keep `check:no-app-realm-rest-bypass`, Desktop offline/outbox tests, and Tester SDK surface tests as the active guard set.
