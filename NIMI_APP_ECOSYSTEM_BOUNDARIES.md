...
- Desktop runtime bridge generated shape residue: generated Runtime bridge proto/method shapes are owned by `kit/shell/tauri`. Desktop must consume `nimi_shell_tauri::runtime_bridge` and must not keep `apps/desktop/src-tauri/src/runtime_bridge/**` generated files or a local `mod runtime_bridge`.
- Apps package projection storage-root boundary: Runtime `GetAppStorage` is the only source for app `data/cache/tmp` roots and active release root (`K-APP-022`). Kit `platform_projection::apps_packages` may project package identity/version/verification state from Runtime install evidence, but it must not persist or bridge storage roots in `~/.nimi/apps/packages.json` or SDK install-evidence loader payloads. Desktop Apps UI gets roots through SDK Runtime app lifecycle helpers; Desktop Avatar handoff gets roots through Runtime bridge `GetAppStorage`. Tester may use Kit projection builders for package state proof, but must not rely on package rows for app storage roots.
