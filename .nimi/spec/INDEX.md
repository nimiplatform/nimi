...
- 执行态计划、冻结包与结果不得写入 `/.nimi/spec/**`；若使用本地执行工作区，topic-bound execution materials 写入 `.local/work/<topic-id>/**`，local-only reports 写入 `.local/report/**`；tracked spec 不依赖具体 `.local` 文件。
