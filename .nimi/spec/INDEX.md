...
本 index 只覆盖当前 repo 下的 public authority surface：`nimi/.nimi/spec/**`。
它不是 private repo `nimi-realm/.nimi/spec/**` 或 `nimi-mods/spec/**` 的完整 cross-repo 总图。

但它仍是当前 public canonical 的 cross-repo upper-layer entrypoint。读取整个
Nimi 时，必须同时区分：

- public canonical：`nimi/.nimi/spec/**`
- private realm / backend / dashboard / creator-side authority：
  `nimi-realm/.nimi/spec/**`
- nested but independent mods authority：`nimi-mods/spec/**`

本 index 只负责把这些 owner surface 的上层读路径指清楚，不会把 private repo 或
mods workspace 的 semantic owner 静默吸回当前 public root。

跨层最短读法：

1. `.nimi/spec/platform/architecture.md` — 先看 cross-repo topology 与 layer framing
2. `.nimi/spec/platform/kernel/protocol-contract.md` — 再看六原语所在的 platform
   primitive layer
3. `.nimi/spec/realm/README.md` — 再落到 public realm semantic persistence read path
4. private repo `nimi-realm/.nimi/spec/**` — 最后进入 private realm / backend /
   creator-side authority
5. `nimi-mods/spec/INDEX.md` — 需要 mods workspace / shared chain authority 时再进入
   nested independent mods surface
...
- Phase 2 Draft（已有 kernel 契约，尚无独立 domain 文档）：
  ...
  - `RuntimeCognitionService`（`K-MEM-*`, `K-KNOW-*`）
  - `RuntimeAgentService`（`K-AGCORE-*`）
...
3. `.nimi/spec/runtime/kernel/runtime-agent-service-contract.md`（`K-AGCORE-004`, `K-AGCORE-015 ~ K-AGCORE-019`）
...
- 执行态计划、冻结包与结果不得写入 `/.nimi/spec/**`；human-authored topic lifecycle reports 写入 `.nimi/topics/{proposal|ongoing|pending|closed}/<topic-id>/**`；legacy execution evidence may still appear under `.local/report/**` as compatibility/local-only operational output；tracked spec 不依赖具体 local 文件。
...
Realm 规范采用 kernel + domain 的两层结构，覆盖 Realm Truth、World State、World History、human-only chat、creator asset、创作者经济与 transit。
...
4. `.nimi/spec/realm/kernel/chat-contract.md`
5. `.nimi/spec/realm/kernel/social-contract.md`
6. `.nimi/spec/realm/kernel/transit-contract.md`
7. 受影响 domain 文档（`truth.md` / `world-state.md` / `world-history.md` / `world.md` / `agent.md` / `chat.md` / `social.md` / `transit.md`）
...
3. `.nimi/spec/runtime/kernel/runtime-memory-service-contract.md`
4. `.nimi/spec/runtime/kernel/runtime-agent-service-contract.md`
5. `.nimi/spec/realm/kernel/transit-contract.md`
6. `.nimi/spec/realm/kernel/tables/commit-authorization-matrix.yaml`
7. `.nimi/spec/realm/realm-interop-mapping.md`
...
`nimi-cognition` 是独立 authority domain。runtime 可以 bridge/consume cognition，并暴露 overlap surface；但 cognition placement 不应被误读成第七执行宿主层。当前 public repo canonical 已把 continuity memory semantic owner 放在 cognition/runtime 路径；realm 在 public authority 中不再保留 active memory owner surface。后续 cross-repo cleanup 只允许收窄 wording 或 tooling，不得把 owner line 写回 realm。
...
7. `.nimi/spec/runtime/kernel/runtime-agent-service-contract.md`
