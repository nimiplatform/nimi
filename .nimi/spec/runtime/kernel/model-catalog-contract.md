...
`local` 在真实本地 voice workflow execution plane 接入并被显式 admitted 前，只允许声明 `audio.synthesize` 与 `audio.transcribe`：不得声明 `voice_workflow_models`、`model_workflow_bindings` 或对应 workflow capability。
