...
- 执行态内容不得进入 `/.nimi/spec/**`；若使用本地执行工作区，topic-bound 计划、baseline、packet 与 evidence 写入 `.local/work/<topic-id>/**`，local-only 执行报告写入 `.local/report/**`；tracked spec 不依赖具体 `.local` 文件。
