...
管理的独立 account-token-broker surface。它不是 scoped binding carrier，也不得作为 Desktop-launched Avatar embodiment binding 的一部分传递。

## K-BIND-002 Avatar Relation Tuple

Avatar binding 必须包含的字段：
...
| Protected access material，仅在 Runtime bridge 后存储 | `allowed-only-behind-runtime-bridge` | Desktop-launched Avatar / app 永远不得作为 durable token 读取 |
...
## K-BIND-008 Avatar `open_new` Anchor Ownership

Desktop-launched Avatar embodiment 不允许调用 `runtime.agent.anchors.open` 或任何 anchor 创建路径。

`open_new` 流程：

1. Desktop 或 Runtime 创建 / 预约 anchor。
2. Runtime 发出包含 `conversation_anchor_id` 的 binding。
3. Desktop-launched Avatar embodiment 仅通过 binding projection 消费 `conversation_anchor_id`。

Desktop-launched Avatar embodiment 不得拥有 anchor 创建、reservation、或所有权。

Avatar-as-primary-app 是未来可 admit 的独立模式。若被 admit，它必须作为 Runtime-brokered local first-party app 使用 account/token/binding 能力，仍不得拥有 refresh token、durable account session、或 independent Realm auth truth。
...
`runtime.agent` reactive chat seam（`K-RPC-004c`、`K-APP-008`）必须验证 caller 持有匹配的 scoped binding：
...
- `RuntimeAgentService.SubscribeAgentEvents` 若被 Desktop-launched Avatar / binding-only consume mode 用于 `runtime.agent.state.*`、`runtime.agent.presentation.*`、或 hook/state projection consumption，必须通过 request context 附带 scoped binding attachment；缺失或不匹配 fail-close。
...
任何场景下都不得回退到 anonymous subject、shared auth、Realm direct identity bootstrap、或 fixture mode。Full first-party app direct Realm data access 只允许通过 `K-ACCSVC-005` / `GetAccessToken` 的 Runtime-issued short-lived access token，不属于 binding fallback。
