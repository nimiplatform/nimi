...
所有条目从研究结论与经审计的来源摘要中提取，按优先级和类别组织，成熟后毕业到现有 spec 域（`.nimi/spec/runtime/`、`.nimi/spec/sdk/`、`.nimi/spec/desktop/`）。面向 Web 的条目不单独创建 `spec/web/`，而是毕业到 `.nimi/spec/desktop/web-adapter.md` 或相关 Desktop 投影文档。
若条目依赖 `.nimi/spec/platform/**` 或 `.nimi/spec/realm/**` 的既有协议、原语、经济或边界词汇，毕业时必须复用那些 kernel 规则与事实表，不得在目标域复制第二份规范正文。
