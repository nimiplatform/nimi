...
`agent-chat-behavior-contract.md`（`D-LLM-022` ~ `D-LLM-026`）。message-action envelope、
follow-up-turn action、以及 model-generated modality prompt payload 也不由 stream layer
拥有，相关真相固定来自
`agent-chat-message-action-contract.md`（`D-LLM-027` ~ `D-LLM-033`）。
...
action-driven follow-up 降格成同 turn text continuation。

若 execution 涉及 `follow-up-turn` 或 modality action delivery，stream consumer 也只能消费
`agent-chat-message-action-contract.md` 已解析出的 resolved outputs。stream layer 不得决定
...
无论是 image、video、还是受 `.nimi/spec/runtime/kernel/voice-contract.md` 约束的 voice
