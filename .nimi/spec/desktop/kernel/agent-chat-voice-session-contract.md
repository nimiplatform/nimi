...
- same-anchor text / voice continuity consumption semantics
...
继续拥有 cross-surface conversation continuity truth；
...
- 当前 conversation anchor / chat surface 是否进入一个 explicit user-triggered voice session
- 该 session 绑定到哪个 runtime-owned `conversation_anchor_id`
- 该 session 的 start / active / exit product meaning 是什么
- live voice use 与同一 conversation anchor text continuity 如何共存
...
- `.nimi/spec/runtime/kernel/agent-conversation-anchor-contract.md`
  （`K-AGCORE-033` ~ `K-AGCORE-035`）继续拥有 explicit conversation continuity anchor
  truth，而不是由 Desktop 再派生一份“同 agent 即同 session”语义
- `conversation-capability-contract.md`（`D-LLM-015` ~ `D-LLM-021`）继续拥有
...
## D-LLM-041 — Explicit Session Entry / Exit And Same-Anchor Continuity

broader voice session 的当前 entry semantics 固定为 explicit user-triggered，并且必须
附着到显式 runtime-owned `conversation_anchor_id`。

固定语义：

- voice session 只能由当前 anchor continuity 内的明确用户交互进入；不得由 route readiness、
  resolved delayed beat、notification、heartbeat、timer、或 background wake 自动触发
- voice session 属于当前 `conversation_anchor_id` continuity；不得在当前 admitted route 中被解释为独立 thread、独立
  assistant mode、或绕开既有 text / beat continuity 的平行 session
- 进入 session 后，text turn、voice playback beat、以及后续 voice input 仍属于同一
  conversation continuity surface；session 不得抹除 source anchor / turn / beat relation
...
- 若 entry / exit evidence 不合法、不可恢复、或无法绑定到当前
  `conversation_anchor_id`，必须 fail-close；
  不得伪造 active session
...
但 scope 仅限当前 `conversation_anchor_id` 内的 foreground session。
...
- transcript / caption 可以存在并被持久化为同一 `conversation_anchor_id` 的 conversation evidence
...
- transcript / caption 必须可恢复到当前 `conversation_anchor_id` / turn / beat /
  session relation；不得
  变成 detached inspection artifact
- transcript 是否 reveal、何时 reveal、以及 reveal 后如何继续保持 anchor-scoped conversation evidence，
  必须遵循同一份 desktop session rule；consumer 不得各自重算一份 hidden / shown truth
- 缺合法 relation、缺 required transcript fields、或无法稳定绑定当前
  `conversation_anchor_id` 时必须
...
- admitted broader voice session 不等于 richer live-assistant voice 已全面落地；当前 route
  只承认 explicit entry、same-anchor continuity、admitted foreground listening modes、
...
- 一旦 active `conversation_anchor_id` 改变、app 失去 foreground、screen 锁定、host suspend、microphone
...
- `.nimi/spec/runtime/kernel/voice-contract.md` — runtime voice workflow / voice
  asset boundary
- `.nimi/spec/runtime/kernel/agent-conversation-anchor-contract.md` — runtime-owned
  conversation continuity truth
