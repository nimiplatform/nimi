...
| `explore-surface-contract.md` | `D-EXPL-*` | Explore 统一 Realm 发现表面产品语义：三区结构（Worlds / Personas / Activity）、WorldCore card/detail、WorldCharacterCore 与 RealmPersona source-detail 路由及 source-generic action model、local materialization intent；不拥有 packet/challenge/snapshot/context、LocalAgent materialization、WorldCore canonical truth |
...
- `tables/realm-source-materialization-actions.yaml`
