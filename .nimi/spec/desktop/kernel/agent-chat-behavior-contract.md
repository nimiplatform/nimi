...
Desktop agent chat may consume `runtime.agent` state and memory projections, but it does not own canonical agent identity, canonical agent memory, life scheduling, or autonomy truth.
...
- desktop-local deferred continuation UI / pending indicator 不得被扩写为 runtime-owned
  life scheduling truth
