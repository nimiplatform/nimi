...
image、voice、video action 在 Desktop product semantics 中共享一份统一的 action
envelope contract；不得为三种 action 各自定义平行 product trigger truth。
...
本契约中的 `action` 只覆盖当前 assistant turn 提交后立即执行的 post-turn modality
actions。deferred continuation 不再属于 message-action contract。

固定语义：

- admitted `modality` 固定为 `image` / `voice` / `video`
