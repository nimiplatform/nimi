...
This contract defines the realm's role as the OAuth 2.0 authorization server for native and public clients within `nimi-realm`. It governs the two endpoints that admitted native clients use to obtain account session material via the PKCE-protected authorization code flow.
...
Realm is the OAuth 2.0 authorization server for native and public clients in `nimi-realm`. The PKCE-protected authorization code flow is the sole admitted method for native clients to obtain realm session material; no other OAuth flow is admitted on the endpoints governed by this contract.
