...
Nimi2D does not own Avatar runtime embodiment, renderer backend execution,
Runtime presentation timeline truth, Desktop stream lifecycle, Realm Agent
Studio product workflow, or upstream image segmentation and occlusion
inpainting.
...
- Nimi2D owns generated asset/package contracts only
- Avatar owns runtime embodiment backend execution
- Runtime owns typed projection and PresentationTimeline truth
- Desktop owns stream lifecycle/UI consumption only
- upstream layer generation is outside Nimi2D
...
- shared implementation helpers under `@nimiplatform/nimi2d/runtime` do not
  move Avatar runtime authority into `.nimi/spec/nimi2d/**`
- stop rules for drift into runtime, product UI, raw images, raw APML, or false
  capability claims
