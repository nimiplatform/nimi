...
| openai_codex | https://chatgpt.com/backend-api/codex | 否 |
| openai_compatible | — | 是 |
...
| spark_reasoning | https://spark-api-open.xf-yun.com/x2 | 否 |
...
| worldlabs | https://api.worldlabs.ai | 否 |
...
| openai_codex | nimillm | 是 | 否 | default_or_explicit |
...
| spark_reasoning | nimillm | 是 | 是 | default_or_explicit |
...
| worldlabs | nimillm | 是 | 是 | default_or_explicit |
...
| AI_LOCAL_SPEECH_PREFLIGHT_BLOCKED | 560 | LOCAL_SPEECH |
| AI_LOCAL_SPEECH_DOWNLOAD_CONFIRMATION_REQUIRED | 561 | LOCAL_SPEECH |
| AI_LOCAL_SPEECH_ENV_INIT_FAILED | 562 | LOCAL_SPEECH |
...
| AI_LOCAL_SPEECH_PREFLIGHT_BLOCKED | FAILED_PRECONDITION | local_speech_bundle_probe_or_consume | grpc_status_or_payload_ok_false |
| AI_LOCAL_SPEECH_DOWNLOAD_CONFIRMATION_REQUIRED | FAILED_PRECONDITION | local_speech_bundle_probe_or_consume | grpc_status_or_payload_ok_false |
| AI_LOCAL_SPEECH_ENV_INIT_FAILED | FAILED_PRECONDITION | local_speech_bundle_prepare | grpc_status |
| AI_LOCAL_SPEECH_HOST_INIT_FAILED | FAILED_PRECONDITION | local_speech_bundle_prepare | grpc_status |
| AI_LOCAL_SPEECH_CAPABILITY_DOWNLOAD_FAILED | FAILED_PRECONDITION | local_speech_bundle_materialize | grpc_status |
| AI_LOCAL_SPEECH_BUNDLE_DEGRADED | FAILED_PRECONDITION | local_speech_bundle_probe_or_consume | grpc_status_or_payload_ok_false |
...
### 13.16 Runtime — Connector Auth Profiles

| Profile | Auth Kind | Allowed Providers | Header Behavior |
|---|---|---|---|
| openai_codex | oauth_managed | openai_codex | codex_oauth |
| anthropic | oauth_managed | anthropic | anthropic |
| qwen_oauth | oauth_managed | openai_compatible | none |
...
| runtime-host | 4 | Wire mod SDK host, hook runtime, speech route resolver, and core data capabilities without executing mod entries |
| runtime-mod-host-ready | 5 | Prepare Desktop mod host state and schedule deferred post-ready/on-demand hydration; mod entry import and setup are not bootstrap readiness gates |
...
| runtime_local_assets_install_verified | Install a verified asset through the runtime-authoritative verified asset catalog; ordinary-user local speech flows still require explicit Download confirmation and runtime-owned bundle projection |
...
| runtime_local_profiles_apply_status | Read the durable terminal or in-flight status for a local AI profile apply session |
| runtime_local_profiles_apply_sessions | List durable local AI profile apply session statuses for reload-safe host and mod observation |
...
| runtime_local_services_install | Host service install helper; shipped product install truth must come from RuntimeLocalService and must not imply Desktop-owned local speech install truth |
| runtime_local_services_start | Host service start helper; shipped product lifecycle truth must come from RuntimeLocalService and must not bypass explicit Download-gated speech bundle flows |
| runtime_local_services_stop | Host service stop helper; shipped product lifecycle truth must come from RuntimeLocalService |
| runtime_local_services_health | Host service health helper; shipped product health truth must come from RuntimeLocalService and may only feed a runtime-owned speech bundle projection |
...
| runtime_local_assets_install | Install an asset from catalog parameters; execution truth is RuntimeLocalService and must not create Desktop-owned speech bundle truth |
...
| runtime_local_assets_scaffold_orphan | Host-local orphan intake helper that promotes an explicitly reviewed loose asset file into runtime-managed storage via RuntimeLocalService |
...
| world | loadWorlds, loadWorldDetailById, loadWorldAgents, loadWorldDetailWithAgents, loadWorldSemanticBundle, loadWorldEvents, loadWorldLorebooks, loadWorldResourceBindings, loadMainWorld, loadWorldLevelAudits | World raw read surfaces for listing, detail, semantic and audit data; page-level display authority stays downstream in bounded consumer seams |
...
| feed | loadPostFeed, loadPostById, loadLikedPosts, createPost, createImageDirectUpload, createVideoDirectUpload, finalizeResource, likePost, unlikePost, updatePostVisibility, deletePost, createReport | Social feed posts, post-local interactions, moderation entry points, and resource uploads; direct-upload sessions require finalizeResource after S3 upload before createPost; createPost references resourceId only |
| explore | loadExploreAgents, loadExploreFeed, loadMoreExploreFeed, loadAgentDetails | Explore public agent recommendations/search, discovery feed pagination, plus raw public agent detail read used by preview and downstream display seams |
...
| agent | loadMyAgents | Agent listing only; final agent-detail display authority stays downstream in bounded display seams, and Agent LLM route/memory stays in host data capabilities for mods |
