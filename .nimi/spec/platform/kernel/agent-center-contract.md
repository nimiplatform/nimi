...
- typed adapter contracts for Runtime Agent execution config, readiness, inspect, autonomy, memory projection, and appearance host-local custody
- failure-state, disabled-state, loading-state, and narrow-layout behavior

Kit does not own:

- Runtime Agent execution, lifecycle, memory admission, event truth, transcript truth, or execution config persistence
- SDK transport or scoped binding custody
- Desktop/Zhiyu local file custody for imported Live2D/VRM/background assets
...
- inject typed host-local appearance/avatar asset adapters
...
- write Runtime Agent execution config outside Runtime/SDK execution-config mutation
...
- Model: committed Runtime Agent execution config, binding editor, readiness projection, revision conflict recovery, and per-capability reason detail.
...
- Appearance: Live2D/VRM/avatar local asset selection, validation, calibration reference, background, reference image/voice projection, and bounded host-local asset custody.
...
- `appearance` and `avatar_asset` are admitted only as bounded host-local asset custody.
...
`audio.synthesize` readiness is Runtime-owned. In this wave, Agent Center renders `audio.synthesize` as a read-only Runtime projection with `not_configured`, `unavailable`, or explicit fail-closed absent state. Editing audio bindings is not admitted by this contract. Apps must not create playable pseudo voice artifacts or app-local voice synthesis truth.
...
| Desktop `ChatSettingsPanel` injected as `modelContent` | Runtime/SDK execution config editor in Kit; generic AI settings outside Agent Center | Replace with Kit Runtime Agent execution config section; keep generic AI config as separate Desktop settings surface if still needed. |
...
| Desktop `avatarContent` / `localAppearanceContent` | Kit appearance section plus typed host-local adapter and Avatar boundary | Replace arbitrary content with typed adapter-driven Kit controls. |
...
| Zhiyu `ZhiyuAiConfigSettings` inside model tab | Runtime/SDK execution config editor in Kit | Replace with Kit Runtime Agent execution config section. |
...
| Zhiyu/desktop local avatar/background file bridge | Host-local adapter plus Avatar contract | Keep as typed adapter consumed by Kit appearance section. |

Every current Agent Center child/panel must map to Kit core, Runtime projection/setting, host-local adapter, Avatar boundary, or app-only outside-Agent-Center placement. Unmapped surfaces block implementation.
