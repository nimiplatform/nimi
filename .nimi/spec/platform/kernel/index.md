...
| `nimi-app-scaffolding-contract.md` | `P-SCAF-*` | Nimi App scaffolding and app-authoring authority: app-authoring ownership, accepted A0-A5 inputs, `standalone` / `workspace-app` profile split, `nimi.app.yaml` as submitted-manifest input only, build-profile requirements, permission declarations as transparency/review input only, managed file taxonomy, `nimi-app create|doctor|update`, doctor/update developer-scaffold semantics, nimicoding projection ownership, local acceptance harness as local evidence only, no public Nimi App admission/install truth from scaffolding, explicit `workspace-app` app-slice admission exception under `P-APP-*`, and A5 model-test follow-up dependency |
