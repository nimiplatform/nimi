...
| `nimi-home-contract.md` | `P-HOME-*` | Nimi Home product entry/shell authority, hosted-shell binding, non-owner rules, surface registry requirement, AIProfile selection consumption, Agent Chat placement boundary, mandatory `AIScopeRef`, no-private-path rule, Library/Discovery non-owner rule, first-screen rule |
