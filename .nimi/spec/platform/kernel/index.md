...
| `SIM` | Nimi Ecosystem Simulator product and selected-source integration contract | `nimi-ecosystem-simulator-contract.md` |
...
| `NAPP` | Nimi App admission contract, including independent macOS protected-local admission | `nimi-app-admission-contract.md`, `nimi-app-local-admission-contract.md`, `macos-protected-local-admission-contract.md` |
...
| `P-DOPEN-*` | Running Desktop Open Intent authority (other-app-to-running-Desktop closed intent protocol, running-only boundary, Desktop target catalog references, sourceHost semantics, result code producer mapping, and executable cross-domain evidence closure) |
| `P-SIM-*` | Nimi Ecosystem Simulator authority (independent product/build, selected-source protocol, canonical renderer factory, App Adapter, deterministic presentation state, shared graph, browser effects, integrated qualification, performance, and release evidence) |
...
| `nimi-ecosystem-simulator-contract.md` | `P-SIM-*` | Independent Nimi Ecosystem Simulator product, current-source module protocol, canonical renderer and App Adapter boundaries, deterministic presentation state, cross-App interaction, final graph, browser effects, qualification, performance, and release evidence |
...
| `macos-protected-local-admission-contract.md` | `P-NAPP-037` | Conjunctive independent production macOS admission plus the isolated, non-promotable local-CA RuntimeDev/Desktop/Electron development profile; Tauri and production signing remain independent gates |
...
| `app-permission-contract.md` | `P-PERM-*` | Five mutually exclusive app authority classes, the closed sixteen-id product permission catalog, zero-prompt app-private authority, owner-selected resources, audit and fail-close admission rules |
...
| `nimi-app-developer-workflow-contract.md` | `P-DEV-*` | Developer repository/submission workflow, one production Developer Mode and Dev Trust Set, run_once/allow_project consent lifetimes, production account mediation, native execution disclosure, no persistent dev autostart, and external-AI-host-exclusive workflow lifecycle boundary |
...
| `desktop-open-intent-contract.md` | `P-DOPEN-*` | Running Desktop Open Intent authority: running-only protocol, Desktop target catalog references, sourceHost enum, result code producer mapping, and executable cross-domain evidence closure |
...
| `config/desktop-shell-ui-home-surfaces.yaml` | `nimi-home-contract.md`, `.nimi/spec/canonical/desktop/shell-ui.authority.yaml` | Non-authoritative Desktop-hosted Nimi Home surface placement machine projection; product ownership remains in `rule.nimi.desktop.shell-ui.r049..r061` |
...
| `tables/nimi-app-registry.yaml` | `nimi-app-admission-contract.md`, `nimi-app-local-admission-contract.md` | Admitted Nimi App registry rows with typed references to trust tier, factory AIProfile selection, canonical capability ids, local compute packs, runtime registration mode, public `{id, reason}` permission requirements (empty is valid), admission status, and distinct local-record inventory projection |
...
| `tables/protected-local-executable-trust-sets.yaml` | `macos-protected-local-admission-contract.md`, `nimi-app-admission-contract.md` and Runtime `K-PLOCAL-003/005` | Closed bidirectional production/non-product Desktop、Linux control-carrier、Runtime service trust sets, signed release-record schema, launch/config authority and build isolation |
| `tables/nimi-app-local-development-admission.yaml` | `P-NAPP-035`, Runtime `K-PLOCAL-009` / `K-APP-027`, Desktop `D-IPC-019/020`, Kit `P-KIT-046`, scaffolding `P-SCAF-018`, SDK `S-TRANSPORT-014` | Production Developer Mode mutable-project authorization, isolated principal/record, zero-permission session, run_once/allow_project consent lifetimes, fixed-service process/session rotation, native risk disclosure, base entitlements and reserved protected-operation ceiling |
...
| `tables/simulator-authority-boundaries.yaml` | `nimi-ecosystem-simulator-contract.md` | Canonical owner matrix for Simulator-owned authority, amended owner contracts, no-change references, excluded product truth, and evidence ownership |
| `tables/simulator-module-contract.yaml` | `nimi-ecosystem-simulator-contract.md` | Closed `nimi.simulator.module/v1`, operation, interaction, canonical renderer factory, Adapter, result, identifier, and path contract |
| `tables/simulator-source-policy.yaml` | `nimi-ecosystem-simulator-contract.md` | Simulator-owned descriptor, source location, immutable object identity, inventory, materialization, digest, resolver, and generated registry contract |
| `tables/simulator-scenario-contract.yaml` | `nimi-ecosystem-simulator-contract.md` | Closed tracked Scenario artifact, initial state, exact module-data coverage, generic launch intents, readiness predicates, generated projection, and artifact digest binding |
| `tables/simulator-state-engine-policy.yaml` | `nimi-ecosystem-simulator-contract.md` | Deterministic snapshot, FIFO transaction, logical clock, async reservation, random, stream, lifecycle, reset, readiness, and replay policy |
| `tables/simulator-error-codes.yaml` | `nimi-ecosystem-simulator-contract.md` | Closed Simulator control-plane error enum, failure-scope/state semantics, and SDK error-reachability boundary |
| `tables/simulator-mandatory-singletons.yaml` | `nimi-ecosystem-simulator-contract.md` | Closed shared-package identity catalog and exact root-lock/resolver authority for the final selected-source graph |
| `tables/simulator-browser-effects.yaml` | `nimi-ecosystem-simulator-contract.md` | Closed browser-global effect classification, permitted owner/port rules, selected-source prohibitions, and required proof classes |
| `tables/simulator-listener-families.yaml` | `nimi-ecosystem-simulator-contract.md` | Closed global listener-family ownership, one-listener coordination, subscriber order, and lifecycle cleanup contract |
| `tables/simulator-performance-policy.yaml` | `nimi-ecosystem-simulator-contract.md` | Structural performance invariants, fixed measurement scenarios, calibration artifact schema, and numeric-budget admission rules |
| `tables/rule-evidence.yaml` | All rule-bearing contracts in the Document Ownership Matrix | Platform formal rule → executable evidence mapping; local evidence and generated self-attestation are excluded |
