...
| `governance-contract.md` | `P-GOV-*` | License matrix, release gates, governance tasks, final-authority-only spec language |
...
| `tables/nimi-app-identity-surfaces.yaml` | `nimi-app-admission-contract.md` | Canonical active app identity surface mapping: `app_id` / SDK `appId` / Runtime caller app id equality, Tauri `identifier` derivation from `ai.nimi.apps.<app_id>`, and explicit app source roots for Desktop, Web, Avatar, Storybook, and Tester |
