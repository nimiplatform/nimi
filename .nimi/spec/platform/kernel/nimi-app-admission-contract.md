...
**A.1 authority disposition:** Windows is admitted independently through K-PLOCAL-008. Runtime owns the durable short-lived launch record and atomic installed session; Platform installer/release authority supplies the active release digest and signing policy; Desktop/Kit carry only the host-private duplicated pipe handle and explicit child handle list. `OpenApp` returns a non-authorizing launch correlation id, never a portable ticket. Renderer metadata, ordinary gRPC, local adoption alone, tester registration and app self-report remain non-authorizing. macOS/Linux remain fail-closed until separately admitted and cannot claim A.1 completion.
