...
2. 再读 `.nimi/spec/desktop/external-agent.md`，查看 Desktop 对 ExternalAgent 的落地映射。
3. 然后读 `.nimi/spec/runtime/kernel/auth-service.md` 与 `.nimi/spec/runtime/kernel/grant-service.md`，确认授权链路与执行边界。
...
- 不在本文件记录执行态结果；执行证据只能落入 local execution report route patterns（如 `.local/report/**`）。
