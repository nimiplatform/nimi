...
export function collectLocalDevelopmentSupervisorParityViolations(sources) {
  const violations = [];
  requireBoth(sources.tsHost, /HEARTBEAT_MS = 3_000/u, sources.rustMod, /PRESENCE_HEARTBEAT_INTERVAL_MS: u64 = 3_000/u, 'heartbeat must remain 3000ms', violations);
  ...
  for (const [tsField, rustField] of [
    ...
    ['desktopPid: process.pid', 'desktop_pid: std::process::id()'],
    ['endpoint: this.endpoint', 'endpoint: endpoint.to_string()'],
    ['startedAt: this.startedAt', 'started_at: started_at.to_string()'],
    ['lastHeartbeatAt: new Date().toISOString()', 'last_heartbeat_at:'],
  ]) {
    if (!sources.tsHost.includes(tsField) || !sources.rustDomain.includes(rustField)) {
