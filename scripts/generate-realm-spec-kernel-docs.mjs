...
function ensureDerivedRuleCatalog() {
  ...
  if (normalizeYaml(existing) !== expected) {
    const message = `Realm rule catalog drift detected: ${relativeToRoot(catalogPath)} must be regenerated by the external projection pipeline`;
    if (CHECK_MODE) {
    ...
    throw new Error(
      [
        message,
        'this repository cannot write .nimi/spec/realm/** as implementation authority',
        'produce a verified projection update and rerun the Realm projection guard',
      ].join('\n')
    );
  }
  if (CHECK_MODE) {
