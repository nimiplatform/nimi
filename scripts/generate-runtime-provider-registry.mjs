...
const supplementalProviders = [
  {
    ...
    supports: { text: true, embed: true, image: true, video: true, tts: true, stt: true, music: false, realtime: false, decide: false, voiceReferenceAudio: false, voiceTextDescription: false },
  },
  {
  ...
  },
  {
    ...
    inventoryMode: 'static_source',
    dynamicInventory: null,
    supports: { text: false, embed: false, image: false, video: false, tts: true, stt: true, music: false, realtime: false, decide: false, voiceReferenceAudio: false, voiceTextDescription: false },
  },
];
...
function capabilityFlags(sourceDoc) {
  ...
  for (const capability of dynamicCapabilities) {
    ...
    if (capability === 'realtime.interact') realtime = true;
    if (capability === 'text.decide') decide = true;
  }
  ...
  for (const model of models) {
    ...
    for (const capability of effectiveCaps) {
      ...
      if (capability === 'realtime.interact') {
        realtime = true;
      }
      if (capability === 'text.decide') {
        decide = true;
      }
    }
  }
...
function renderProviderRecord(record) {
  return `\t${goString(record.id)}: {\n` +
    ...
    `\t\tSupportsRealtime: ${boolLiteral(record.supports.realtime)},\n` +
    `\t\tSupportsDecide: ${boolLiteral(record.supports.decide)},\n` +
...
function renderGoFile(records, tokenProbeProviders) {
...
async function main() {
  ...
  for (const supplemental of supplementalProviders) {
    const existing = recordsByID.get(supplemental.id);
    if (existing) {
      recordsByID.set(supplemental.id, {
        ...existing,
        ...supplemental,
        capabilities: existing.capabilities,
        supplemental: existing.supplemental,
      });
      continue;
    }
    recordsByID.set(supplemental.id, {
  ...
  const formatted = spawnSync('gofmt', ['-w', generatedPath], {
    cwd: repoRoot,
    encoding: 'utf8',
  });
  if (formatted.status !== 0) {
    throw new Error(
      `gofmt generated provider registry failed: ${formatted.stderr || formatted.error || `exit ${formatted.status}`}`,
    );
  }
  await fs.writeFile(
