...
const specs = [
  ...
  {
    input: 'desktop-command-execution-classification.yaml',
    source: 'config/desktop-command-execution-classification.yaml',
...
function header(title, sourceName) {
  return normalizeMarkdown([
    ...
    `> Source: \`${sourceName.includes('/') ? sourceName : `.nimi/spec/desktop/kernel/tables/${sourceName}`}\``,
    '',
  ].join('\n'));
}
...
function renderGeneratedIndex(entries) {
  ...
  for (const entry of entries) {
    out += `\n| \`${entry.output}\` | \`${entry.source || `tables/${entry.input}`}\` |`;
  }
  out += '\n';
...
async function main() {
  ...
  for (const spec of specs) {
    const inputPath = spec.source
      ? path.join(repoRoot, spec.source)
      : path.join(tablesDir, spec.input);
    const outputPath = path.join(outDir, spec.output);

    const parsed = await parseYamlFile(inputPath);
    const rendered = spec.render(parsed, spec.source || spec.input);
