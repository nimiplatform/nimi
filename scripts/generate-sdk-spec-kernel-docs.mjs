...
const tablesDir = path.join(repoRoot, '.nimi', 'spec', 'sdk', 'kernel', 'tables');
const outDir = path.join(repoRoot, '.nimi', 'spec', 'sdk', 'kernel', 'generated');
...
function header(title, sourceName) {
  return normalizeMarkdown([
    ...
    `> Source: \`.nimi/spec/sdk/kernel/tables/${sourceName}\``,
    '',
  ].join('\n'));
}
...
async function main() {
  ...
  if (checkMode) {
    ...
    if (drifted.length > 0) {
      ...
      process.stderr.write('run `pnpm exec nimicoding generate-spec-derived-docs --profile nimi --scope sdk` to regenerate.\n');
      process.exitCode = 1;
      return;
    }
