...
const surfacesPath = path.join(repoRoot, 'config/desktop-shell-ui-renderer-design-surfaces.yaml');
const sidebarsPath = path.join(repoRoot, 'config/desktop-shell-ui-renderer-design-sidebars.yaml');
const overlaysPath = path.join(repoRoot, 'config/desktop-shell-ui-renderer-design-overlays.yaml');
const allowlistsPath = path.join(repoRoot, 'config/desktop-shell-ui-renderer-design-allowlists.yaml');
...
for (const filePath of filesToCheck) {
  ...
  const rawBrandMatches = collectMatches(content, /#1476D4|#45B8D6|#4ECCA3/giu);
  if (rawBrandMatches.length > 0 && !isAllowlisted(filePath, 'raw_color', content)) {
    hardFailures.push(`${fileRel}: raw brand color (#1476D4 current / #45B8D6, #4ECCA3 legacy) is forbidden on baseline/shared surfaces`);
  }
