...
function resolveMethodName(serviceMethodCounts, service, operationIdRaw) {
  const baseMethodName = toLowerCamel(operationIdRaw);
...
export function parseRealmOperations(spec) {
  ...
  for (const [pathName, pathItem] of Object.entries(paths)) {
    ...
    for (const rawMethod of OPERATION_METHODS) {
      ...
      const methodName = resolveMethodName(serviceMethodCounts, service, operationIdRaw);
