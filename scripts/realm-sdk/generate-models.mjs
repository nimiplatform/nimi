...
export function writeGeneratedModels(repoRoot, spec) {
  ...
  const modelMapLines = [
    ...
    ...modelEntries.map(({ schemaName, symbolName }) => (
      `  ${symbolName}: components['schemas']['${escapeSingleQuoteString(schemaName)}'];`
    )),
    '};',
    '',
  ];
  writeFileSync(path.join(generatedDir, 'model-map.ts'), modelMapLines.join('\n'), 'utf8');
