...
if (process.platform !== 'darwin' || process.arch !== 'arm64') throw new Error('macOS development security helper requires native Apple Silicon macOS');
...
export const PRIVILEGED_HELPER_SOURCES = Object.freeze([
  generated,
  path.join(source, 'Support.swift'),
  path.join(source, 'Installer.swift'),
...
runReleaseCommand('/usr/bin/xcrun', ['swift-frontend','-c','-O','-whole-module-optimization','-parse-as-library','-target','arm64-apple-macos13.0','-sdk',sdk,'-module-name','NimiMacOSDevSecurityInstaller','-o',objectPath,...PRIVILEGED_HELPER_SOURCES], { cwd: root, inherit: true });
