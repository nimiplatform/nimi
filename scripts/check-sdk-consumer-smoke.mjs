...
async function writeSmokeEntry(appDir) {
  const source = [
    ...
    "if (typeof createModRuntimeClient !== 'function') throw new Error('mod runtime export invalid');",
    ...
    "const runtimeCalls = [];",
    "const runtimeClient = createModRuntimeClient('world.nimi.sdk-smoke', {",
    "  runtime: {},",
    "  runtimeHost: {",
    "    route: {",
    "      listOptions: async (input) => { runtimeCalls.push(['route.listOptions', input]); return { capability: input.capability, selected: { source: 'token-api', connectorId: 'connector-1', model: 'gpt-4.1-mini' }, resolvedDefault: { source: 'token-api', connectorId: 'connector-1', model: 'gpt-4.1-mini' }, localRuntime: { models: [] }, connectors: [] }; },",
    "      resolve: async (input) => { runtimeCalls.push(['route.resolve', input]); return { capability: input.capability, source: 'token-api', provider: 'openai', model: input.binding?.model || 'gpt-4.1-mini', connectorId: input.binding?.connectorId || 'connector-1' }; },",
    ...
    "await runtimeClient.ai.text.generate({ prompt: 'hello', binding: { source: 'token-api', connectorId: 'connector-1', model: 'gpt-4.1-mini' } });",
    "if (runtimeCalls[0]?.[1]?.modId !== 'world.nimi.sdk-smoke') throw new Error('mod runtime modId injection invalid');",
    "try {",
    "  await import('@nimiplatform/sdk/mod/ai');",
    "  throw new Error('legacy mod ai export still exists');",
    "} catch (error) {",
    "  const message = String(error?.message || error);",
    "  if (message.includes('legacy mod ai export still exists')) throw error;",
    "}",
