...
async function discoverPrimitiveFragmentRels() {
  const dir = path.join(tablesDir, 'nimi-ui-primitives');
  let entries = [];
  try {
...
function normalizeFragmentRel(fragmentRel) {
  ...
  if (!rel) return rel;
  if (rel.startsWith('.nimi/') || rel.startsWith('apps/') || rel.startsWith('kit/')) {
    return rel;
  }
  return path.posix.join('.nimi/spec/platform/kernel/tables', rel);
}
...
function renderRootDesign(frontMatter) {
  ...
  return [
    ...
    '',
    'Canonical authority remains `.nimi/spec/platform/kernel/**`; this root file exists so tools that look for repository-level `DESIGN.md` can discover the Kit projection without inventing a second design source.',
    '',
    '## Overview',
    '',
    ...
    '- Do treat `.nimi/spec/platform/kernel/**` as canonical authority.',
...
function renderTailwindThemeCss(fullProjection) {
  const lines = [
    ...
    '/* Source: .nimi/spec/platform/kernel/tables/nimi-ui-*.yaml */',
...
async function loadProjectionInputs() {
  const tokensDoc = await readYamlRel('.nimi/spec/platform/kernel/tables/nimi-ui-tokens.yaml');
  const themesDoc = await readYamlRel('.nimi/spec/platform/kernel/tables/nimi-ui-themes.yaml');
  const primitivesDoc = await readYamlRel('.nimi/spec/platform/kernel/tables/nimi-ui-primitives.yaml');
  const compositionsDoc = await readYamlOptionalRel('.nimi/spec/platform/kernel/tables/nimi-ui-compositions.yaml', { components: [] });
  const adoptionDoc = await readYamlOptionalRel('.nimi/spec/platform/kernel/tables/nimi-ui-adoption.yaml', { modules: [] });
  const allowlistsDoc = await readYamlOptionalRel('.nimi/spec/platform/kernel/tables/nimi-ui-allowlists.yaml', { items: [] });
  const kitRegistry = await readYamlRel('.nimi/spec/platform/kernel/tables/nimi-kit-registry.yaml');
