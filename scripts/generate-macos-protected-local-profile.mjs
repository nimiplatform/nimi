...
function buildProfile(input) {
  ...
  const repairSuccessReceipt = requiredObject(
    legacyRepair.success_receipt,
    'Runtime legacy repair success receipt',
  );
  ...
  const result = {
    ...
    runtimeLegacyRepairTerminalCommitPolicy: requiredText(legacyRepair.terminal_commit_policy, 'Runtime legacy repair terminal commit policy'),
    ...
    runtimeLegacyRepairInvocationDeadline: requiredText(legacyRepair.invocation_deadline, 'Runtime legacy repair invocation deadline'),
    runtimeLegacyRepairFailureEvidence: requiredText(legacyRepair.failure_evidence, 'Runtime legacy repair failure evidence'),
    runtimeLegacyRepairPostRepairCarrierDisposition: requiredText(legacyRepair.post_repair_carrier_disposition, 'Runtime legacy repair post-repair carrier disposition'),
    runtimeLegacyRepairParentPrivateCustodyProof: requiredText(legacyRepair.parent_private_custody_proof, 'Runtime legacy repair parent private-custody proof'),
    runtimeLegacyRepairParentFinalHelperProcessTreePolicy: requiredText(legacyRepair.parent_final_helper_process_tree_policy, 'Runtime legacy repair parent final-helper process-tree policy'),
    runtimeLegacyRepairFreshBootstrapAuthorityProof: requiredText(legacyRepair.fresh_bootstrap_authority_proof, 'Runtime legacy repair fresh bootstrap authority proof'),
    runtimeLegacyRepairFreshBootstrapNestedProcessPolicy: requiredText(legacyRepair.fresh_bootstrap_nested_process_policy, 'Runtime legacy repair fresh bootstrap nested-process policy'),
    runtimeLegacyRepairCustodyProofInvalidation: requiredText(legacyRepair.custody_proof_invalidation, 'Runtime legacy repair custody-proof invalidation'),
    runtimeLegacyRepairCleanNoJournalDisposition: requiredText(legacyRepair.clean_no_journal_disposition, 'Runtime legacy repair clean no-journal disposition'),
    runtimeLegacyRepairSuccessReceiptSchemaVersion: requiredText(repairSuccessReceipt.schema_version, 'Runtime legacy repair success receipt schema version'),
    runtimeLegacyRepairSuccessReceiptRequiredFields: requiredArray(repairSuccessReceipt.required_fields, 'Runtime legacy repair success receipt required fields').map((value) => requiredText(value, 'Runtime legacy repair success receipt field')),
    runtimeLegacyRepairSuccessReceiptPreservedFields: requiredArray(repairSuccessReceipt.preserved_fields, 'Runtime legacy repair success receipt preserved fields').map((value) => requiredText(value, 'Runtime legacy repair success receipt preserved field')),
    runtimeLegacyRepairSuccessReceiptSourceHelperDisposition: requiredText(repairSuccessReceipt.source_helper_disposition, 'Runtime legacy repair success receipt source helper disposition'),
    runtimeLegacyRepairSuccessReceiptCurrentCarrierAction: requiredText(repairSuccessReceipt.current_carrier_action, 'Runtime legacy repair success receipt current carrier action'),
    runtimeLegacyRepairSuccessReceiptStaleCarrierAction: requiredText(repairSuccessReceipt.stale_carrier_action, 'Runtime legacy repair success receipt stale carrier action'),
    ...
    runtimeDirectoryCacheResetExecutable: requiredAbsolute(principalConstraints.directory_cache_reset_executable, 'Runtime Directory Services cache reset executable'),
    runtimeDirectoryCacheResetPolicy: requiredText(principalConstraints.directory_cache_reset_policy, 'Runtime Directory Services cache reset policy'),
    ...
    runtimePOSIXIdentityLookupAPI: requiredText(principalConstraints.posix_identity_lookup_api, 'Runtime POSIX identity lookup API'),
    runtimePOSIXIdentityLookupResultPolicy: requiredText(principalConstraints.posix_identity_lookup_result_policy, 'Runtime POSIX identity lookup result policy'),
    runtimePrincipalDeletionProjectionPolicy: requiredText(principalConstraints.principal_deletion_projection_policy, 'Runtime principal deletion projection policy'),
    runtimePrincipalDiagnosticFields: requiredArray(principalConstraints.principal_diagnostic_fields, 'Runtime principal diagnostic fields').map((value) => requiredText(value, 'Runtime principal diagnostic field')),
    runtimePrincipalDiagnosticReasonCodes: requiredArray(principalConstraints.principal_diagnostic_reason_codes, 'Runtime principal diagnostic reason codes').map((value) => requiredText(value, 'Runtime principal diagnostic reason code')),
...
let generatedRuntimePOSIXIdentityLookupAPI = ${q(p.runtimePOSIXIdentityLookupAPI)}
let generatedRuntimePOSIXIdentityLookupResultPolicy = ${q(p.runtimePOSIXIdentityLookupResultPolicy)}
let generatedRuntimePrincipalDeletionProjectionPolicy = ${q(p.runtimePrincipalDeletionProjectionPolicy)}
let generatedRuntimePrincipalDiagnosticFields = [${p.runtimePrincipalDiagnosticFields.map(q).join(', ')}]
let generatedRuntimePrincipalDiagnosticReasonCodes = [${p.runtimePrincipalDiagnosticReasonCodes.map(q).join(', ')}]
...
let generatedRuntimeDirectoryCacheResetExecutable = ${q(p.runtimeDirectoryCacheResetExecutable)}
let generatedRuntimeDirectoryCacheResetPolicy = ${q(p.runtimeDirectoryCacheResetPolicy)}
...
let generatedRuntimeLegacyRepairParentPrivateCustodyProof = ${q(p.runtimeLegacyRepairParentPrivateCustodyProof)}
let generatedRuntimeLegacyRepairParentFinalHelperProcessTreePolicy = ${q(p.runtimeLegacyRepairParentFinalHelperProcessTreePolicy)}
let generatedRuntimeLegacyRepairFreshBootstrapAuthorityProof = ${q(p.runtimeLegacyRepairFreshBootstrapAuthorityProof)}
let generatedRuntimeLegacyRepairFreshBootstrapNestedProcessPolicy = ${q(p.runtimeLegacyRepairFreshBootstrapNestedProcessPolicy)}
let generatedRuntimeLegacyRepairCustodyProofInvalidation = ${q(p.runtimeLegacyRepairCustodyProofInvalidation)}
let generatedRuntimeLegacyRepairCleanNoJournalDisposition = ${q(p.runtimeLegacyRepairCleanNoJournalDisposition)}
...
let generatedRuntimeLegacyRepairTerminalCommitPolicy = ${q(p.runtimeLegacyRepairTerminalCommitPolicy)}
...
let generatedRuntimeLegacyRepairInvocationDeadline = ${q(p.runtimeLegacyRepairInvocationDeadline)}
let generatedRuntimeLegacyRepairFailureEvidence = ${q(p.runtimeLegacyRepairFailureEvidence)}
let generatedRuntimeLegacyRepairPostRepairCarrierDisposition = ${q(p.runtimeLegacyRepairPostRepairCarrierDisposition)}
let generatedRuntimeLegacyRepairSuccessReceiptSchemaVersion = ${q(p.runtimeLegacyRepairSuccessReceiptSchemaVersion)}
let generatedRuntimeLegacyRepairSuccessReceiptRequiredFields = [${p.runtimeLegacyRepairSuccessReceiptRequiredFields.map(q).join(', ')}]
let generatedRuntimeLegacyRepairSuccessReceiptPreservedFields = [${p.runtimeLegacyRepairSuccessReceiptPreservedFields.map(q).join(', ')}]
let generatedRuntimeLegacyRepairSuccessReceiptSourceHelperDisposition = ${q(p.runtimeLegacyRepairSuccessReceiptSourceHelperDisposition)}
let generatedRuntimeLegacyRepairSuccessReceiptCurrentCarrierAction = ${q(p.runtimeLegacyRepairSuccessReceiptCurrentCarrierAction)}
let generatedRuntimeLegacyRepairSuccessReceiptStaleCarrierAction = ${q(p.runtimeLegacyRepairSuccessReceiptStaleCarrierAction)}
