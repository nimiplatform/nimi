...
const authorityRules = [
  ...
  },
  {
    ...
    evidence: '.nimi/spec/runtime/kernel/tables/rule-evidence.rules-core-auth.yaml',
    clauses: [
      ['REALM_OPERATION_AUTHORITY_BLOCK_MISSING', /Exact per-operation Realm unary authority remains blocked pending a separate[\s\S]*Runtime admission/iu],
      ['REALM_FALLBACK_DENIAL_MISSING', /no public grant,[\s\S]*portable envelope, renderer\/app token provider, or direct Realm path is a[\s\S]*fallback/iu],
    ],
  },
  {
    ruleId: 'K-ACCSVC-024',
  ...
  },
  {
    ...
    clauses: [
      ...
      ['GENERIC_PROXY_DENIAL_MISSING', /no method-id\/bytes proxy or generic protected Runtime forwarding is[\s\S]*admitted/iu],
      ['KIT_ZERO_GRANT_DENIAL_MISSING', /zero-grant session may project permission posture, but it authorizes no[\s\S]*protected operation/iu],
