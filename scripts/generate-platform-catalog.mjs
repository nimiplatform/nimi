...
const kitCapabilitiesOutputPath = path.join(
  repoRoot,
  'kit/shell/capabilities/src/ai-profile-factory.generated.ts',
);
const rustOutputPath = path.join(repoRoot, 'kit/shell/tauri/src/platform_catalog/ai_profile_factory.rs');
const runtimeGoOutputPath = path.join(
  repoRoot,
  'runtime/internal/platformcatalog/ai_profile_factory_generated.go',
);
...
function renderKitCapabilities(profileDoc, factoryRows) {
  const catalogVersion = Number(profileDoc?.version ?? 0);
  if (!Number.isInteger(catalogVersion) || catalogVersion <= 0) {
    throw new Error('ai-profile factory catalog version must be a positive integer for Kit projection');
  }
  ...
  return [
    ...
    '};',
    '',
    `export const NIMI_PLATFORM_AI_PROFILE_FACTORY_CATALOG_ID = ${JSON.stringify(catalogId)};`,
    `export const NIMI_PLATFORM_AI_PROFILE_FACTORY_CATALOG_VERSION = ${catalogVersion};`,
    "export const NIMI_PLATFORM_AI_PROFILE_SELECTION_POLICY_REF = 'P-AIPS-004';",
    '',
    `export const NIMI_PLATFORM_AI_PROFILE_FACTORY_ROWS = ${stableStringify(factoryRows)} as const satisfies readonly NimiPlatformAIProfileFactoryRow[];`,
    '',
  ].join('\n');
}
...
function renderRuntimeGo(profileDoc, factoryRows) {
  const catalogVersion = Number(profileDoc?.version ?? 0);
  if (!Number.isInteger(catalogVersion) || catalogVersion <= 0) {
    throw new Error('ai-profile factory catalog version must be a positive integer for Runtime projection');
  }
  const catalogId = asString(profileDoc?.catalog_id, 'ai-profile factory catalog_id');
  const rows = factoryRows.map((row) => [
    '\t{',
    '\t\tAlias: ' + JSON.stringify(row.alias) + ',',
    '\t\tCapabilitySet: ' + goStringSlice(row.capabilitySet) + ',',
    '\t\tHostCapabilityProfileRefs: ' + goStringSlice(row.hostCapabilityProfileRefs) + ',',
    '\t},',
  ].join('\n')).join('\n');
  return [
    ...
    '}',
    '',
    'const FactoryAIProfileCatalogID = ' + JSON.stringify(catalogId),
    'const FactoryAIProfileCatalogVersion = ' + catalogVersion,
    '',
...
function formatGo(source) {
  ...
  if (result.status !== 0) {
    throw new Error('gofmt failed for Runtime Platform catalog projection: ' + (result.stderr || result.stdout));
  }
...
async function main() {
  ...
  const renderedKitCapabilities = renderKitCapabilities(profileDoc, factoryRows);
  const renderedRust = formatRust(renderRust(profileDoc, factoryRows));
  const renderedRuntimeGo = formatGo(renderRuntimeGo(profileDoc, factoryRows));

  if (checkMode) {
    let currentVNextApp = '';
    let currentKitCapabilities = '';
    let currentRust = '';
    let currentRuntimeGo = '';
    try {
    ...
    try {
      currentKitCapabilities = await fs.readFile(kitCapabilitiesOutputPath, 'utf8');
    } catch {
      throw new Error(`Kit platform catalog projection missing: ${path.relative(repoRoot, kitCapabilitiesOutputPath)}`);
    }
    try {
    ...
    try {
      currentRuntimeGo = await fs.readFile(runtimeGoOutputPath, 'utf8');
    } catch {
      throw new Error('Runtime Platform catalog projection missing: ' + path.relative(repoRoot, runtimeGoOutputPath));
    }
    ...
    if (currentKitCapabilities !== renderedKitCapabilities) {
      throw new Error(`Kit platform catalog projection drift detected. Run pnpm generate:platform-catalog.`);
    }
    ...
    if (currentRuntimeGo !== renderedRuntimeGo) {
      throw new Error('Runtime Platform catalog projection drift detected. Run pnpm generate:platform-catalog.');
    }
  ...
  await fs.mkdir(path.dirname(kitCapabilitiesOutputPath), { recursive: true });
  await fs.mkdir(path.dirname(rustOutputPath), { recursive: true });
  await fs.mkdir(path.dirname(runtimeGoOutputPath), { recursive: true });
  await fs.writeFile(vNextAppOutputPath, renderedVNextApp, 'utf8');
  await fs.writeFile(kitCapabilitiesOutputPath, renderedKitCapabilities, 'utf8');
  await fs.writeFile(rustOutputPath, renderedRust, 'utf8');
  await fs.writeFile(runtimeGoOutputPath, renderedRuntimeGo, 'utf8');
  process.stdout.write(`generated ${path.relative(repoRoot, vNextAppOutputPath)}\n`);
  process.stdout.write(`generated ${path.relative(repoRoot, kitCapabilitiesOutputPath)}\n`);
