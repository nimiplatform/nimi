...
const registrySource = path.join(repoRoot, '.nimi', 'spec', 'platform', 'kernel', 'tables', 'nimi-app-registry.yaml');
const releaseDescriptorsSource = path.join(repoRoot, '.nimi', 'spec', 'platform', 'kernel', 'tables', 'nimi-app-release-descriptors.yaml');
const devKernelFixtureSource = path.join(repoRoot, 'config', 'dev-kernel-checkpoint-acceptance.json');
...
const installerSource = readFileSync(source, 'utf8')
  ...
  .replace('__BUILD_DEV_KERNEL_FIXTURE_SHA256__', devKernelFixtureSha256)
