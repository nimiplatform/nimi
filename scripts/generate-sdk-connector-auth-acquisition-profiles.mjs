...
async function main() {
  ...
  const out = renderTS(profiles);

  if (check) {
    const current = await fs.readFile(sdkOutPath, 'utf8');
    if (current !== out) {
      throw new Error('connector auth acquisition profile generated file is out of date');
    }
    return;
  }

  await fs.writeFile(sdkOutPath, out);
}
