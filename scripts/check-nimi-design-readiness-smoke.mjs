...
const checkId = 'nimi-kit-ui-card-design-readiness';
...
const evidenceDir = path.join(repoRoot, '.nimi', 'local', 'evidence', checkId);
...
const fullSuiteBlockers = fullSuiteGateChecks
  .filter((gate) => !gate.ok)
  .map((gate) => ({
    id: gate.id,
    ...
    stdout: gate.stdout,
    stderr: gate.stderr,
    classification: 'full_suite_blocker',
  }));
...
const report = {
  ...
  fullSuiteGateChecks,
  fullSuite: {
    status: fullSuiteBlockers.length === 0 ? 'ready' : 'blocked',
    blockers: fullSuiteBlockers,
  },
  allGateChecks: [...requiredGateChecks, ...fullSuiteGateChecks],
  gateChecks: requiredGateChecks,
  failures,
};

write(path.join(evidenceDir, 'readiness-report.json'), `${JSON.stringify(report, null, 2)}\n`);
write(path.join(evidenceDir, 'README.md'), `# Nimi Design Readiness Evidence
...
- Full-suite status: \`${report.fullSuite.status}\`
`);
...
process.stdout.write(
  `nimi design readiness smoke passed: ${path.relative(repoRoot, evidenceDir)}; full-suite ${report.fullSuite.status}\n`,
);
