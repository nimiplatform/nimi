...
test('fails high-risk design docs without authority metadata', () => {
  ...
  writeFile(
    path.join(repoRoot, '.local/example-change/example-design.md'),
    [
      '---',
...
test('accepts preflight-required spec status in high-risk frontmatter docs', () => {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'nimi-high-risk-docs-'));
  writeFile(
    path.join(repoRoot, '.local/example-change/migration-design.md'),
    [
      '---',
...
test('treats refactor plans as high-risk docs that require authority metadata', () => {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'nimi-high-risk-docs-'));
  writeFile(
    path.join(repoRoot, '.local/example-change/desktop-chat-refactor.md'),
    [
      '---',
