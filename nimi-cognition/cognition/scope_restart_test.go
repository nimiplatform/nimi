...
func TestKnowledgeScopeRegistry_DurableAcrossRestart(t *testing.T) {
	...
	if err := c1.RuntimeBridge().SaveKnowledge(context.Background(), runtimeKnowledgeAuthorizationAt(RuntimeAuthorizationActionWritePage, RuntimeBridgeOperationSaveKnowledge, scope.Owner, scopeID, ts), page); err != nil {
	...
	loaded, err := c2.RuntimeBridge().LoadKnowledge(context.Background(), runtimeKnowledgeAuthorizationAt(RuntimeAuthorizationActionReadPage, RuntimeBridgeOperationLoadKnowledge, got.Owner, scopeID, ts), scopeID, "page-restart-1")
	if err != nil {
