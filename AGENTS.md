...
- Debug/fix order: `runtime` → `sdk` → `apps/desktop`/`apps/web` → `nimi-mods`. Reuse `@nimiplatform/kit` first for UI work via `kit/README.md` and `.nimi/spec/platform/kernel/tables/nimi-kit-registry.yaml`.
