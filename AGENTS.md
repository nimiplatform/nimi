...
- `Work Type=alignment` must align to existing authority with no parallel truth. `Work Type=redesign` changes authority/canonical ownership and must not proceed without prior `spec/**` alignment.
- Debug/fix order: `runtime` → `sdk` → `apps/desktop`/`apps/web` → `nimi-mods`. Reuse `nimi-kit` first for UI work via `kit/README.md` and `spec/platform/kernel/tables/nimi-kit-registry.yaml`.
...
- Spec: run affected consistency + docs drift commands. For `spec/**/kernel/**`, also run `pnpm check:spec-human-doc-drift`; if it fails, run `pnpm generate:spec-human-doc`.
...
- Treat `spec/**` as the current repo-wide authority and `/.nimi/spec/**` as the generated canonical tree; cutover readiness is evidence only and still needs explicit redesign admission.
