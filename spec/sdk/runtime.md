...
- `SDKR-028`: Phase 1 标准事件名（完整集合）：`runtime.connected` / `runtime.disconnected` / `error` / `auth.token.issued` / `auth.token.revoked`。新增事件名需同步更新本列表。

## 4. Metadata 与凭据传递（领域增量）

- `SDKR-030`: credential 分离语义遵循 `S-TRANSPORT-002` 与 runtime `K-KEYSRC-001`（路径模型）、`K-KEYSRC-002`（互斥）、`K-KEYSRC-003`（metadata keys）。
...
- `SDKR-034`: SDK 必须透传 runtime key-source metadata keys：`x-nimi-key-source`、`x-nimi-provider-type`、`x-nimi-provider-endpoint`、`x-nimi-provider-api-key`、`x-nimi-app-id`。字段语义以 `K-KEYSRC-003` 为权威。
...
- `SDKR-060`: `RegisterApp` 调用模型：SDK 不在 `connect()` 中隐式调用 `RegisterApp`。应用层需显式调用或通过高阶模块触发。Session 生命周期管理（open/refresh/revoke）由应用层驱动。
...
- `SDKR-063`: Grant 服务方法直接通过 `raw.call` 或高阶 grant 模块调用。SDK 不内置自动授权流。
- `SDKR-064`: `ListTokenChain` 分页遵循 `K-PAGE-002`（page_token 语义）。SDK 不封装自动翻页。
