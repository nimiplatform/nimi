...
`NIMI-010`: 入口互斥校验由上游按 `K-KEYSRC-002`（互斥）与 `K-KEYSRC-004`（评估顺序）执行，`nimillm` 不重建第二套入口规则。
...
`NIMI-020`: 内部命名与对外 gRPC 命名映射（`K-RPC-005`）必须由上游维护，`nimillm` 不导出 gRPC 方法名常量。
...
`NIMI-040`: `AI_MODEL_PROVIDER_MISMATCH` 的校验逻辑在 `nimillm` 执行，但规则定义权在 `K-ERR-004`（错误映射）与 `K-KEYSRC-004`（评估顺序）。此校验属于 `K-KEYSRC-004` step 9，由上游调度链调用 `nimillm` 执行，非 `nimillm` 独立发起。

## 5. 发现与探测职责

- `NIMI-050`: `ListRemoteModels` 返回完整模型快照给 `ConnectorService`；分页与 token 在 `ConnectorService` 边界执行（见 `K-PAGE-001` 与 `K-PAGE-002`）。
...
- `NIMI-070`: 所有实际出站调用前都必须执行 `K-SEC-001`（校验对象）与 `K-SEC-004`（执行期强制）校验。
- `NIMI-071`: Phase 1 不启用私网 allowlist；相关配置字段只能处于关闭态。

## 8. 流式执行责任

- `NIMI-080`: `StreamGenerateText` 与 `SynthesizeSpeechStream` 的回调事件必须满足 `K-STREAM-002`（阶段边界）、`K-STREAM-003`（文本流）与 `K-STREAM-004`（语音流）。
