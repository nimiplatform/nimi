...
| 字段 | 类型 | 约束 |
|---|---|---|
| `connector_id` | string | ULID |
| `kind` | ConnectorKind | `LOCAL_MODEL` \| `REMOTE_MANAGED` |
| `owner_type` | ConnectorOwnerType | `SYSTEM` \| `REALM_USER` |
| `owner_id` | string | SYSTEM 常量或 JWT sub |
| `provider` | string | `local` \| `gemini` \| `openai` \| ... |
| `endpoint` | string | local 固定空串；remote 非空 |
| `label` | string | — |
| `status` | ConnectorStatus | `ACTIVE` \| `DISABLED` |
| `created_at` | int64 | Unix ms |
| `updated_at` | int64 | Unix ms |
| `has_credential` | bool | 展示缓存值（非门禁） |
| `local_category` | LocalConnectorCategory | — |

字段编号由 `.proto` 文件管控，spec 不定义。
...
### 5.4 ListProviderCatalog

- `CONN-075`: `ListProviderCatalog` 返回 `kernel/tables/provider-catalog.yaml` + `kernel/tables/provider-capabilities.yaml` 的合并视图。
- `CONN-076`: 响应不含 credential 信息，不做 owner 鉴权，任何已认证 app 均可调用。
- `CONN-077`: Desktop 客户端应优先使用 `ListProviderCatalog` 获取 provider 默认 endpoint，本地硬编码仅作为 runtime 不可用时的 fallback。

### 5.5 TestConnector / ListConnectorModels
...
- `CONN-100`: ConnectorService 单元测试必须覆盖以下场景：
  1. remote connector CRUD 全路径（Create/Get/List/Update/Delete）
  2. local connector 启动补齐（CONN-022）与 CRUD 拒绝（CONN-003）
  3. 不可变字段更新拒绝（CONN-010）
  4. Delete 三步补偿流程（Section 7.4）含中断后重试
  5. `delete_pending=true` 的 NOT_FOUND 信息隐藏（CONN-080）
  6. 并发写串行化验证（CONN-090）
- `CONN-101`: CI 命令 `pnpm check:runtime-spec-kernel-consistency` 必须通过。
- `CONN-102`: `go test ./internal/services/connector/...` 与 `go vet ./internal/services/connector/...` 必须零错误。
