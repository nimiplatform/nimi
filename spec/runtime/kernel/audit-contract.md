...
Runtime 中存在三套审计字段定义，各有明确适用范围：

| 字段模型 | 定义位置 | 适用范围 | 说明 |
|---|---|---|---|
| **K-AUDIT-001 最小字段** | 本文档 | 所有审计路径（通用 floor） | AI 执行审计最小字段集合，全局审计与本地审计均需至少包含这些字段。是字段完整性的下限。 |
| **K-AUDIT-006 AuditEventRecord** | 本文档 | 全局审计存储（`RuntimeAuditService`） | `ListAuditEvents` / `ExportAuditEvents` 使用的完整 schema。包含 auth/grant 相关字段（`principal_id`、`token_id`、`consent_id` 等），这些字段仅在 auth/grant domain 审计事件中填充，其他 domain 事件中为空。 |
...
**关系规则**：
- K-AUDIT-001 是所有审计字段的通用子集（floor）。K-AUDIT-006 和 K-LOCAL-016 均是 K-AUDIT-001 的超集。
