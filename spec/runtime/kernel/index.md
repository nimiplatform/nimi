...
- 执行态内容不得进入 `spec/**`；若使用本地执行工作区，topic-bound 计划、baseline、packet 与 evidence 写入 `nimi-coding/.local/<topic-id>/**`，local-only 执行报告写入 `nimi-coding/.local/report/**`；tracked spec 不依赖具体 `.local` 文件。
