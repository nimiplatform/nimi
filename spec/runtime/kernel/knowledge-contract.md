...
| 决策 | 当前状态 | 推迟原因 |
|---|---|---|
| **持久化策略** | in-memory only（K-KNOW-005），重启丢失 | 需评估嵌入模型规模与磁盘 I/O 成本后决定持久化格式（SQLite/mmap/文件系统） |
| **索引更新策略** | 仅支持全量覆盖（`overwrite=true`） | 增量更新需定义文档变更检测机制与部分重建协议 |
| **BuildIndex 进度上报** | 仅返回 `task_id`，无进度回调 | 需与 Workflow 事件流（K-WF-*）集成后统一设计 |
