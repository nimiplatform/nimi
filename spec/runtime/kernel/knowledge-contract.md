...
| **knowledge-base mod (IndexedDB)** | 独立实现 | KB mod 使用浏览器 IndexedDB 实现向量存储与 cosine similarity 检索，不依赖 RuntimeKnowledgeService。当 K-KNOW Phase 2 持久化+真向量搜索就绪后，评估迁移。详见 `nimi-mods/runtime/knowledge-base/SSOT.md` |
