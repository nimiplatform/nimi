...
若 `connector_id` 与 inline metadata 都未提供，请求不进入 managed / inline 路径，继续按 runtime config 或 anonymous local-runtime 默认路由评估。
...
**Local-runtime 路径**（`connector_id` 为空，且不存在 inline remote 凭据 metadata）：
