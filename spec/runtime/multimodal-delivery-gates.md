...
- topic-bound 本地迭代计划与阶段拆解：`nimi-coding/.local/<topic-id>/**`。
- Gate 执行结果与证据：local execution report route patterns（如 `nimi-coding/.local/report/**`）；tracked spec 不绑定具体 `.local` 文件名。
