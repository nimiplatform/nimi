...
- Put topic-bound local execution materials in `nimi-coding/.local/<topic-id>/**` and local reports in `nimi-coding/.local/report/**` when a local workspace is used. Tracked spec content must not depend on concrete `.local` files.
