...
- artifact list / verified list / install / import / remove 通过受管 Local AI bridge facade 暴露，但其数据面来自 runtime SDK `RuntimeLocalRuntimeService`，不是新增 Tauri command。
...
token-api connector 路径必须保持 runtime-only：Desktop 不得恢复 legacy provider adapter factory 或直接 provider `listModels` / `healthCheck` 调用来旁路 Runtime。
...
| local_ai_models_adopt | Adopt a go-runtime discovered local AI model into desktop state without download |
