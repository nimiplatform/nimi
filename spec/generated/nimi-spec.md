...
1. `core`：全权限 `*`。
2. `builtin`：完整 hook 子系统 + LLM + action + audit/meta。
3. `injected`：受限 turn hook（仅 pre-model, post-state）+ 完整 LLM。
4. `sideload`：无 event.subscribe / 无 data.register / 无 inter-mod.provide + 完整 LLM。
5. `codegen`：最小权限（text LLM + UI extension + user data API）。
...
- 启动序列中由 `D-BOOT-001` 后注入，通过 `desktopBridge.logRendererEvent` 转发到 Tauri backend。
...
退避算法：`delayMs = min(maxDelayMs, initialDelayMs * 2^(attempt-1))`
...
**Layer 2: Bearer Token 管理** — Token 存储在 Zustand `auth.token` 中，同步到 DataSync hot state。Desktop 使用 OS 原生 keyring 存储，Web 使用 localStorage 加过期机制（敏感页面需二次验证，logout 时完全清除）。
...
| SDK_REALM_BASE_URL_REQUIRED | SDK_REALM | — |
...
| SDK_RUNTIME_APP_ID_REQUIRED | SDK_CONFIG | — |
...
| F-MULTIMODAL-001 | 多模态 AI 能力（TTS/STT/图像/视频生成） | medium | platform | proposed |
...
| F-WFTRIGGER-001 | 工作流触发器系统 | medium | platform | proposed |
...
| F-SDENGINE-001 | SD WebUI 引擎集成 | low | integration | proposed |
...
| Source ID | 标题 | 路径 |
|---|---|---|
| RESEARCH-OWUI-001 | Open WebUI vs Nimi Deep Audit | dev/research/open-webui-vs-nimi-deep-audit-2026-03-01.zh.md |
| RESEARCH-DIFY-001 | Dify vs Nimi Deep Audit | dev/research/dify-vs-nimi-deep-audit-2026-03-01.zh.md |
| RESEARCH-GROWTH-001 | Nimi Growth Strategy and Market Research | dev/research/nimi-growth-strategy-and-market-research-2026-02-28.zh.md |
| RESEARCH-MMAUD-001 | Multimodal Provider Audit | dev/research/multimodal-provider-audit-2026-02-24.md |
| RESEARCH-PCHAT-001 | prompts.chat vs Nimi Deep Audit | dev/research/prompts-chat-vs-nimi-deep-audit-2026-03-01.zh.md |
| RESEARCH-VOLC-001 | VolcEngine Provider Research | dev/research/volcengine-provider-research.md |
| RESEARCH-COPAW-001 | CoPaw vs Nimi Deep Audit | dev/research/copaw-vs-nimi-deep-audit-2026-03-01.zh.md |
| RESEARCH-SDWUI-001 | Stable Diffusion WebUI vs Nimi Deep Audit | dev/research/stable-diffusion-webui-vs-nimi-deep-audit-2026-03-01.zh.md |
