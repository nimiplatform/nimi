...
`agent-chat-beat-action-contract.md`（`D-LLM-027` ~ `D-LLM-033`）继续拥有 resolved
...
`spec/runtime/kernel/voice-contract.md`（`K-VOICE-*`）继续拥有 `voice_workflow.tts_v2v`、
...
- 当前 thread 中使用的 voice identity / `VoiceReference` product meaning 是什么
- preset/custom voice selection 与 packet-bounded clone / design trigger 的
  desktop product meaning 是什么
- workflow 结果如何回到当前 thread continuity
...
- `spec/runtime/kernel/voice-contract.md`（`K-VOICE-*`）继续拥有 workflow type、
  `VoiceReference` schema、`VoiceAsset` schema、scenario job、以及 discovery channel
  truth，而不是 desktop current-thread product meaning
...
agent chat richer voice workflow 只允许在当前 thread 内被显式 admit；不得由 route
...
- 每个 admitted richer workflow intent 必须与一个 capability key 和一个 workflow type
  ...
  workflow success
- `voice_workflow.tts_v2v` 在 desktop product meaning 中表达“当前 thread 内通过参考语音
  / 音频驱动 voice creation”；`voice_workflow.tts_t2v` 表达“当前 thread 内通过文本设计
  / 指令驱动 voice creation”
- resolved `promptPayload` 或等价 typed workflow input 若无法与所声明的 workflow type
  ...
  field 补齐
- 当前 admitted route 只覆盖 foreground current-thread continuity；不得借道升级成
...
- Desktop 不得发明第四种 local-only voice identity source；admitted voice identity
  ...
  - `voice_asset_id`
  - `provider_voice_ref`
- `preset_voice_id` 在 desktop product meaning 中表达“当前 thread 使用某个系统预置音色
  作为目标 voice identity”
- `voice_asset_id` 在 desktop product meaning 中表达“当前 thread 使用某个已存在或刚由
...
preset/custom voice selection 在 agent chat richer voice workflow 中属于 current-thread
execution truth，而不是 discovery list truth。

固定语义：

- `ListPresetVoices` 与 `ListVoiceAssets` 只提供候选 inventory；selection truth 只有在
  Desktop 把某个 `VoiceReference`-compatible choice 固化进当前 thread workflow intent /
...
- preset/custom voice selection 既可以作为 workflow 输入，也可以作为 workflow completion
  后当前 thread 的可复用 voice identity 结果；两种情况下都必须保持同一份 stable
  `VoiceReference` meaning
- 当前首包只 admit packet-bounded clone / design trigger semantics：trigger 必须是当前
  thread 内显式 admit 的 richer workflow intent，并保持与 source turn / action 的强关联
...
- 若 trigger 需要当前 packet 之外的 consent、ownership、policy、或跨 thread voice
...
agent chat richer voice workflow 的 admitted return path 固定属于当前 thread continuity。

固定约束：

- workflow submit、pending、completed、failed、canceled 的 desktop product projection 必须
  能恢复到 source thread / turn / beat / action relation，以及 admitted workflow type
- runtime scenario job lifecycle、事件流、`VoiceAsset` schema、与 `VoiceReference` schema
  仍由 runtime 拥有；本契约只拥有“这些 runtime-owned outputs 如何作为当前 thread
  product continuity 被接回 Desktop” 的 truth
- workflow completion 可以把结果接回当前 thread 为：
  - workflow status / progress evidence
  - 一个新产生或已确认可复用的 voice identity
  - 一个 projected voice playback continuation
  但不得把结果投影成 detached thread、detached asset page success、或与 source turn
  无关的伪成功 artifact
- 当 workflow completion 需要向当前 thread 暴露 projected playback outcome 时，
  本契约拥有“为什么这属于当前 thread voice continuation” 的 product meaning；transport、
...
- 脱离当前 thread 的 standalone custom voice management / library semantics
...
- `spec/desktop/kernel/agent-chat-voice-executor-contract.md` — narrow
...
- `spec/desktop/kernel/llm-adapter-contract.md` — runtime-aligned voice route
  and API boundary
- `spec/runtime/kernel/voice-contract.md` — runtime workflow / `VoiceReference`
