...
projection 语义。Agent chat 的 multi-beat ordering、turn-mode、experience-policy
/ settings 不由 stream layer 拥有，相关行为真相固定来自
`agent-chat-behavior-contract.md`（`D-LLM-022` ~ `D-LLM-026`）。delayed beat、
pending beat invalidation、modality action envelope、以及 model-generated
modality prompt payload 也不由 stream layer 拥有，相关真相固定来自
`agent-chat-beat-action-contract.md`（`D-LLM-027` ~ `D-LLM-033`）。
...
若 Agent chat execution 使用 multi-beat delivery，stream consumer 只能按
`agent-chat-behavior-contract.md`（`D-LLM-025`）已解析出的 beat order 消费与投影
lifecycle。stream layer 不得自行拆分、合并、重排、补拍、删拍，或把多 beat plan
降格成 stream-owned 的单 beat 成功语义。

若 execution 涉及 delayed follow-up beat 或 modality action delivery，stream
consumer 也只能消费 `agent-chat-beat-action-contract.md` 已解析出的 resolved
beat/action outputs。stream layer 不得决定 pending beat 是否仍然有效、不得补造
image/voice/video action、不得改写 relation、也不得生成 substitute
`promptPayload`。
...
workflow，job stream 都不得反向成为 action existence、pending invalidation、或
modality prompt semantics 的 owner。
