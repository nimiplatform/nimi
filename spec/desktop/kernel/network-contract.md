...
**跨传输重试参数差异说明**：Desktop HTTP 重试参数（120ms initial / 900ms cap）与 SDK Runtime gRPC 重试参数（SDKR-045: 200ms initial / 3000ms cap）不同。此差异是设计意图：HTTP（Realm API）为低延迟场景，gRPC（Runtime）为推理负载场景，需更大退避区间。
