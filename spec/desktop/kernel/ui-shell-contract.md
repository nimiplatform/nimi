...
`chat`、`contacts`、`runtime-config`、`settings` 的内部左侧栏必须属于同一个 desktop sidebar family。

- 统一 family 固定为 `desktop-sidebar-v1`，事实源为 `tables/renderer-design-sidebars.yaml`。
- 允许的信息架构变体仅限：
  - `chat`：`header + search + entity-row list`
...
- family 必须通过共享 sidebar primitive 暴露一致的 slot：
  ...
  - `itemList+`
  - `resizeHandle?`
- 主 shell 最外层 icon rail 不属于本规则范围；本规则只治理 feature 内部左侧栏。

## D-SHELL-024 — Sidebar Item Taxonomy

desktop sidebar family 的 item 语义固定为：

- `entity-row`：头像/实体型列表项，适用于 chat thread 与 contact record。
...
- search、primary action、active row、section label 与 trailing affordance 必须在四个治理内 sidebar 上保持同一家族的一致语义与交互反馈。
