...
`agent-chat-beat-action-contract.md`（`D-LLM-027` ~ `D-LLM-033`）继续拥有 `voice`
...
surface；`spec/runtime/kernel/voice-contract.md` 继续拥有 `voice_workflow.*` 与 voice
...
- `spec/desktop/kernel/agent-chat-voice-executor-contract.md` — resolved voice
...
- `spec/desktop/kernel/llm-adapter-contract.md` — runtime-aligned TTS / STT
