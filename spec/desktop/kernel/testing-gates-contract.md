...
- design hard gate 只针对 `renderer-design-surfaces.yaml` 中声明为 `baseline` 的试点 surface；secondary/admin 与 exception surface 不在首批 hard gate 范围内。
...
- 同一个 `check:desktop-design-contract` 命令必须输出 remaining local `Button` / `IconButton` 族与 baseline file adoption 覆盖率。
