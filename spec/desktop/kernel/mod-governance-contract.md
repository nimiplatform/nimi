...
Mod 发布资产与 Marketplace 可见性必须解耦：

- `nimi-mods` 仅可作为官方 mod 的源码/验证工作区，不得被 Desktop 当作 Marketplace 真相源扫描。
- 对 external / official package，Desktop Marketplace 的可见性必须来自独立 catalog 发布面，而不是 source repo 文件树或 release asset 枚举。
