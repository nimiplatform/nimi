...
`MUST`: 平台治理执行项必须使用 `OSG-<Priority>-NN` 命名并记录在 local execution workspaces such as `nimi-coding/.local/<topic-id>/**` 或等效非规范执行面中。kernel 只定义任务分级口径（P0/P1/P2）与命名约束，不承载具体待办清单。
