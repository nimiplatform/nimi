...
type ModSdkHost = Parameters<typeof setModSdkHost>[0];
...
function createMockHost(): ModSdkHost {
  ...
  const runtimeHook = {
    subscribeEvent: async (input: { topic: string; handler: HookEventHandler; once?: boolean }) => {
      ...
      if (input.once) {
        handlers.push(async (payload: unknown) => {
          ...
          eventHandlers.set(
            input.topic,
            (eventHandlers.get(input.topic) || []).filter((handler) => handler !== input.handler),
          );
        });
      } else {
    ...
    unsubscribeEvent: (input: { topic?: string }) => {
      if (!input.topic) {
        return;
      }
      eventHandlers.delete(input.topic);
    },
    publishEvent: async (input: { topic: string; payload: unknown }) => {
    ...
    queryData: async (input: { capability: string; query: unknown }) => {
    ...
    registerDataProvider: async (input: { capability: string; handler: DataProviderHandler }) => {
      dataProviders.set(input.capability, input.handler);
    },
    unregisterDataProvider: (input: { capability?: string }) => {
      if (!input.capability) {
        return;
      }
    ...
    unregisterUIExtension: (input: { slot?: string }) => {
      if (!input.slot) {
        return;
      }
    ...
    registerTurnHookV2: async (input: { point: string; handler: TurnHookHandler }) => {
      const handlers = turnHooks.get(input.point) || [];
      handlers.push(input.handler);
      turnHooks.set(input.point, handlers);
    },
    unregisterTurnHook: (input: { point?: string }) => {
      if (!input.point) {
        return;
      }
      turnHooks.delete(input.point);
    },
    invokeTurnHooks: async (input: { point: string; context: Record<string, unknown> }) => {
    ...
    registerInterModHandlerV2: async (input: { channel: string; handler: InterModHandler }) => {
      interModHandlers.set(input.channel, input.handler);
    },
    unregisterInterModHandler: (input: { channel?: string }) => {
      if (!input.channel) {
        return;
      }
      interModHandlers.delete(input.channel);
    },
    requestInterMod: async (input: { channel: string; payload: unknown }) => {
    ...
    broadcastInterMod: async (input: { channel: string; payload: unknown }) => {
      const handler = interModHandlers.get(input.channel);
      if (!handler) {
        return [];
      }
