...
| 自动迁移 + 硬切换 | runtime 已实现迁移并删除旧路径 | 完成 | 已新增 daemon 启动链迁移集成测试：`runtime/internal/entrypoint/runtime_config_startup_chain_test.go` |
...
| CLI `set` 文件锁 + 原子写 | 已实现（lock + atomic write） | 完成 | 已新增并发冲突集成测试：`TestRunRuntimeConfigSetConcurrentWriteConflict` |
...
| restart required 语义 | CLI `set` 返回 `CONFIG_RESTART_REQUIRED` | 完成 | 已在 runtime-config panel 中增加显式重启提示（`runtime-config-panel-controller.ts`） |
...
go test ./internal/entrypoint -count=1
