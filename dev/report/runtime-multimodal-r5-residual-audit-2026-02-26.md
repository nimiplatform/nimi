...
4. `TODO` R6-S3 余项（不阻断当前门禁）
   - Kimi 图像经 chat multimodal 的专用 adapter 尚未实现（当前走 openai-compat，能力不足时 fail-close）。
   - GLM 非视频模态的厂商特性字段（如专属响应结构）尚未补专用 adapter（当前走 openai-compat）。
