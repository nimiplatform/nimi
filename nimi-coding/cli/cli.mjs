...
function usage() {
  process.stdout.write(
    [
      ...
      '',
      'Continuous Run (inline default, external worker optional):',
      ...
      '  run-loop-once          Optional provider-backed step: generate prompt, invoke codex exec, ingest worker signal/output, and emit a stable structured summary for one running phase',
      '  run-until-blocked      Optional provider-backed foreground loop until pause/fail/completed/superseded or loop guard refusal, with stable structured summary output',
