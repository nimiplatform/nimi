...
func externalDescriptor(payload []byte) appreleasecatalog.Descriptor {
	...
	return appreleasecatalog.Descriptor{
		...
		Version:         "1.0.0",
		AdmissionTrack:  appreleasecatalog.AdmissionTrackOrdinaryReleaseProof,
		DescriptorClass: appreleasecatalog.DescriptorClassExternalImmutableArtifact,
		Publisher: appreleasecatalog.Publisher{
			GitHubNamespace:   "github.com/example",
			NamespaceKind:     "org",
			IdentityAssurance: "domain-verified",
		...
		Artifact: appreleasecatalog.Artifact{
			Locator:         "https://github.com/example/clock/releases/download/v1.0.0/clock.tgz",
			DigestAlgorithm: "sha256",
			SHA256:          hex.EncodeToString(sum[:]),
			Size:            strconvItoa(len(payload)),
			SizeBreakdown: appreleasecatalog.ArtifactSize{
				Download:   strconvItoa(len(payload)),
				Installed:  strconvItoa(len(payload)),
				UserData:   "0",
				Cache:      "0",
				SharedDeps: "0",
			},
		...
		ArtifactMirrorRef:    "nimi-mirror://community.clock/v1.0.0/clock.tgz",
		MirrorLicenseCleared: true,
		BuildAssurance:       "reproducible-build",
		DependencyAssurance:  "lockfile-and-scanner-evidence",
		PlatformSigningAssurance: appreleasecatalog.PlatformSigningAssurance{
			MacOSNotarization:       "notarized",
			MacOSDeveloperIDSubject: "Developer ID Application: Example Clock",
			WindowsCodeSigning:      "signed",
			InstallerSignature:      "signed",
			EntitlementsRef:         "entitlements/community.clock",
			SigningSubject:          "example",
		},
		...
		UpdateChannelRef:    "community-clock-stable",
		RollbackEligibility: "previous-admitted-descriptor",
		Review: appreleasecatalog.Review{
			...
			Decision:                          "approved",
			AdjudicatorKind:                   "manual-reviewer",
			AdjudicatorRef:                    "review/community.clock/v1.0.0",
			DecidedAt:                         "2026-06-30T00:00:00Z",
		},
		Support: appreleasecatalog.Support{
			DiagnosticsBundleFields:    []string{"runtime", "storage"},
			RedactionRules:             []string{"strip-account-token"},
			UserVisibleIssueCategories: []string{"install-failed"},
			EscalationPath:             "support/community.clock",
			KillSwitchVisibility:       "ordinary-visible",
			RecoveryInstructions:       []string{"reinstall"},
		},
