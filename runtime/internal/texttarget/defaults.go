...
func EnsureLocalQualifiedModel(modelID string) string {
	...
	if strings.HasPrefix(lower, "local/") || strings.HasPrefix(lower, "localai/") || strings.HasPrefix(lower, "nexa/") {
		return normalized
	}
...
func IsHighLevelQualifiedModel(modelID string) bool {
	...
	return lower == "local" || lower == "localai" || lower == "nexa" || LooksLikeQualifiedRemoteModel(normalized)
}
