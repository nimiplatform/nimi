...
func (b *Backend) postRaw(ctx context.Context, path string, requestBody any) ([]byte, error) {
	response, err := b.postRawResponse(ctx, path, requestBody)
	if err != nil {
		return nil, err
	}
	defer func() { _ = response.Body.Close() }()
...
// postRawResponse validates one raw response while leaving a successful body
// open for its accepting Runtime consumer. The caller owns exactly one Close.
func (b *Backend) postRawResponse(ctx context.Context, path string, requestBody any) (*http.Response, error) {
	payload, err := json.Marshal(requestBody)
	if err != nil {
