...
// migrateLegacyCanonicalRegistration performs the single pre-release schema
// cut from the retired installed/local_import/development vocabulary. It
// preserves stable registration and subject identity, then leaves only the
// canonical schema; normal reads never carry a legacy compatibility path.
func (kernel *Kernel) migrateLegacyCanonicalRegistration(ctx context.Context) error {
	...
	newColumns := []string{
		...
		"provenance_attestation_refs_json",
		"provenance_revision",
	...
	hasLegacySourceDigest := columns["source_digest"]
	if newColumnCount == len(newColumns) && !hasLegacySourceDigest {
		return nil
	}
	legacyColumns := []string{
		...
		"declaration_generation", "source_digest", "declaration_digest", "state", "created_unix_nano",
		"updated_unix_nano", "tombstoned_unix_nano",
	}
	if !hasLegacySourceDigest || newColumnCount != 0 {
	...
	for _, name := range legacyColumns {
		if !columns[name] {
			return fmt.Errorf("initialize registered App schema: legacy canonical_registration is missing %s", name)
		}
	}
	migratingColumns, err := sqliteTableColumns(ctx, kernel.db, "canonical_registration_migrating")
	if err != nil {
		return err
	}
	if len(migratingColumns) != 0 {
		return fmt.Errorf("initialize registered App schema: unexpected canonical_registration_migrating table")
	}

	connection, err := kernel.db.Conn(ctx)
	if err != nil {
		return fmt.Errorf("acquire registered App migration connection: %w", err)
	}
	defer func() { _ = connection.Close() }()
	if _, err := connection.ExecContext(ctx, `PRAGMA foreign_keys = OFF`); err != nil {
		return fmt.Errorf("disable registered App migration foreign keys: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("begin registered App schema migration: %w", err)
	}
	defer func() { _ = tx.Rollback() }()
	migrationCreate := strings.Replace(
		canonicalRegistrationCreateStatement,
		"CREATE TABLE IF NOT EXISTS canonical_registration",
		"CREATE TABLE canonical_registration_migrating",
		1,
	)
	statements := []string{
		migrationCreate,
		`INSERT INTO canonical_registration_migrating(
		...
		) SELECT registration_handle, registered_app_subject, app_id, display_name,
			CASE source_class
				WHEN 'installed' THEN 'verified'
				WHEN 'local_import' THEN 'user_imported'
				WHEN 'development' THEN 'local_development'
			END,
			source_ref, shell_kind, raw_declaration_json, activated_domains_json,
			source_generation, declaration_generation, '', '[]', 0, '',
			declaration_digest, state, created_unix_nano, updated_unix_nano, tombstoned_unix_nano
		FROM canonical_registration`,
		`DROP TRIGGER IF EXISTS canonical_registration_identity_immutable`,
		`DROP TRIGGER IF EXISTS canonical_registration_no_reactivation`,
		`DROP TRIGGER IF EXISTS canonical_registration_no_delete`,
		`DROP TABLE canonical_registration`,
		`ALTER TABLE canonical_registration_migrating RENAME TO canonical_registration`,
	}
	for _, statement := range statements {
		if _, err := tx.ExecContext(ctx, statement); err != nil {
			return fmt.Errorf("migrate registered App schema: %w", err)
		}
	}
	if err := assertSQLiteForeignKeys(ctx, tx); err != nil {
		return err
	}
	if err := tx.Commit(); err != nil {
		return fmt.Errorf("commit registered App schema migration: %w", err)
	}
	if _, err := connection.ExecContext(ctx, `PRAGMA foreign_keys = ON`); err != nil {
		return fmt.Errorf("restore registered App migration foreign keys: %w", err)
	}
	var foreignKeys int
	if err := connection.QueryRowContext(ctx, `PRAGMA foreign_keys`).Scan(&foreignKeys); err != nil {
		return fmt.Errorf("read registered App migration foreign-key posture: %w", err)
	}
	if foreignKeys != 1 {
		return fmt.Errorf("registered App migration foreign keys were not restored")
	}
	foreignKeysEnabled = true
	if err := assertSQLiteForeignKeys(ctx, connection); err != nil {
		return err
	}
	return nil
}
...
func assertSQLiteForeignKeys(ctx context.Context, queryer sqliteQueryer) error {
	rows, err := queryer.QueryContext(ctx, `PRAGMA foreign_key_check`)
	if err != nil {
		return fmt.Errorf("check registered App migration foreign keys: %w", err)
	}
	defer func() { _ = rows.Close() }()
	if rows.Next() {
		...
		if err := rows.Scan(&table, &rowID, &parent, &foreignKeyID); err != nil {
			return fmt.Errorf("read registered App migration foreign-key failure: %w", err)
		}
		return fmt.Errorf("registered App migration foreign-key failure: table=%s parent=%s row=%v key=%d", table, parent, rowID, foreignKeyID)
	}
	return rows.Err()
}

func (kernel *Kernel) initialize(ctx context.Context) error {
	if err := kernel.migrateLegacyCanonicalRegistration(ctx); err != nil {
		return err
	}
	statements := []string{
		canonicalRegistrationCreateStatement,
