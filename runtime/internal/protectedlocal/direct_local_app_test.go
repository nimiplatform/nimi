...
func TestDirectInstalledBindingPreservesSourceGenerationsAndOneUse(t *testing.T) {
	for _, trustClass := range []LocalAppTrustClass{LocalAppTrustVerified, LocalAppTrustUserImported} {
		t.Run(string(trustClass), func(t *testing.T) {
			...
			if !ok || actual != handle || connection.TrustClass() != trustClass || connection.Process() != process {
