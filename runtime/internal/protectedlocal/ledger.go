...
func (ledger *Ledger) verifyLogicalRecords(ctx context.Context, commits []commitRow) error {
	...
	if err := ledger.verifyLifecycleRecords(ctx, commitBySequence, createdCounts, revokedCounts); err != nil {
		return err
	}
	for _, commit := range commits {
		...
		case "lifecycle_challenge_prepare":
			created, revoked := createdCounts[commit.sequence], revokedCounts[commit.sequence]
			if !((created == 1 && revoked == 0) || (created == 2 && revoked == 1)) {
				return ledger.rollbackFailure(fmt.Errorf("verify protected lifecycle challenge prepare: record mismatch"))
			}
		case "lifecycle_intent_consume", "lifecycle_challenge_expire":
			if createdCounts[commit.sequence] != 1 || revokedCounts[commit.sequence] != 1 {
				return ledger.rollbackFailure(fmt.Errorf("verify protected lifecycle intent transition: record mismatch"))
			}
		default:
...
func (ledger *Ledger) StartRuntime(ctx context.Context) (Identifier, error) {
	...
	err = ledger.commit(ctx, "runtime_start", payload, func(tx *sql.Tx, sequence uint64) error {
		...
		if err := ledger.setLifecycleRevocations(ctx, tx, sequence, nil); err != nil {
			return err
		}
...
func (ledger *Ledger) RecordDesktopSessionRevoked(ctx context.Context, sessionID Identifier) error {
	...
	return ledger.commit(ctx, "desktop_session_revoke", sessionID[:], func(tx *sql.Tx, sequence uint64) error {
		...
		if err := ledger.setLifecycleRevocations(ctx, tx, sequence, &sessionID); err != nil {
			return err
		}
		return nil
	})
...
func (ledger *Ledger) discardPending(ctx context.Context, sequence uint64) error {
	...
	if err := ledger.deletePendingLifecycleCreations(ctx, tx, sequence); err != nil {
		return err
	}
	if err := ledger.restorePendingLifecycleRevocations(ctx, tx, sequence); err != nil {
		return err
	}
...
var protectedLocalSchema = []string{
	...
	`CREATE TABLE IF NOT EXISTS protected_lifecycle_challenge (
		challenge_id BLOB PRIMARY KEY CHECK(length(challenge_id) = 32),
		desktop_session_id BLOB NOT NULL CHECK(length(desktop_session_id) = 32),
		process_tuple_hash BLOB NOT NULL CHECK(length(process_tuple_hash) = 32),
		...
		displayed_intent_hash BLOB NOT NULL CHECK(length(displayed_intent_hash) = 32),
		expected_adoption_generation INTEGER NOT NULL CHECK(expected_adoption_generation >= 0),
		delete_durable_data INTEGER NOT NULL CHECK(delete_durable_data IN (0, 1)),
		health_repair_action INTEGER NOT NULL CHECK(health_repair_action BETWEEN 0 AND 4),
		target_job_id TEXT NOT NULL,
		runtime_boot_epoch BLOB NOT NULL CHECK(length(runtime_boot_epoch) = 32),
		issued_unix_nano INTEGER NOT NULL,
		deadline_unix_nano INTEGER NOT NULL,
		created_commit_sequence INTEGER NOT NULL REFERENCES protected_security_commit(commit_sequence),
		revoked_commit_sequence INTEGER REFERENCES protected_security_commit(commit_sequence),
		record_hmac BLOB NOT NULL CHECK(length(record_hmac) = 32)
	) STRICT`,
	`CREATE UNIQUE INDEX IF NOT EXISTS protected_lifecycle_challenge_one_outstanding ON protected_lifecycle_challenge(desktop_session_id, action, app_id) WHERE revoked_commit_sequence IS NULL`,
	`CREATE TABLE IF NOT EXISTS protected_lifecycle_intent (
		intent_id BLOB PRIMARY KEY CHECK(length(intent_id) = 32),
		challenge_id BLOB NOT NULL UNIQUE CHECK(length(challenge_id) = 32),
		desktop_session_id BLOB NOT NULL CHECK(length(desktop_session_id) = 32),
		process_tuple_hash BLOB NOT NULL CHECK(length(process_tuple_hash) = 32),
		account_generation INTEGER NOT NULL CHECK(account_generation >= 0),
		action TEXT NOT NULL,
		...
		release_ref TEXT NOT NULL,
		artifact_digest BLOB NOT NULL CHECK(length(artifact_digest) = 32),
		displayed_impact_digest BLOB NOT NULL CHECK(length(displayed_impact_digest) = 32),
		expected_adoption_generation INTEGER NOT NULL CHECK(expected_adoption_generation >= 0),
		delete_durable_data INTEGER NOT NULL CHECK(delete_durable_data IN (0, 1)),
		health_repair_action INTEGER NOT NULL CHECK(health_repair_action BETWEEN 0 AND 4),
		target_job_id TEXT NOT NULL,
