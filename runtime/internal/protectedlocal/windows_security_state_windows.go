...
func OpenWindowsRuntimeSecurityState(
	...
	if process.principalSID != principal.serviceSID {
		return nil, principalFailure("bind Windows Runtime security state to process", fmt.Errorf("service principal capability mismatch"))
	}
	if root.serviceSID != principal.serviceSID || root.path == "" {
		return nil, custodyFailure("open Windows Runtime security state", fmt.Errorf("validated protected state-root capability required"))
	}
