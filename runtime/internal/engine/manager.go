...
func (m *Manager) ListEngines() []SupervisorInfo {
	...
	for kind, s := range m.supervisors {
		if kind == engineManagedImageBackend || kind == engineImageExecutionHost || kind == engineVideoExecutionHost || kind == engineVisionExecutionHost || kind == engineFaceSwapExecutionHost {
			continue
		}
		running[kind] = s.Info()
	}
