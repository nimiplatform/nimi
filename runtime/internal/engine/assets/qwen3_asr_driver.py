...
def allow_empty_transcript(request: dict[str, Any]) -> bool:
    ...
    if not isinstance(extensions, dict):
        extensions = {}
    is_first_run_probe = bool_request(request, "nimi_first_run_baseline_probe") or bool_request(extensions, "nimi_first_run_baseline_probe")
    allow_empty = (
...
def handle_transcribe(request: dict[str, Any], cli_default_model: str) -> dict[str, Any]:
    ...
    if not text:
        if allow_empty:
            response: dict[str, Any] = {"text": "", "empty_transcript": True}
