...
# Modified by Nimi on 2026-09-09: use four-dimensional SDPA inputs and
# omit the redundant all-true mask for a single packed image sequence.
...
def sdpa_attention(
    ...
    attention_mask = None
    if len(q_cu_seqlens) > 2:
        attention_mask = torch.zeros(
            [1, 1, seq_length, seq_length], device=q.device, dtype=torch.bool
        )
