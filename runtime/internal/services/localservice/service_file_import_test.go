...
func TestRuntimeDependencySetupRequiresConfirmationAndUsesRuntimeJob(t *testing.T) {
	svc := newTestService(t)
	setLocalRuntimePlatformForTest(t, "windows", "amd64")
	t.Setenv("NIMI_RUNTIME_GPU_VENDOR", "nvidia")
	t.Setenv("NIMI_RUNTIME_GPU_CUDA_READY", "false")
	...
	resolved, err := svc.ResolveLocalRuntimeDependency(context.Background(), &runtimev1.ResolveLocalRuntimeDependencyRequest{
		DependencyId: "cuda-user-space-runtime",
		LocalAssetId: imported.GetAsset().GetLocalAssetId(),
	})
	if err != nil {
		t.Fatalf("ResolveLocalRuntimeDependency: %v", err)
	}
	if !resolved.GetDependency().GetConfirmationRequired() {
		t.Fatalf("expected confirmation-required dependency, got %#v", resolved.GetDependency())
	}

	_, err = svc.StartLocalRuntimeDependencySetup(context.Background(), &runtimev1.StartLocalRuntimeDependencySetupRequest{
		DependencyId: "cuda-user-space-runtime",
		LocalAssetId: imported.GetAsset().GetLocalAssetId(),
		Confirmed:    false,
	})
	if err == nil {
		t.Fatal("expected setup without confirmation to fail closed")
	}
	if mgr.ensureSharedAcceleratorDependencyCalls != 0 {
		t.Fatalf("setup without confirmation must not start runtime dependency install, got %d calls", mgr.ensureSharedAcceleratorDependencyCalls)
	}

	started, err := svc.StartLocalRuntimeDependencySetup(context.Background(), &runtimev1.StartLocalRuntimeDependencySetupRequest{
		DependencyId: "cuda-user-space-runtime",
		LocalAssetId: imported.GetAsset().GetLocalAssetId(),
		Confirmed:    true,
	})
	if err != nil {
		t.Fatalf("StartLocalRuntimeDependencySetup confirmed: %v", err)
	}
	if mgr.ensureSharedAcceleratorDependencyCalls != 1 {
		t.Fatalf("expected one runtime-owned dependency setup call, got %d", mgr.ensureSharedAcceleratorDependencyCalls)
	}
	if started.GetTransfer().GetState() != localTransferStateCompleted {
		t.Fatalf("expected completed transfer, got %#v", started.GetTransfer())
	}
	if started.GetDependency().GetState() != "ready_managed" {
		t.Fatalf("expected ready_managed dependency, got %#v", started.GetDependency())
	}
}

func TestRuntimeDependencySetupCanResolveSharedCUDAWithoutImportedAsset(t *testing.T) {
	svc := newTestService(t)
	setLocalRuntimePlatformForTest(t, "windows", "amd64")
	t.Setenv("NIMI_RUNTIME_GPU_VENDOR", "nvidia")
	t.Setenv("NIMI_RUNTIME_GPU_CUDA_READY", "false")
	...
	if err != nil {
		t.Fatalf("ResolveLocalRuntimeDependency without asset: %v", err)
	}
	if !resolved.GetDependency().GetConfirmationRequired() {
		t.Fatalf("expected confirmation-required shared dependency, got %#v", resolved.GetDependency())
	}
	if got := resolved.GetDependency().GetConsumerId(); got != "" {
		t.Fatalf("shared dependency resolution must not invent an image consumer id, got %q", got)
	}
	...
	if err != nil {
		t.Fatalf("StartLocalRuntimeDependencySetup without asset: %v", err)
	}
	if mgr.ensureSharedAcceleratorDependencyCalls != 1 {
		t.Fatalf("expected one runtime-owned shared dependency setup call, got %d", mgr.ensureSharedAcceleratorDependencyCalls)
	}
	if got := started.GetDependency().GetConsumerId(); got != "" {
		t.Fatalf("shared setup must not invent an image consumer id, got %q", got)
	}
	if started.GetDependency().GetState() != "ready_managed" {
		t.Fatalf("expected ready_managed dependency, got %#v", started.GetDependency())
	}
