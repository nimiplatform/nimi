...
func localProviderPreferenceOrder(goos string, capability string) []string {
	...
	case "windows":
		switch normalizeLocalRoutingCapability(capability) {
...
func localProviderPreferenceRank(goos string, capability string, provider string) int {
	...
	for index, engine := range localProviderPreferenceOrder(goos, capability) {
		if normalizedProvider == engine {
			return index
		}
	}
	return len(localProviderPreferenceOrder(goos, capability))
}
