...
func TestWarmLegacyLlamaAssetAlwaysRequiresPrivateExecutionHost(t *testing.T) {
	...
	if reason, ok := grpcerr.ExtractReasonCode(err); !ok || reason != runtimev1.ReasonCode_AI_ROUTE_UNSUPPORTED {
		t.Fatalf("unexpected reason mismatch: got=%v ok=%v want=%v", reason, ok, runtimev1.ReasonCode_AI_ROUTE_UNSUPPORTED)
	}
}
