...
func TestBuiltInDesktopAppUsesFormalRegistrationSessionAndEffectiveAccess(t *testing.T) {
	...
	authorized, err := service.AuthorizeBuiltInAppIngress(
	...
	secondAuthorized, err := service.AuthorizeBuiltInAppIngress(
	...
	service.builtInLocalAppMu.Lock()
	binding := service.builtInLocalApps[builtInLocalAppConnectionKey{desktop: desktop, appID: "nimi.desktop"}]
	service.builtInLocalAppMu.Unlock()
	...
	renewedAuthorized, err := service.AuthorizeBuiltInAppIngress(
