...
func TestOpenAppLaunchesInstalledApp(t *testing.T) {
	...
	resp, err := svc.OpenApp(context.Background(), &runtimev1.OpenAppRequest{
		AppId: "nimi.parentos",
		Scope: appOpenScope("nimi.parentos"),
	})
	if err != nil {
	...
	if proj.GetScope().GetKind() != "app" || proj.GetScope().GetOwnerId() != "nimi.parentos" {
		t.Fatalf("scope = %v, want app/nimi.parentos", proj.GetScope())
	}
...
func TestOpenAppFailsClosedWithoutScopeRef(t *testing.T) {
	...
	resp, err := svc.OpenApp(context.Background(), &runtimev1.OpenAppRequest{AppId: "nimi.parentos"})
	if err != nil {
		t.Fatalf("OpenApp: %v", err)
	}
	proj := resp.GetProjection()
...
func TestOpenAppFailsClosedOnNonAppScopeKind(t *testing.T) {
	...
	resp, err := svc.OpenApp(context.Background(), &runtimev1.OpenAppRequest{
		AppId: "nimi.parentos",
		Scope: &runtimev1.AppOpenScopeRef{Kind: "account", OwnerId: "nimi.parentos"},
	})
	if err != nil {
		t.Fatalf("OpenApp: %v", err)
	}
...
func TestOpenAppFailsClosedOnCorruptedAppData(t *testing.T) {
	...
	durableRoot := filepath.Join(dataRoot, "apps", "nimi.parentos", "data")
