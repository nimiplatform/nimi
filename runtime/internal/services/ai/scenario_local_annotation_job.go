...
func (s *Service) submitLocalAnnotationScenarioJob(ctx context.Context, req *runtimev1.SubmitScenarioJobRequest, mode runtimev1.ExecutionMode, ignored []*runtimev1.IgnoredScenarioExtension) (*runtimev1.SubmitScenarioJobResponse, error) {
	...
	effective, err := s.captureLocalAnnotationEffectiveInputs(ctx, req.GetHead(), req.GetSpec().GetTextAnnotate())
	if err != nil {
		return nil, err
	}
	...
	ticket := s.localAnnotationJobOrder.reserve()
	go s.runLocalAnnotationScenarioJob(jobCtx, jobID, ticket)
	return &runtimev1.SubmitScenarioJobResponse{Job: stored}, nil
}

func (s *Service) runLocalAnnotationScenarioJob(ctx context.Context, jobID string, ticket *localMediaSubmissionTicket) {
	...
	plan, err := annotationPlanFromResolvedAssembly(assembly)
	if err != nil {
	...
	onStart := func() error {
		release, err := s.acquireAsyncScenarioJobLease(ctx, job.GetHead().GetAppId(), "scenario_job_local_annotation")
		if err != nil {
			return err
		}
	...
	_ = s.completeAnnotationScenarioJob(jobID, result)
}
