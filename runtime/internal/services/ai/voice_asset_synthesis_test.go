...
func TestLocalQwenVoiceAssetCreateUseDeleteLifecycle(t *testing.T) {
	...
	submitResp, err := svc.SubmitScenarioJob(context.Background(), &runtimev1.SubmitScenarioJobRequest{
		Head: &runtimev1.ScenarioRequestHead{
			...
			TargetRef:     setExactLocalScenarioTargetForTest(t, svc, "speech/qwen3tts-base", "voice_workflow.voice_clone", svc.localModel.(*fakeLocalModelLister).responses[0].Assets[1]),
	...
	synthResp, err := svc.SubmitScenarioJob(context.Background(), &runtimev1.SubmitScenarioJobRequest{
		Head: &runtimev1.ScenarioRequestHead{
			...
			SubjectUserId: "user-001",
			ModelId:       "speech/qwen3tts-base",
			TargetRef:     setExactLocalScenarioTargetForTest(t, svc, "speech/qwen3tts-base", "audio.synthesize", svc.localModel.(*exactTargetLocalModelLister).asset),
			RoutePolicy:   runtimev1.RoutePolicy_ROUTE_POLICY_LOCAL,
			Fallback:      runtimev1.FallbackPolicy_FALLBACK_POLICY_DENY,
			TimeoutMs:     30_000,
		},
