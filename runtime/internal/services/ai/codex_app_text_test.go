...
func codexAppFixture(t *testing.T, handler http.HandlerFunc) (managedCloudScenarioTestFixture, func(accountservice.LocalAppOperation, string) context.Context) {
	t.Helper()
	server := httptest.NewServer(handler)
	t.Cleanup(server.Close)
	fixture := newManagedCloudScenarioTestFixture(t, "openai_codex", "gpt-5.6-sol", server.URL, Config{AllowLoopbackEndpoint: true})
	target, _ := structpb.NewStruct(map[string]any{"provider": "openai_codex", "providerModelId": "gpt-5.6-sol", "remoteModelCatalogId": fixture.descriptor.GetRemoteModelCatalogId()})
...
func TestCodexAppTextSSESyncAndStreamPreserveContinuityRoundTrip(t *testing.T) {
	...
	fixture, decision := codexAppFixture(t, func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path != "/responses" || r.Header.Get("Authorization") != "Bearer test-key" || body["stream"] != true || body["store"] != false || body["model"] != "gpt-5.6-sol" {
