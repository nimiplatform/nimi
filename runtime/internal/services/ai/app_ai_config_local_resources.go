...
func (s *Service) projectLocalResourceProjection(option localexecution.LoadoutOption) *runtimev1.AIConfigLocalResourceProjection {
	...
	if (option.CapabilityContract == capabilitydriver.MiniMaxMusic3CapabilityContract || option.CapabilityContract == capabilitydriver.MusicTranscribeCapabilityContract || option.CapabilityContract == capabilitydriver.VoiceConvertCapabilityContract) && s.capabilityDrivers != nil {
		driver, _ := s.capabilityDrivers.Resolve(option.CapabilityContract, capabilitydriver.IdentityFromProto(option.Implementation))
