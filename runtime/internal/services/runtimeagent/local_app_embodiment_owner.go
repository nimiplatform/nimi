...
const (
	...
	localAppEmbodimentRevalidationInterval = 250 * time.Millisecond
...
// localAppEmbodimentScopeResolver is the formal-session composition seam. It
// owns current AppOperation coverage plus opaque-handle and exact
...
type localAppEmbodimentSemanticOwner interface {
	...
	SubscribeLocalAppEmbodiment(context.Context, localAppEmbodimentScope, uint64, func(context.Context) error, func() error, func(localAppEmbodimentEvent) error) error
}
...
func (o *localAppEmbodimentReadOwner) Subscribe(
	...
	return o.subscribe(ctx, req, nil, emit)
}

func (o *localAppEmbodimentReadOwner) SubscribeEstablished(
	ctx context.Context,
	req localAppEmbodimentSubscribeRequest,
	...
	return o.subscribe(ctx, req, established, emit)
}
...
func (o *localAppEmbodimentReadOwner) subscribe(
	...
	if established == nil {
		established = func() error { return nil }
	}
	return o.owner.SubscribeLocalAppEmbodiment(ctx, scope, req.AfterSequence, revalidate, established, func(event localAppEmbodimentEvent) error {
...
// runtimeAgentEmbodimentSemanticOwner projects the bounded Runtime semantic
// owner used by the two formal App embodiment reads.
...
func (o runtimeAgentEmbodimentSemanticOwner) SubscribeLocalAppEmbodiment(
	...
	if revalidate == nil || established == nil || emit == nil {
		return errLocalAppEmbodimentUnavailable
	}
	if err := o.validateScope(ctx, scope); err != nil {
		return err
	...
	revalidationTicker := time.NewTicker(localAppEmbodimentRevalidationInterval)
...
func projectRuntimeAgentEmbodimentEvent(
	...
	case runtimev1.AgentEventType_AGENT_EVENT_TYPE_PRESENTATION:
		...
		case runtimev1.AgentPresentationEventFamily_AGENT_PRESENTATION_EVENT_FAMILY_VOICE_TIMING_READY,
			runtimev1.AgentPresentationEventFamily_AGENT_PRESENTATION_EVENT_FAMILY_VOICE_ARTIFACT_AVAILABLE,
			runtimev1.AgentPresentationEventFamily_AGENT_PRESENTATION_EVENT_FAMILY_VOICE_TIMING_TERMINAL:
...
func projectRuntimeAgentEmbodimentVoiceTiming(
	...
	correlationRef := firstNonEmpty(
		strings.TrimSpace(detail.GetMessageId()),
		strings.TrimSpace(detail.GetAudioArtifactId()),
	)
	if !validLocalAppConversationSelector(correlationRef) || !validLocalAppConversationSelector(detail.GetTurnId()) {
		return nil, fmt.Errorf("Runtime embodiment voice correlation is incomplete")
	}
	phase := ""
	switch detail.GetVoiceTimingPhase() {
	case runtimev1.AgentVoiceTimingPhase_AGENT_VOICE_TIMING_PHASE_ACTIVE:
		phase = "active"
	case runtimev1.AgentVoiceTimingPhase_AGENT_VOICE_TIMING_PHASE_COMPLETED:
		phase = "completed"
	case runtimev1.AgentVoiceTimingPhase_AGENT_VOICE_TIMING_PHASE_FAILED:
		phase = "failed"
	case runtimev1.AgentVoiceTimingPhase_AGENT_VOICE_TIMING_PHASE_INTERRUPTED:
		phase = "interrupted"
	case runtimev1.AgentVoiceTimingPhase_AGENT_VOICE_TIMING_PHASE_CANCELED:
		phase = "canceled"
	default:
