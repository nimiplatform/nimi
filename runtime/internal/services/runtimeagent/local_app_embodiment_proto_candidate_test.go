...
func TestLocalAppEmbodimentCandidateProtoProjectionIsClosed(t *testing.T) {
	now := time.Now().UTC()
	snapshot, err := localAppEmbodimentSnapshotToProto(localAppEmbodimentSnapshot{
		...
		VoiceTiming: &localAppEmbodimentVoiceTiming{
			Phase: "active", DurationMillis: 640, DeadlineOffsetMillis: 80, TurnRef: "turn-1", VoiceRef: "voice-1",
		},
	})
	if err != nil {
		t.Fatal(err)
