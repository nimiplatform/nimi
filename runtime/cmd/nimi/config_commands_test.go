...
func TestRunRuntimeConfigSetPreservesSourceMaterializationPacketSecret(t *testing.T) {
	homeDir := t.TempDir()
	setCmdTestHome(t, homeDir)
	t.Setenv("NIMI_RUNTIME_CONFIG_PATH", "")
	clearRuntimeConfigCommandEnv(t)
	...
	setOutput, err := captureStdoutFromRun(func() error {
		return runRuntimeConfig([]string{
			"set",
			"--set", "sourceMaterializationPacketHmacSecret=source-packet-secret-v1",
			"--json",
		})
	})
	if err != nil {
		t.Fatalf("set sourceMaterializationPacketHmacSecret: %v", err)
	}
	setPayload := parseJSONMap(t, setOutput)
	if asString(setPayload["reasonCode"]) != configReasonRestartRequired {
		t.Fatalf("set reasonCode mismatch: %s", setOutput)
	...
	if cfg.SourceMaterializationPacketHMACSecret != "source-packet-secret-v1" {
		t.Fatalf("source materialization packet secret mismatch after set: %q", cfg.SourceMaterializationPacketHMACSecret)
	}
	...
	cfg, loadErr = config.LoadFileConfig(cfgPath)
	if loadErr != nil {
		t.Fatalf("LoadFileConfig after unrelated set: %v", loadErr)
	}
	if cfg.SourceMaterializationPacketHMACSecret != "source-packet-secret-v1" {
		t.Fatalf("source materialization packet secret should survive unrelated set, got %q", cfg.SourceMaterializationPacketHMACSecret)
	}

	unsetOutput, err := captureStdoutFromRun(func() error {
		return runRuntimeConfig([]string{
			"set",
			"--unset", "sourceMaterializationPacketHmacSecret",
			"--json",
		})
	})
	if err != nil {
		t.Fatalf("unset sourceMaterializationPacketHmacSecret: %v", err)
	}
	unsetPayload := parseJSONMap(t, unsetOutput)
	if asString(unsetPayload["reasonCode"]) != configReasonRestartRequired {
		t.Fatalf("unset reasonCode mismatch: %s", unsetOutput)
	...
	if strings.TrimSpace(cfg.SourceMaterializationPacketHMACSecret) != "" {
		t.Fatalf("source materialization packet secret should be unset, got %q", cfg.SourceMaterializationPacketHMACSecret)
	}
}
