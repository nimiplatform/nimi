...
function ensureManifestVersion(value: unknown): string {
  ...
  if (!manifestVersion) {
    throw createNimiError({
      ...
      reasonCode: 'APP_SCOPE_MANIFEST_INVALID',
...
function ensureScopeNamespace(scope: ScopeName, appId: string): void {
  ...
  if (!scope.startsWith(expectedPrefix)) {
    throw createNimiError({
      ...
      reasonCode: 'APP_SCOPE_NAMESPACE_FORBIDDEN',
  ...
  const resolvePublishedCatalogVersion = (version?: ScopeCatalogVersion): ScopeCatalogVersion => {
    ...
    if (!latest) {
      throw createNimiError({
        ...
        reasonCode: 'APP_SCOPE_CATALOG_UNPUBLISHED',
