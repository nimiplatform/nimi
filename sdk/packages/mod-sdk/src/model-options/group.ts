import type { GroupedModelOptions } from './presets';
...
export function groupModelOptions(models: string[], query: string): GroupedModelOptions {
  const filtered = filterModelOptions(models, query);
  const grouped = new Map<keyof typeof MODEL_TYPE_LABELS, string[]>();
