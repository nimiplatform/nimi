...
import type { AiFallbackPolicy, AiRoutePolicy } from '@nimiplatform/sdk-types';
...
function ensureText(value: unknown, fieldName: string): string {
  ...
  if (!normalized) {
    throw createNimiError({
      ...
      reasonCode: 'SDK_AI_PROVIDER_CONFIG_INVALID',
...
function toRuntimePrompt(prompt: LanguageModelV3Prompt): {
  ...
  for (const message of prompt) {
    ...
    const content = Array.isArray(message.content)
      ? message.content.map(extractTextValue).filter((text) => text.length > 0).join('\n')
      : '';
    if (!content) {
      continue;
    }
...
function toBase64(value: Uint8Array): string {
  ...
  throw createNimiError({
    ...
    reasonCode: 'SDK_AI_PROVIDER_BASE64_UNAVAILABLE',
...
function ensureRuntime(config: NimiAiProviderConfig): {
  ...
  if (!config.runtime || !config.runtime.ai) {
    throw createNimiError({
      ...
      reasonCode: 'SDK_AI_PROVIDER_RUNTIME_REQUIRED',
