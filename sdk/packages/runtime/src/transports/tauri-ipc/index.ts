...
function toBase64(bytes: Uint8Array): string {
  ...
  throw createNimiError({
    ...
    reasonCode: 'SDK_RUNTIME_BASE64_ENCODER_UNAVAILABLE',
...
function fromBase64(value: string): Uint8Array {
  ...
  throw createNimiError({
    ...
    reasonCode: 'SDK_RUNTIME_BASE64_DECODER_UNAVAILABLE',
    actionHint: 'use_node_or_tauri_runtime',
    source: 'sdk',
  });
}
...
function normalizeRequestBytes(input: unknown): Uint8Array {
  ...
  throw createNimiError({
    ...
    reasonCode: 'SDK_RUNTIME_REQUEST_BYTES_REQUIRED',
  ...
  return {
      ...
      try {
        ...
        if (typeof response.responseBytesBase64 !== 'string') {
          throw createNimiError({
            ...
            reasonCode: 'SDK_RUNTIME_TAURI_UNARY_BYTES_MISSING',
      ...
      try {
        ...
        if (!streamId) {
          throw createNimiError({
            ...
            reasonCode: 'SDK_RUNTIME_TAURI_STREAM_ID_MISSING',
        ...
        unsubscribe = await Promise.resolve(listen(createEventName(config, streamId), (event) => {
          ...
          if (eventType === 'error') {
            onStreamError(payload.error || createNimiError({
              ...
              reasonCode: 'SDK_RUNTIME_TAURI_STREAM_REMOTE_ERROR',
              actionHint: 'retry_or_reopen_stream',
              source: 'runtime',
            }));
            return;
    ...
    closeStream: async (input: RuntimeStreamCloseCall): Promise<void> => {
      ...
      try {
        ...
        throw asNimiError(error, {
          reasonCode: 'SDK_RUNTIME_TAURI_STREAM_CLOSE_FAILED',
