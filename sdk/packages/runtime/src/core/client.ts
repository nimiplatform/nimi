...
function encodeRequest<Request>(
  ...
  try {
    const payload = codec.requestType.create(request as Partial<Request>);
    return codec.requestType.toBinary(payload);
  } catch (error) {
    throw createNimiError({
      message: `${methodId} request encode failed: ${error instanceof Error ? error.message : String(error)}`,
      reasonCode: 'SDK_RUNTIME_REQUEST_ENCODE_FAILED',
...
function decodeUnaryResponse<Response>(
  ...
  try {
    return codec.responseType.fromBinary(payload);
  } catch (error) {
    throw createNimiError({
  ...
  const unary = <Request, Response>(methodId: string) => async (
    ...
    if (!codec) {
      throw createNimiError({
        ...
        reasonCode: 'SDK_RUNTIME_CODEC_MISSING',
