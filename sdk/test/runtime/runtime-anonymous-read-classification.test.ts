...
test('RuntimeAnonymousReadMethodIds set has exactly 130 members (current spec shard size)', () => {
  ...
  assert.equal(distinct.size, 130, 'classifier set must contain exactly 130 method ids');
});
