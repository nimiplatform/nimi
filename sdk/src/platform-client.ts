...
type PlatformDomains = {
  ...
  world: {
    ...
    listWorldBindings: RealmServices['WorldsService']['worldControllerGetWorldBindings'];
...
import {
  ...
  resolvePlatformRealmBaseUrl,
  resolveToken,
  timestampToMillis,
} from './platform-client-auth-session.js';
...
function createDomains(
  ...
  return {
    ...
    world: {
      ...
      listWorldBindings: (worldId) => realm.services.WorldsService.worldControllerGetWorldBindings(worldId),
...
export async function createPlatformClient(input: PlatformClientInput): Promise<PlatformClient> {
  ...
  const runtimeSubjectUserIdProvider = async () => {
    ...
    const jwtSubjectUserId = decodeJwtSubjectUserId(await runtimeAccessTokenProvider());
    if (jwtSubjectUserId) {
      return jwtSubjectUserId;
    }
    return '';
  };
