...
export type RealmWorldLorebookListPayload = RealmServiceResult<'WorldsService', 'worldControllerGetWorldLorebooks'>;
export type RealmWorldBindingListPayload = RealmServiceResult<'WorldsService', 'worldControllerGetWorldBindings'>;
export type RealmWorldSceneListPayload = RealmServiceResult<'WorldsService', 'getWorldScenes'>;

export type RealmWorldApiCaller = <T>(task: (realm: Realm) => Promise<T>, fallbackMessage?: string) => Promise<T>;
export type RealmWorldErrorEmitter = (
  action: string,
  error: unknown,
  details?: JsonObject,
) => void;

export type RealmWorldSemanticBundle = {
...
export type RealmWorldHistoryPayload = PublicWorldHistoryPayload;
...
export function buildRealmWorldDetailWithAgentsCacheKey(
  worldId: string,
...
export async function loadRealmWorldList(
...
export async function loadRealmMainWorld(
  callApi: RealmWorldApiCaller,
  emitRealmWorldError: RealmWorldErrorEmitter,
): Promise<WorldDetailDto> {
  try {
    ...
    return requireRecord(payload, 'MAIN_WORLD_CONTRACT_INVALID') as WorldDetailDto;
  } catch (error) {
...
export async function loadRealmWorldLevelAudits(
  callApi: RealmWorldApiCaller,
  emitRealmWorldError: RealmWorldErrorEmitter,
  worldId: string,
  limit = 20,
...
export async function loadRealmWorldDetailById(
  callApi: RealmWorldApiCaller,
  emitRealmWorldError: RealmWorldErrorEmitter,
  worldId: string,
...
export async function loadRealmWorldHistory(
  callApi: RealmWorldApiCaller,
  emitRealmWorldError: RealmWorldErrorEmitter,
  worldId: string,
): Promise<RealmWorldHistoryPayload> {
  const normalizedWorldId = normalizeWorldId(worldId);
  try {
    const payload = await callApi(
    ...
    return record as RealmWorldHistoryPayload;
  } catch (error) {
...
export async function loadRealmWorldLorebooks(
  callApi: RealmWorldApiCaller,
  emitRealmWorldError: RealmWorldErrorEmitter,
  worldId: string,
): Promise<RealmWorldLorebookListPayload> {
...
export async function loadRealmWorldBindings(
  callApi: RealmWorldApiCaller,
  emitRealmWorldError: RealmWorldErrorEmitter,
  worldId: string,
): Promise<RealmWorldBindingListPayload> {
  return loadWorldAssetList(
    callApi,
    emitRealmWorldError,
    worldId,
...
export async function loadRealmWorldScenes(
  callApi: RealmWorldApiCaller,
  emitRealmWorldError: RealmWorldErrorEmitter,
  worldId: string,
): Promise<RealmWorldSceneListPayload> {
  return loadWorldAssetList(
    callApi,
    emitRealmWorldError,
    worldId,
...
export async function loadRealmWorldAgents(
  callApi: RealmWorldApiCaller,
  emitRealmWorldError: RealmWorldErrorEmitter,
  worldId: string,
...
export async function loadRealmWorldDetailWithAgents(
  callApi: RealmWorldApiCaller,
  emitRealmWorldError: RealmWorldErrorEmitter,
  worldId: string,
  recommendedAgentLimit?: number,
...
export async function loadRealmWorldSemanticBundle(
  callApi: RealmWorldApiCaller,
  emitRealmWorldError: RealmWorldErrorEmitter,
  worldId: string,
): Promise<RealmWorldSemanticBundle> {
  const normalizedWorldId = normalizeWorldId(worldId);
