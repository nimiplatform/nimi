...
fn live2d_reference_path(
    ...
    label: &str,
    path_label: &str,
    errors: &mut Vec<StandardAgentCenterValidationIssue>,
) -> Option<String> {
...
fn validate_live2d_moc_header(
    ...
    package_path: &str,
    errors: &mut Vec<StandardAgentCenterValidationIssue>,
    warnings: &mut Vec<StandardAgentCenterValidationIssue>,
) {
    ...
    if size_mb > 30.0 {
        warnings.push(issue(
            ...
            Some(package_path.to_string()),
            StandardAgentCenterValidationIssueSeverity::Warning,
        ));
    }
}
