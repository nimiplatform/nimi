...
pub(super) fn account_operations_path(
    roots: &crate::runtime_app_storage::StandardAppStorageRoots,
    account_id: &str,
) -> Result<PathBuf, String> {
    Ok(account_dir(roots, account_id)?
        .join("operations")
        .join(OPERATIONS_FILE_NAME))
}
...
pub(super) fn append_operation_record_to_path(
    path: &Path,
    record: &StandardAgentCenterResourceOperationRecord,
) -> Result<(), String> {
...
pub(super) fn append_operation_record(
    ...
    let path = operations_path(roots, account_id, local_agent_ref)?;
...
pub(super) fn build_operation_record(
    ...
    status: &str,
    reason_code: &str,
) -> StandardAgentCenterResourceOperationRecord {
...
pub(super) fn record_resource_operation(
    ...
    append_operation_record(roots, account_id, local_agent_ref, &record)?;
    Ok(event_id)
}
...
pub(super) fn record_account_resource_operation(
    ...
    let event_id = record.event_id.clone();
    let path = account_operations_path(roots, account_id)?;
    append_operation_record_to_path(&path, &record)?;
    Ok(event_id)
}
...
pub(super) fn quarantine_path(
    ...
    let root = agent_center_dir(roots, account_id, local_agent_ref)?;
...
pub(super) fn quarantine_agent_center_tree(
    ...
    let source = agent_center_dir(roots, account_id, local_agent_ref)?;
    ...
    let destination =
        account_quarantine_path(roots, account_id, "agent_local_resources", local_agent_ref)?;
