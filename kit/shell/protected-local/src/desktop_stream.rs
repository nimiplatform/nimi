...
pub(crate) async fn open_bundled_avatar(
    ...
    impl Encoder for RawBytesEncoder {
        type Item = Vec<u8>;
        type Error = Status;
        fn encode(&mut self, item: Self::Item, destination: &mut EncodeBuf<'_>) -> Result<(), Self::Error> {
    ...
    grpc.ready().await.map_err(|_| BundledAvatarRuntimeError::new("runtime-service-unavailable", true))?;
    ...
    let response = grpc.server_streaming(
        request,
    ...
    tokio::spawn(async move {
        loop {
            match stream.message().await {
                Ok(Some(bytes)) => {
                    if sender.send(Ok(bytes)).await.is_err() { break; }
                }
                Ok(None) => break,
                Err(status) => {
...
fn map_status(status: tonic::Status) -> BundledAvatarRuntimeError {
    ...
    let reason = crate::grpc_status::runtime_reason(&status).unwrap_or_else(|| match status.code() {
