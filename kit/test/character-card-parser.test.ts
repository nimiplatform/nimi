...
import { parseCharacterCardV2 } from '../core/src/character-card/index.js';
...
describe('parseCharacterCardV2 (shared kit surface)', () => {
  ...
  it('rejects malformed JSON without throwing', () => {
    const result = parseCharacterCardV2('{ not json');

    expect(result.card).toBeNull();
    expect(result.rawCard).toBeNull();
    expect(result.validation.valid).toBe(false);
  });
});
