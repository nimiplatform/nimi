      ...
      <div className="overflow-x-auto">
        <table className="min-w-full border-collapse" aria-label={ariaLabel}>
            ...
            <tr>
              {columns.map((column) => (
                <th
                  ...
                  className={cn(
                    'nimi-data-table__th px-3 py-2 text-[length:var(--nimi-type-caption-size)] font-bold uppercase tracking-[var(--nimi-type-label-letter-spacing)] text-[var(--nimi-text-muted)]',
          ...
          <tbody className="nimi-data-table__body divide-y divide-[var(--nimi-border-subtle)]">
            {rows.length === 0 ? (
              <tr>
                <td colSpan={columns.length} className="nimi-data-table__empty px-3 py-6 text-center text-[length:var(--nimi-type-body-size)] text-[var(--nimi-text-secondary)]">
                  {empty ?? 'No rows'}
                </td>
              </tr>
              ...
              <tr key={rowKey(row, rowIndex)} className="nimi-data-table__row hover:bg-[var(--nimi-action-ghost-hover)]">
                {columns.map((column) => (
                  <td
                    key={column.key}
                    className={cn(
                      'nimi-data-table__td px-3 py-2 text-[length:var(--nimi-type-body-size)] text-[var(--nimi-text-secondary)]',
                      alignClass[column.align ?? 'left'],
                    )}
                  >
