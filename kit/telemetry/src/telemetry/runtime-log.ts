...
export type RuntimeLogger = (payload: RuntimeLogPayload) => void;
...
export function toRuntimeLogMessage(message: unknown): RuntimeLogMessage {
...
export function emitRuntimeLog(payload: RuntimeLogPayload): void {
  const normalized: RuntimeLogPayload = {
    ...
    message: toRuntimeLogMessage(payload.message),
  };
  if (!runtimeLogger) {
