...
- Current surfaces:
  ...
  - `ui`: active
- `runtime`: active for runtime-client-backed app-AI composer/session adapters and orchestration provider integration, including `simple-ai`, provider registry wiring, history budgeting, and stream normalization; pure app-AI text generate/stream helpers live in `@nimiplatform/sdk/ai-app`; Runtime Agent Chat remains Runtime Agent lifecycle, not this Kit surface
...
- Bind ordinary app-AI text generation through `@nimiplatform/sdk/ai-app`; use `chat/runtime` when the app needs reusable Kit composer/session adapters or the `simple-ai` conversation provider.
...
- Check `@nimiplatform/sdk/ai-app` for ordinary app-AI generate/stream helpers, then check `chat/runtime` for reusable composer/session adapters and orchestration providers before wrapping runtime text APIs directly in app code.
