  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className={cn('block truncate text-[13px]', label ? 'font-medium text-[var(--nimi-text-primary)]' : 'text-[var(--nimi-field-placeholder)]')}>{label || placeholder}</span>
        {hasDetail ? (
          <span className={cn(
      ...
      <svg aria-hidden="true" className="h-4 w-4 shrink-0 text-[var(--nimi-text-muted)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="m9 18 6-6-6-6" /></svg>
    </button>
  );
}
