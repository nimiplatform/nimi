...
export function parseVoiceWorkflowParams(stored: Readonly<Record<string, unknown>>): VoiceWorkflowParamsState {
  return {
    ...
    previewText: typeof stored.previewText === 'string' ? stored.previewText : DEFAULT_VOICE_WORKFLOW_PARAMS.previewText,
    language: typeof stored.language === 'string' ? stored.language : DEFAULT_VOICE_WORKFLOW_PARAMS.language,
    preferredName: typeof stored.preferredName === 'string' ? stored.preferredName : DEFAULT_VOICE_WORKFLOW_PARAMS.preferredName,
