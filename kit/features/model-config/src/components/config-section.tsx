...
export function ConfigSection(props: { readonly title: string; readonly children: ReactNode; readonly className?: string }) {
  return (
    <section className={cn('space-y-3', props.className)}>
      <h3 className="m-0 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--nimi-text-muted)]">{props.title}</h3>
      {props.children}
...
export function ConfigAccordionSection(props: {
  ...
  return (
    <section>
      <button type="button" onClick={props.onToggle} className="flex w-full items-center justify-between py-2.5">
        <h3 className="m-0 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--nimi-text-muted)]">{props.title}</h3>
...
export function DisabledConfigNote(props: { readonly label: string }) {
  return <div className="rounded-[var(--nimi-radius-md)] border border-dashed border-[var(--nimi-border-subtle)] bg-[var(--nimi-surface-panel)] px-3 py-4 text-center text-[11px] text-[var(--nimi-text-secondary)]">{props.label}</div>;
}
