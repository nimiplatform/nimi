...
function protectedMusicClient() {
  ...
  const artifact = { artifactId: 'artifact-music-1', mimeType: 'audio/wav', bytes: new Uint8Array(), uri: '', sha256: 'abc', sizeBytes: '7680058', durationMs: '20000', frameCount: '960000', fps: 0, width: 0, height: 0, sampleRateHz: 48000, channels: 2 };
  const score = { ...artifact, artifactId: 'artifact-score-1', mimeType: 'text/vnd.abc', sizeBytes: '659', durationMs: '0', frameCount: '0', sampleRateHz: 0, channels: 0 };
  const getScenarioArtifacts = vi.fn<NimiProtectedLocalScenarioJobClient['getScenarioArtifacts']>(async () => ({ jobId: 'job-music-1', artifacts: [artifact, score], traceId: 'trace-music-1', output: { output: { oneofKind: 'musicGenerate', musicGenerate: { artifacts: [artifact, score] } } } }));
...
describe('runRuntimeMusicGenerate', () => {
  it('keeps both artifacts and the typed termination through the protected async carrier', async () => {
    ...
    if (!result.ok) return;
    expect(result.output.firstArtifact).toMatchObject({ artifactId: 'artifact-music-1', mimeType: 'audio/wav', sampleRateHz: 48000, channels: 2 });
    expect(result.output.artifactCount).toBe(2);
    expect(result.output.generation.termination).toBe('budget-limit');
    expect(result.output.generation.generatedScore?.artifactId).toBe('artifact-score-1');
    ...
    expect(request?.spec?.spec).toEqual({ oneofKind: 'musicGenerate', musicGenerate: { prompt: 'bright synth-pop', negativePrompt: '', lyrics: '[Verse]\nCity lights are waking.', style: '', title: '', durationSeconds: 120, instrumental: false, scoreConditioning: 0, returnGeneratedScore: false, seed: undefined, score: undefined, audioReference: undefined } });
  });
  ...
  it('returns typed input-invalid before contacting Runtime for empty music input', async () => {
    ...
    expect(result.error.reasonCode).toBe('SDK_LOCAL_APP_INPUT_INVALID');
