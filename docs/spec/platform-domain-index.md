...
| 容器 | 覆盖面 |
|---|---|
| `.nimi/spec/platform/admission.authority.yaml` | spec 准入标准(可准入/硬拒绝类、O1-O3 迁移义务、R1-R6 执行规则) |
| `.nimi/spec/platform/testing-discipline.authority.yaml` | 测试纪律(ready 后固化 / bug 回归探测两形态) |
...
| `.nimi/spec/platform/authority-admission.authority.yaml` | app-slice 与 package 权威准入边界(含 realm 投影委托边界的引用式条款) |
