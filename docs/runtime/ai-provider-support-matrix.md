...
| NimiLLM | Online (`token-api`) | `nimillm/*` | `text/stream/embed/image/video/tts/stt` | Yes | Yes | REST + SSE | `openai_compat_adapter` | Cloud default backend + provider-hint routing. | [provider_cloud_test.go](../../runtime/internal/services/ai/provider_cloud_test.go) | [nimillm.ts](../examples/providers/nimillm.ts) |
| Alibaba (cloud adapter) | Online (`token-api`) | `aliyun/*`, `alibaba/*` | `text/stream/embed/image/video/tts/stt` | Yes | Yes | REST + SSE | `openai_compat_adapter` | Prefix route + availability fail-close. | [provider_cloud_test.go](../../runtime/internal/services/ai/provider_cloud_test.go) | [nimillm.ts](../examples/providers/nimillm.ts) |
...
go test ./internal/services/ai -run TestCloudProviderNimiLLMAllModalities -count=1
...
NIMI_LIVE_NIMILLM_BASE_URL=https://your-nimillm-endpoint \
NIMI_LIVE_NIMILLM_API_KEY=sk-xxx \
NIMI_LIVE_NIMILLM_MODEL_ID=nimillm/gpt-4o-mini \
go test ./internal/services/ai -run TestLiveSmokeNimiLLMGenerateText -count=1 -v
```
