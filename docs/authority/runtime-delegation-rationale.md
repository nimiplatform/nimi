...
> 本文为 rationale/历史散文,非规范权威;规范 = `.nimi/spec/runtime/delegation.authority.yaml`。下文保留的 `Contract`、`Authority`、`MUST` 和旧 Rule ID 标题均为历史标签，不能覆盖或扩展 canonical authority。
...
MCP is also deferred and unavailable; neither protocol has a current delegated
adapter, transport, execution path, or product claim.
...
session, request, and result semantics. It does not activate a protocol
adapter. MCP and A2A remain unavailable future seams, and protocol wire objects
...
A provider kind is usable only after a separate Runtime capability owner admits
its identity, lifecycle, authorization, credential custody, and audit contract.
MCP and A2A kinds remain inactive and unavailable; a registry or type entry
cannot activate either protocol.
...
MCP and A2A have no current production consumer, adapter, or execution path.
Desktop and Avatar may consume only separately admitted typed Runtime
delegation projections; consumer presence cannot activate a protocol.

---

# Deferred MCP Adapter Boundary

The former production MCP adapter blueprint is retired. MCP has no current
Runtime product contract, transport, server lifecycle, tool registry, execution
path, SDK surface, or readiness role.

MCP provider kinds remain inactive and unavailable. Registry, type, generated
code, documentation, or fixture presence cannot activate them. A future MCP
design requires separate product-owner and security authority before any
dependency, Runtime registration, transport, tool surface, consumer API, or
success fixture is introduced.

Runtime-owned delegation firewall, approval, audit, and replay semantics remain
protocol-neutral. They do not imply current MCP support.
