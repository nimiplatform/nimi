...
Nimi Coding 将 AI 辅助实现视为一项承载权威的工作。我们的核心主张非常明确：在复杂项目中应用 AI 辅助编程，只有在正式接受产出之前，明确界定权威来源、影响范围、执行工作包 (Packet)、审计关口以及收尾标准，才能取得成功。

本白皮书旨在论证这一主张，并详细阐述该方法论旨在应对的各种失败模式。

## 传统 AI 编程模式的局限性

对于小规模的代码修改，单一的 AI 助手可以读取少量文件、修改代码、运行测试并完成任务。然而，当面对复杂的项目时，这种模式往往会失效。AI 助手可能会：
...
在 AI 助手的单一执行循环内部，这些问题很难被察觉。在当时的情境下，每一个步骤看起来都像是正确的。
