...
本页提供 10 个已准入反模式 (Admitted catalog keys) 的字段级技术参考。如需了解概念层面的叙述性解释，请参阅 [禁用反模式](/zh/nimicoding/forbidden-shortcuts)。
...
| `legacy_alias` | 严禁通过软别名 (Soft alias) 维持过时语义的存活状态 |
...
| `silent_owner_cut_reopen` | 严禁在执行下游处理的波次 (Wave) 内部，静默修改或重开所有权域内的核心真相 |
...
| 标准替代方案 | 优先设计具有完整语义的系统契约；分层应基于业务本体 (Ontology) 而非演进时间 (Chronology)。 |
...
| 标准替代方案 | 实施彻底切断 (Hard cut)；或经由显式准入，设定包含弃用时间线的双轨过渡契约。 |
...
| 评估属性 | 参数说明 |
| --- | --- |
| 触发条件 | 类型化契约在处理失败时，通过返回缺省或无意义的回退数据 (Fallback) 进行错误掩盖。 |
...
| 失效后果 | 使所有权域内的核心修改掩藏于具体的业务执行中；导致审计追溯链路 (Lineage) 断裂。 |
...
| 防护机制 | 核心检测目标 |
| --- | --- |
| 常规审计流程 (Audit) | 对代码及文档输出进行深度的模式分析及识别。 |
