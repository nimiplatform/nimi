...
> 状态：现已可用。具身化语义层是 Avatar 正在运行的后端无关核心层，负责把 Agent 语义翻译成后端中立的提示。

具身化语义层是一组**宿主无关**的 API。它从 Agent 的真实状态出发，向下产出某个具体后端的渲染调用。NAS 处理器调用这层接口，由它再翻译成 Live2D（当前）或 VRM（下一步）需要的指令。
