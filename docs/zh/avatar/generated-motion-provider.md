...
> 状态：现已可用。提供器契约是 APML 驱动动作的运行时证明路径；能力配置和映射 sidecar 定义了每个后端能做什么。

生成式动作提供器把 Runtime 强类型的动作、表情、姿态、注视和活动意图，转换成可以直接执行的后端输出——不依赖物理 `.vrma` 文件或其它手工制作的素材。正是它让 Agent 的动作"活"起来：动作在运行时生成，而不是回放一份预先做好的资产。
