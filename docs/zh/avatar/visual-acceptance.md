...
Avatar 载体视觉验收契约已在内核层级获得准入。证据分类体系、故障即关闭边界以及多后端证据形态均已确定。自动化验收工具正在积极建设中；目前并非所有证据类别都已交付。
...
故障即关闭的姿态是刻意的。没有已准入证据的成功声明被视为 `placeholder_success` 捷径，并将被拒绝。
