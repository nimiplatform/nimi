...
Nimi 的本地路由是**引擎优先**。你不用先挑一个模型再祈祷有东西能跑它；你先挑一个引擎，引擎再把模型解析成可跑的包。
...
这正好是"模型优先"的反面——那种先挑名字再祈祷兼容的玩法。引擎优先意味着兼容决定归引擎所有。
...
需要 CUDA 的引擎走基于 materializer 的安装路径，分阶段明确：
