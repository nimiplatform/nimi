...
Runtime 是实际承担 AI 任务执行的底层架构。它负责将平台层面的意图转化为受严格治理的执行操作。这其中包括管理提供商（Provider）、模型目录、流式传输、工作流、本地路由策略、权限委派、行为审计、多模态产物，以及由 Runtime 直接维护的 Agent 参与逻辑。
...
5. 最终，工作流流转至 `COMPLETED` 状态，底层审计账本记录同步落盘。
...
1. 本地能力注册表向 Runtime 报告该设备已授权开放的能力类别。
2. 设备硬件画像模块向 Runtime 陈述该设备当前的算力边界与硬件支撑条件。
