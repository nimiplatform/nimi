...
配置中的提供商名称必须使用 `provider-catalog.yaml` 中的规范值。别名和旧名称将被**拒绝**。这防止了两个配置以不同方式拼写同一个提供商并静默路由到不同路径的情况。
...
配置写入使用 **临时文件 + 重命名** 原子写入。崩溃的写入不会留下半写入的配置；要么新的配置原子性地落地，要么旧的配置仍然在位。
...
1. **用户运行 `config init`。** 运行时写入一个新的 `~/.nimi/config.json`，其中包含 `schemaVersion: 1` 和内置默认值。
