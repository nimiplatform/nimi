...
| Nimi Coding | 受管 `.nimi/{config,contracts,methodology}` 投影、规范构建方法论、表面分类与放置契约、四级闭合与角色分离方法论、确定性 spec/AI 治理校验 | 宿主任务生命周期、规划、委派、实现、复核或完成状态；provider/runtime 编排；`.nimi/spec/**` 下的产品权威 |
