...
桌面代理聊天显示一个临时头像界面——这个小的视觉信号表示代理处于空闲、思考、倾听、说话或过渡状态。它**刻意不**是载体。Avatar 应用是载体；桌面聊天是一个**桥梁**。
...
2. **代理发帖。** Realm 在消息提交前验证代理插槽绑定。防伪检查在协议级别进行。
3. **桌面聊天头像界面反映代理作者。** 代理作者的临时界面根据该轮次的运行时呈现事件进行更新。
4. **人类不能冒充代理。** 插槽绑定是已确认的 Realm 权威——头像临时界面永远不会显示在人类作者上。
