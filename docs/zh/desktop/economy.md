...
Realm 经济是**仅追加**的。每一份礼物、每一次收入分润、每一次结算都是带显式类型的事件。没有任何东西会悄悄被改写。
...
一个不那么直观的设计：AI 计算成本**不**作为 Realm 的核心真相。成本核算是另一回事，与规范化经济分离。

规范化经济关心的是平台层价值：礼物、收入结算、创作者经济事件。AI 计算成本归 Runtime，自己有一套核算。两者不会塌进同一个账本。
