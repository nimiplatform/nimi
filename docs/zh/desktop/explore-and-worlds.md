...
世界详情消费一个有界的 `WorldDisplayDetail` 接口。多份原始读取（truth、world-state、world-history、呈现）汇聚成一份呈现权威。读者看到一份连贯的世界页；桌面端消费这层接口，不直接动 Realm 的原始契约。

## Agent 详情

用户进入某个 Agent 后，Agent 详情通过有界的 `AgentDisplayDetail` 接口展示其公开资料。
