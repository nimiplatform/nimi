...
Tab 是真实的隔离边界。Mod 在 Tab 内崩溃不会拖垮桌面端，断路器把 Tab 熔断。
...
开发者用 Codegen 起一个 Mod 时，拿到的是已经符合准入能力级别和 Hook 能力白名单的强类型骨架。这一点让「写一个新 Mod」不必要求深层的平台知识。
...
一个有 bug 的 Mod 不会拖垮桌面端。熔断就是隔离边界。
