...
已在内核级别被准入。面向用户的审批+隔离UI作为方向被准入；完整的表面正在积极构建中。

## 什么是委托控制

桌面委托控制表面是**面向用户的控制平面**，用于委托能力——用户在此看到外部AI建议的审批提示，审查隔离的证据，批准或拒绝委托的操作。

运行时方面（网关+输出防火墙+裁决）位于[运行时→委托能力](/runtime/delegated-capability)。此页面涵盖将这些裁决呈现给用户的桌面控制表面。

## 边界

| 拥有 | 不拥有 |
| --- | --- |
| 审批提示渲染+决策捕获 | 防火墙裁决（运行时） |
| 隔离证据显示 | 隔离语义（运行时） |
| 面向用户的理由文本 | 理由代码语义（运行时） |
| 每个用户的审批默认策略偏好 | 信任层级准入（运行时） |

控制表面负责渲染和决策捕获。裁决和隔离逻辑属于运行时权限。

## 读者场景：审批提示

外部AI提议一个工具调用；防火墙裁决为`APPROVAL_REQUIRED`。

1. **运行时发出需要审批的事件。** 包含类型化的委托请求、防火墙裁决、敏感度和建议操作。
2. **桌面控制表面进行渲染。** 审批卡片显示外部AI建议的内容、为什么需要审批以及用户的批准/拒绝选项。
3. **用户决定。** 批准或拒绝。记录原因。
4. **运行时根据用户决定采取行动。** 记录针对委托会话的审批；如果批准，运行时在其自身的审计谱系下采取行动。
...
- 它不会默默地改变隔离语义。
- 它不允许绕过审批的快捷方式。
- 它不允许用户偏好策略覆盖运行时准入的审批要求（偏好设置在准入的策略范围内）。
