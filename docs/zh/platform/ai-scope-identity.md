...
(`P-AISC-001..P-AISC-005`)。第一阶段模组范围的 AI 配置目前正在运行；
其余范围类型（应用、模块、特性）已作为方向获得准入，其消费端界面正在积极构建中。
...
`kind` 枚举是**封闭的**。应用、模组和消费者不能发明第五种类型。
...
1.  **清单身份。** 模组附带一个稳定的清单 ID（例如 `com.example.notes`）。
2.  **范围身份。** 桌面端将模组的 AI 配置桥接到 `AIScopeRef{ kind: 'mod', ownerId: 'com.example.notes', surfaceId: 'workspace' }` 下。
...
1.  **显式范围扩展。** 模组准入多个 `AIScopeRef{ kind: 'mod', ownerId: <清单>, surfaceId: <稳定的工作区名称> }` 实例。
...
1.  **所有者实体被销毁。** 模组清单被移除。
2.  **范围被销毁。** 所有与该模组绑定的 `AIScopeRef` 实例同步销毁。
