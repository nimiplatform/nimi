...
Cloud AI 消费、路由选择、readiness、预算、Credential 与 App authorization。
...
2. Runtime 从 active session 推导 account、App identity、authorization 与
   LocalAgent access。
3. Runtime 解析已准入 AI route、readiness、Quota 与 Budget。
4. Runtime 在提交 Conversation 或其他 LocalAgent 真相前校验 model output。
...
不得阻塞普通 LocalAgent Conversation、Memory、Knowledge、voice 或 readiness。
