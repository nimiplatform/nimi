...
> 状态：运行中 (Running)。认知的 `MemoryService` 和 `KnowledgeService` 通过 `C-COG-*` 提供；运行时桥接契约是已准入的消费路径。

Cognition 包含**两个不同的服务**，它们共同承载了代理对用户和事实的持续理解。两者看似相似，底层行为却截然不同。
...
桥接是**消费**，不是吞并。Runtime 不能重新定义记忆 bank 是什么或知识是如何策划的。Cognition 不依赖 Runtime 来成为自己。
