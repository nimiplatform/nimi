...
已准入的参考矩阵和 refgraph 解释边界 (`C-COG-047..C-COG-049`) 已被接受。跨家族引用工具和清理提案 UI 正在积极构建中。
...
这条规则防止了权限倒置：一个咨询工件存储到内核的反向引用不会提升其自身为内核所有权。
...
| 移除阻塞 | 区分强弱传入支持；不要将两者扁平化为一个通用阻塞字符串 |
| 已移除的源 | 不贡献实时支持 |
| 已移除的目标 | 作为损坏依赖证据保持可见 |
...
3. **权限保留。** 咨询工件不会因为存储反向引用而成为内核的所有者。

## 读者场景：知识条目变得陈旧

维护者将一个条目标记为已移除。内存曾引用过它。

1. **条目移除。** 知识条目状态：已移除。
2. **内存的传出引用现在损坏。** 参考图反映：目标已移除。
3. **损坏依赖证据。** 内存的历史记录显示引用指向了一个现已移除的目标；内存不会被静默重写。
4. **清理提案。** 显示损坏的依赖以进行显式处理。

边界防止“知识改变”静默地修改“内存曾经说过的内容”。

## 参考图不做的事情

- 它不吸收运行时审查、复制、别名或提供者排名语义。
- 它不允许咨询工件通过反向引用声称内核所有权。
- 它不将强弱支持扁平化为一个单一的阻塞字符串。
- 它不允许已移除的源贡献实时支持。
- 它不允许同一轮次的归档和移除。
