...
可运行的投影表面已被准入为方向；运行时侧的 WEE 引擎本身也被准入为平台方向（参见 [WEE 执行](/platform/worlds/wee-execution)）。
...
| 拥有 | 不拥有 |
| --- | --- |
| WEE 状态的类型化投影 API | WEE 引擎语义（运行时 `K-WEV-*`） |
| 易于阅读的投影形状 | 领域规范真相（领域） |
| 锚定范围的读取表面 | 运行时本地执行证据定义 |

WEE 状态是**运行时本地执行证据**，而不是领域的规范真相。投影使这些证据可被消费，但不会将其提升为领域的权威。

## 读取场景：应用程序显示 WEE 阶段进度
