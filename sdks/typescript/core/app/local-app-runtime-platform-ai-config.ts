...
function projectCloudTargetOption(value: unknown, index: number): void {
  ...
  if (option && Object.hasOwn(option, 'musicInput')) {
    if (!['music.generate', 'music.transcribe', 'audio.voice.convert'].includes(String(option.capabilityContract))) localAppProjectionError('music input capability contract');
