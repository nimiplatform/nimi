...
export type NimiLocalAppConversationWork = {
...
export type NimiLocalAppConversationToolCall = {
  readonly callId: string; readonly turnId: string; readonly name: string; readonly argumentsJson: string;
};
...
export function validateConversationWork(value: unknown): NimiLocalAppConversationWork {
  ...
  const result = {
    ...
    tools: work.tools.map((value) => {
      ...
      return { name, description: workText(tool?.description, 2048), inputSchemaJson };
    }),
  };
