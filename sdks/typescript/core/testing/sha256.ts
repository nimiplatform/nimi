...
export function sha256Ascii(input: string): string {
  if (!/^[\x20-\x7e]*$/u.test(input)) {
    throw new TypeError('SHA-256 trace input must be printable ASCII');
  }
  const bytes = new Uint8Array(input.length);
  for (let index = 0; index < input.length; index += 1) bytes[index] = input.charCodeAt(index);
  return hexEncode(sha256Bytes(bytes));
}
...
function sha256Bytes(data: Uint8Array): Uint8Array {
  ...
  for (let index = 0; index < hash.length; index += 1) output.setUint32(index * 4, hash[index]);
  return digest;
}
