...
function writeGeneratedFiles({
  ...
  const generatedLogos = Object.entries(moduleLogos).map(([moduleId, assetPath]) => (
    `  ${JSON.stringify(moduleId)}: new URL(${JSON.stringify(assetPath)}, import.meta.url).href,`
  ));
  writeFileSync(path.join(generatedRoot, 'app-logos.ts'), [
    '// Generated by apps/simulator/build/registry.mjs. Do not edit.',
    'export const simulatorSelectedModuleLogos = {',
    ...generatedLogos,
    '} as const;',
    '',
  ].join('\n'));
...
export function generateSelectedModuleRegistry({
  ...
  const styleInputs = {};
  const moduleLogos = {};
  mkdirSync(path.join(generatedRoot, 'app-assets'), { recursive: true });
  ...
  for (const [orderingKey, descriptor] of sorted.entries()) {
    ...
    const iconPath = path.resolve(appRoot, ...descriptor.icon.split('/'));
    if (!iconPath.startsWith(`${appRoot}${path.sep}`) || !existsSync(iconPath)) {
      fail('SIM_SELECTED_ICON', 'selected App icon must stay inside the selected App root and exist', descriptor.icon);
    }
    const iconStat = lstatSync(iconPath);
    if (!iconStat.isFile() || iconStat.size < 1 || iconStat.size > 4 * 1024 * 1024) {
      fail('SIM_SELECTED_ICON', 'selected App icon must be a regular image file no larger than 4 MiB', descriptor.icon);
    }
    const iconName = `${descriptor.module_id}${path.extname(descriptor.icon).toLowerCase()}`;
    copyFileSync(iconPath, path.join(generatedRoot, 'app-assets', iconName));
    moduleLogos[descriptor.module_id] = `./app-assets/${iconName}`;
