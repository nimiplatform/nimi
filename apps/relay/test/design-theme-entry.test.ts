...
describe('relay design theme entry contract', () => {
  it('uses the shared Nimi theme provider with the relay accent pack and light scheme', () => {
    ...
    assert.doesNotMatch(mainSource, /accentPack="desktop-accent"/u);
  });
  ...
  it('imports the shared foundation and relay accent styles', () => {
    ...
    assert.doesNotMatch(stylesSource, /@nimiplatform\/nimi-kit\/ui\/themes\/desktop-accent\.css/u);
