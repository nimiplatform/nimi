...
async function loadRemint() {
  const output = buildSync({
    entryPoints: [path.join(root, 'src/shell/agent/conversation-selection-remint.ts')],
    bundle: true,
    format: 'esm',
    platform: 'node',
    target: 'node22',
...
test('remints a rotated handle only by exact durable Conversation anchor proof', async () => {
  const { remintZhiyuConversationSelection } = await loadRemint();
  const calls = [];
  const result = await remintZhiyuConversationSelection({
...
test('does not hide an owner or transport failure behind selector mismatch', async () => {
  ...
  await assert.rejects(() => remintZhiyuConversationSelection({
    previousConversationAnchorId: 'anchor-durable-1',
    currentReferences: [reference('agent_ref_current_a'), reference('agent_ref_current_b')],
    conversation: {
      async snapshot(input) {
