...
const zhiyuRoot = path.resolve(__dirname, '..');
const rendererPort = 1472;
...
function isZhiyuRendererProcess(commandLine) {
  const normalized = normalizeForMatch(commandLine);
  const normalizedZhiyuRoot = normalizeForMatch(zhiyuRoot);
  return normalized.includes(normalizedZhiyuRoot)
    && normalized.includes('vite')
    && normalized.includes('--port 1472');
}
...
function ensureRendererPortAvailable() {
  ...
  for (const pid of pids) {
    ...
    if (!isZhiyuRendererProcess(commandLine)) {
      throw new Error(
        `Port ${rendererPort} is already in use by PID ${pid}. ` +
        'It is not a recognized zhiyu renderer process, so cleanup was skipped.',
      );
    }

    console.log(`[dev-renderer-port] Stopping stale zhiyu renderer on port ${rendererPort} (PID ${pid}).`);
    process.kill(pid);
  }
}
