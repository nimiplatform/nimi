...
const MODEL_CONFIG_COPY: Record<string, string> = {
  ...
  'Zhiyu.aiConfig.subtitle': '通过 Runtime/SDK 选择织愈的模型路由',
  'Zhiyu.aiConfig.close': '关闭模型配置',
  'ModelConfig.section.chat.title': '对话',
  'ModelConfig.section.chat.subtitle': '文本与流式对话',
  'ModelConfig.section.chat.detail': '用于本地 Agent 对话、文本生成和 chat.stream。',
  ...
  'ModelConfig.section.image.subtitle': '图像生成',
  'ModelConfig.section.image.detail': '用于 Runtime-owned image.generate 场景任务。',
  'ModelConfig.capability.textGenerate.title': 'Text generate',
  'ModelConfig.capability.textGenerate.subtitle': 'Runtime text route',
  'ModelConfig.capability.textGenerate.detail': '织愈对话与 chat.stream 共用此 Runtime 文本绑定。',
  'ModelConfig.capability.textEmbed.title': 'Text embed',
  'ModelConfig.capability.textEmbed.subtitle': 'Runtime embedding route',
  'ModelConfig.capability.textEmbed.detail': '为记忆与相似度召回选择嵌入模型。',
  'ModelConfig.capability.imageGenerate.title': 'Image generate',
  'ModelConfig.capability.imageGenerate.subtitle': 'Runtime image route',
  'ModelConfig.capability.imageGenerate.detail': '为后续图像生成与 artifact 面板选择模型。',
  ...
  'ModelConfig.editor.common.seedHint': '留空则由 Runtime 决定。',
  ...
  'ModelConfig.editor.image.customOptionsHint': '每行一个 Runtime 参数。',
...
function bindingStatus(
  ...
  if (!runtimeReady) {
    return {
      ...
      title: 'Runtime 不可用',
      detail: runtimeDetail || '等待外部 Runtime daemon。',
    };
  }
  ...
  if (!targetRef) {
    return {
      ...
      title: '需要模型目标',
      detail: `运行 ${capabilityId} 前必须先绑定 Runtime targetRef。`,
    };
  }
