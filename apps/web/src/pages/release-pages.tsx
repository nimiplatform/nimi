...
const EN_COPY: PublicPageCopy = {
  ...
  download: {
    ...
    statusBody:
      'No stable Nimi GitHub Release has been published. Accepted unsigned previews use explicit immutable vX.Y.Z-preview.N GitHub prereleases and never enter the stable latest path. Signed RC and Stable remain blocked until production platform signing is available.',
    releaseAction: 'View GitHub Releases and unsigned previews',
    release: {
      ...
      paragraphs: [
        ...
        'No platform currently has an official stable Nimi download. An explicit unsigned developer preview may be downloaded from GitHub Releases after its scoped checks pass; it contains only the artifacts listed below and is not a standalone product installer.',
      ],
    },
...
const ZH_COPY: PublicPageCopy = {
  ...
  download: {
    ...
    statusBody:
      'GitHub 上还没有 Nimi 稳定版 Release。通过验收的 unsigned preview 只使用不可变的 vX.Y.Z-preview.N GitHub prerelease，不进入 stable latest；production 平台签名可用前，signed RC 与 Stable 继续 fail closed。',
    releaseAction: '查看 GitHub Releases 与 unsigned preview',
    release: {
      title: '稳定版状态',
      paragraphs: [
        ...
        '目前没有任何平台提供正式稳定版下载。范围内检查通过后，可从 GitHub Releases 下载显式 unsigned developer preview；它只包含下方列出的制品，不是独立产品 installer。',
      ],
    },
    platformTitle: '平台可用性',
    platforms: [
...
export function DownloadPage() {
  ...
  return (
    <PageShell
      ...
      action={<a className="release-primary-action" href={RELEASES_URL}>{page.releaseAction}</a>}
    >
      <PageSection title={page.release.title}>
        <TextSectionContent section={page.release} />
        <p><a href={RELEASES_URL}>GitHub Releases</a></p>
      </PageSection>
