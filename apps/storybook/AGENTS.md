...
- The app-owned area is `src/shell/routes/product-area.tsx`, `src/storybook/**` (engine, ai boundary, store, memory, and Play/Studio UI), and the Storybook contract tests under `test/**`.
- Storybook Engine (`src/storybook/engine/**`) is the app-owned rule-of-truth authority. The `storybook-truth-package` is canonical; Studio/Play/render are projections with governing truth refs. Do not let projections, renderers, or UI become a second truth source.
- AI execution flows only through `src/storybook/ai/**`, which calls admitted `runtime.ai.*` / `runtime.media.*` via an `AIConfig` binding. No provider/model hardcoding, no app-local provider routing, no fabricated output — missing binding/runtime fails closed to a typed unavailable state.
- Storybook memory/run/transcript/feedback data is app-internal and project-scoped only. Realm is stage-one structural reference (no mutation/marketplace/legacy compat); Forge and the external narrative-engine are design references, not imports.
