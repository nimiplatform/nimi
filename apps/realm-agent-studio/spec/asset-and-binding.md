...
| --- | --- | --- | --- |
| Avatar image | Prefer `Resource(IMAGE)` or `OwnableAsset` plus `Binding(PRESENTATION, hostType=AGENT, bindingPoint=AGENT_AVATAR)`. | Binding enum and upsert DTO support `AGENT_AVATAR`, `hostType=AGENT`, `objectType=RESOURCE/ASSET/BUNDLE` (`sdk/src/realm/generated/schema.ts:4212` to `:4289`; `:13298` to `:13320`). | Partially pinned. Raw `UpdateCreatorAgentDto.avatarUrl` exists but is insufficient as public asset authority without a chosen Resource/Binding/profile rule. |
...
| Profile cover | Realm `profileCoverUrl` profile attribute. | `UpdateCreatorAgentDto.profileCoverUrl` plus `UserLiteDto`, `UserPrivateDto`, and `UserProfileDto` read projections exist (`sdk/src/realm/generated/schema.ts:6549` to `:6558`; `:6769` to `:6786`; `:6795` to `:6825`; `:6832` to `:6860`). | Profile cover attribute is admitted. Binding enum still does not expose separate agent banner/background/cover binding points; world/scene binding points must not be reused by analogy. |
