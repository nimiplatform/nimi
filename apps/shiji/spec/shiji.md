...
1. **监护人输入层** — 家长/监护人在受保护入口录入孩子的年龄、兴趣、擅长领域、表达方式、学习目标与注意事项
...
3. **地图视图** 是 Phase 2 的候选能力，但当前 `map-profile` 仍为 proposed，因此不属于稳定 Explore 第二视图
4. **地图不替代时间线**，只在 typed map surface 获批后补充地点、路径、事件锚点和时空联动理解
...
| App state | `src/shell/renderer/app-shell/app-store.ts` |
...
| Scene | 场景地点（未来 typed surface） | 长安大明宫、洛阳 |
