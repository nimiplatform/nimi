    ...
    <section className={S.radius + ' mb-6 p-5'} style={{ background: S.card, boxShadow: S.shadow }}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-[16px] font-semibold" style={{ color: S.text }}>智能识别分析</h2>
            </div>
            <button
              onClick={onRefresh}
              disabled={aiLoading}
                ...
                return (
                  <div key={index} className={S.radiusSm + ' px-4 py-3'} style={{ background: style.bg, border: `1px solid ${style.border}` }}>
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span className="text-[14px] font-semibold" style={{ color: S.text }}>{alert.title}</span>
                    </div>
                    <p className="text-[13px] ml-6" style={{ color: S.sub }}>{alert.message}</p>
                  </div>
                );
              })}
            </div>
          ...
          {analysis.diagnoses.length > 0 ? (
            <div className="mb-4">
              <h3 className="text-[14px] font-semibold mb-2" style={{ color: S.text }}>诊断汇总</h3>
              <div className="flex flex-wrap gap-1.5">
                {analysis.diagnoses.slice(0, 12).map((diagnosis) => (
                  <button
                    key={diagnosis.diagnosis}
                    onClick={() => onSelectDiagnosis(diagnosis.diagnosis)}
                    className={S.radiusSm + ' text-[13px] px-2.5 py-1 transition-colors hover:opacity-80'}
          ...
          {analysis.medications.length > 0 ? (
            <div className="mb-4">
              <h3 className="text-[14px] font-semibold mb-2" style={{ color: S.text }}>用药汇总</h3>
              <div className="flex flex-wrap gap-1.5">
          ...
          <div className="flex gap-3 flex-wrap mb-4">
            {Object.entries(analysis.eventsByType).map(([type, count]) => (
              <div key={type} className="text-[13px] flex items-center gap-1" style={{ color: S.sub }}>
          ...
          {aiLoading && !aiInsight ? (
            ...
            <div className={S.radiusSm + ' p-3'} style={{ background: '#f9faf7', border: `1px solid ${S.border}` }}>
              <div className="flex items-center gap-1.5 mb-1.5">
                ...
                <span className="text-[13px] font-semibold" style={{ color: S.text }}>AI 综合分析</span>
              </div>
              <p className="text-[14px] leading-relaxed" style={{ color: S.text }}>{aiInsight}</p>
            </div>
          ) : null}
    </section>
  );
