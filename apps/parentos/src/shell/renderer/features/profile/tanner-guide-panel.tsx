        ...
        <div className="px-7 pb-3 space-y-2">
          <div>
            ...
            {detail.steps.map((step, index) => (
              <p key={index} className="text-[10px] leading-relaxed pl-3 relative" style={{ color: S.sub }}>
  ...
  return (
    <div className={`${S.radius} mt-6 overflow-hidden`} style={{ boxShadow: S.shadow }}>
      <button onClick={() => setExpanded(!expanded)} className="w-full flex items-center justify-between px-5 py-4 text-left" style={{ background: 'linear-gradient(135deg, #5a7a5a, #7a9a6a)' }}>
        <div>
          <h3 className="text-[14px] font-bold text-white">{latestBG ? '当前阶段发育指导' : '发育指导参考'}</h3>
          <p className="text-[11px] text-white/60 mt-0.5">{guidance.title} · 基于{latestBG ? '最新评估结果' : '青春前期'} · 点击每条建议查看详细指导</p>
        </div>
          ...
          {buildGuidanceSections(guidance).map((section) => (
            <div key={section.title}>
              <div className="flex items-center gap-2 mb-2">
                ...
                <h4 className="text-[12px] font-semibold" style={{ color: S.text }}>{section.title}</h4>
              </div>
