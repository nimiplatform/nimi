...
export default function TannerPage() {
  ...
  return (
    <div className={S.container} style={{ paddingTop: S.topPad, minHeight: '100%' }}>
      ...
      <div className="flex items-center justify-between mb-1">
        <div className="flex items-center gap-2">
          ...
          <div className="group relative">
            ...
            <div className="pointer-events-none absolute left-0 top-7 z-50 w-[320px] rounded-xl p-4 text-[13px] leading-relaxed opacity-0 transition-opacity duration-200 group-hover:pointer-events-auto group-hover:opacity-100"
              style={{ background: '#1e293b', color: '#e0e4e8', boxShadow: '0 8px 24px rgba(0,0,0,0.2)' }}>
              <p className="text-[14px] font-semibold text-white mb-2">参考标准</p>
              <ul className="space-y-2">
                <li>
                  <span className="text-[#4ECCA3] font-medium">Tanner 分期标准</span>
                  <span className="block text-[12px] text-[#a0a8b4] mt-0.5">Marshall WA, Tanner JM. Variations in pattern of pubertal changes in girls/boys.</span>
                  <span className="block text-[12px] text-[#7a8090]">Arch Dis Child 1969;44:291-303 / 1970;45:13-23</span>
                </li>
                <li>
                  <span className="text-[#4ECCA3] font-medium">中国儿童青春期参考</span>
                  <span className="block text-[12px] text-[#a0a8b4] mt-0.5">中华医学会儿科学分会内分泌遗传代谢学组. 中枢性性早熟诊断与治疗专家共识（2022）</span>
                </li>
              </ul>
              <p className="text-[12px] mt-2 pt-2 border-t border-white/10 text-[#808890]">女孩 B2 通常 8-13 岁出现 · 男孩 G2 通常 9-14 岁出现</p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
      ...
      {showGuide && (
        <div className={`${S.radius} mb-5 overflow-hidden`} style={{ boxShadow: S.shadow }}>
          <div className="px-5 py-4" style={{ background: 'linear-gradient(135deg, #4a6a8a, #6a8ab0)' }}>
            <h3 className="text-[16px] font-bold text-white mb-1">什么是 Tanner 分期？</h3>
            <p className="text-[13px] text-white/70">Tanner 分期是国际通用的青春期发育评估标准，将{isFemale ? '乳房' : '外生殖器'}和阴毛发育各分为 5 期。</p>
          </div>
          <div className="p-5 space-y-4" style={{ background: S.card }}>
            <div>
              <h4 className="text-[14px] font-semibold mb-1" style={{ color: S.text }}>如何判断？</h4>
              <p className="text-[13px] leading-relaxed" style={{ color: S.sub }}>
                {isFemale
            ...
            <div>
              <h4 className="text-[14px] font-semibold mb-1" style={{ color: S.text }}>正常发育时间参考</h4>
              <div className="grid grid-cols-2 gap-2">
                <div className={`${S.radiusSm} p-3`} style={{ background: '#f8faf9', border: `1px solid ${S.border}` }}>
                  <p className="text-[13px] font-semibold" style={{ color: S.text }}>{isFemale ? '乳房发育 B2 出现' : '睾丸增大 G2 出现'}</p>
                  <p className="text-[12px]" style={{ color: S.sub }}>{isFemale ? '正常: 8-13 岁 · 平均 10.5 岁' : '正常: 9-14 岁 · 平均 11.5 岁'}</p>
                  <p className="text-[12px] mt-1" style={{ color: '#dc2626' }}>{isFemale ? '<8 岁出现需排查性早熟' : '<9 岁出现需排查性早熟'}</p>
                </div>
                <div className={`${S.radiusSm} p-3`} style={{ background: '#f8faf9', border: `1px solid ${S.border}` }}>
                  <p className="text-[13px] font-semibold" style={{ color: S.text }}>阴毛 PH2 出现</p>
                  <p className="text-[12px]" style={{ color: S.sub }}>{isFemale ? '通常在 B2 后 6-12 个月出现' : '通常与 G2 同时或稍后出现'}</p>
                  <p className="text-[12px] mt-1" style={{ color: S.sub }}>单纯阴毛早现可能为肾上腺功能早现</p>
                </div>
              </div>
            </div>
            <div>
              <h4 className="text-[14px] font-semibold mb-1" style={{ color: S.text }}>何时需要就医？</h4>
              <ul className="text-[13px] leading-relaxed space-y-0.5" style={{ color: S.sub }}>
          ...
          <div className="px-5 py-3 flex justify-end" style={{ background: '#f8faf9', borderTop: `1px solid ${S.border}` }}>
            <button onClick={() => setShowGuide(false)} className={`px-4 py-1.5 text-[14px] font-medium text-white ${S.radiusSm}`} style={{ background: S.accent }}>我知道了</button>
          </div>
        </div>
      )}
      ...
      <h2 className="text-[14px] font-semibold mb-3 mt-6" style={{ color: S.text }}>
