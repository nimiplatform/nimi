        ...
        <div className="relative">
          ...
          {grouped.map((group) => (
            <div key={group.monthLabel} className="relative pl-10 pb-6">
              ...
              {/* Month label */}
              <p className="text-[14px] font-bold mb-3" style={{ color: S.text }}>{group.monthLabel}</p>
                    ...
                    <div key={report.reportId}
                      ...
                      {isExpanded && data && (
                        <div className="px-4 pb-4" style={{ borderTop: `1px solid ${S.border}` }}>
                          <p className="text-[12px] py-2" style={{ color: S.sub }}>
                            ...
                            {data.measurements.map((m, i) => {
                              ...
                              return (
                                <div key={i} className={`flex items-center gap-2.5 p-2.5 ${S.radiusSm}`}
                                  ...
                                  <span className="text-[14px] flex-1" style={{ color: S.text }}>{info.name}</span>
                                  <span className="text-[14px] font-bold" style={{ color: S.text }}>{m.value}</span>
                                  <span className="text-[12px] w-12" style={{ color: S.sub }}>{info.unit}</span>
                                  <span className="text-[12px]" style={{ color: S.sub }}>{m.measuredAt}</span>
                                </div>
                              );
                            })}
                          </div>
                          {data.measurements.some((m) => m.notes) && (
                            <div className="mt-2">
                              {data.measurements.filter((m) => m.notes).map((m, i) => (
                                <p key={i} className="text-[12px]" style={{ color: S.sub }}>📝 {m.notes}</p>
                              ))}
                            </div>
                          )}
                        </div>
