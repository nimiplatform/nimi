...
Nimi Avatar 当前正常启动路径已经采用 real runtime/SDK consume chain。Mock scenario 仍保留，但只作为显式 dev/test fixture，用于：

- fixture-driven UI/NAS regression evidence
- bounded local testing without silent fallback
- integration corpus for runtime consume parity checks
...
- Fixture activation rule（显式 `VITE_AVATAR_DRIVER=mock`）

本 contract 不定义正常产品启动路径；它只定义 fixture surface。
...
│
└── sdk/                       # Real runtime/SDK primary adapter
...
App-level 代码只依赖 `AgentDataDriver`。当前默认 factory 必须返回 `SdkDriver`；mock 只在显式 fixture mode 启用：
...
**Boundary rule**: `src/shell/renderer/app-shell/` / `nas/` / `live2d/` **只 import `driver/types.ts` + `driver/factory.ts`**，不直接 import `mock/` 或 `sdk/`。Mock activation must stay explicit and local.
...
- Normal app boot uses `SdkDriver`
- Normal app data / events consume runtime presentation/state/turn surfaces
- runtime/bootstrap failure must fail closed; no silent downgrade to mock

### 6.2 Explicit Fixture Override

dev/test build 可通过环境变量切回 Mock：

```
VITE_AVATAR_DRIVER=mock    # force mock driver
VITE_AVATAR_DRIVER=sdk     # force SDK (default current path)
...
Mock driver 已降级为 dev/test 工具；scenario files 保留作 integration test corpus 永久留存于 `src/shell/renderer/mock/scenarios/`。
