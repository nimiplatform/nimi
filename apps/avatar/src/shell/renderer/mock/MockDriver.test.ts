...
describe('MockDriver', () => {
  ...
  it('emits time-based events at the scheduled offset', async () => {
    const scenario = makeScenario({
      events: [
        { kind: 'time', at_ms: 1000, type: 'avatar.fixture.presentation.activity_requested', detail: { activity_name: 'happy', category: 'emotion', intensity: 'moderate', source: 'mock' } },
        { kind: 'time', at_ms: 2500, type: 'avatar.fixture.presentation.activity_requested', detail: { activity_name: 'sad', category: 'emotion', intensity: 'moderate', source: 'mock' } },
      ],
    });
    ...
    expect(names).toEqual(['avatar.fixture.presentation.activity_requested:happy']);
    vi.advanceTimersByTime(1500);
    expect(names).toEqual(['avatar.fixture.presentation.activity_requested:happy', 'avatar.fixture.presentation.activity_requested:sad']);
  });

  it('updates bundle activity when a avatar.fixture.presentation.activity_requested event fires', async () => {
    const scenario = makeScenario({
      events: [
  ...
  it('fires reactive trigger when matching upstream app event is emitted', async () => {
    ...
    driver.onEvent((ev) => {
      if (ev.name === 'avatar.fixture.presentation.activity_requested') {
  ...
  it('does not fire trigger if filter does not match', async () => {
      ...
      triggers: [
        {
          ...
          on: 'avatar.user.click',
          filter: { region: 'head' },
          emit: { type: 'avatar.fixture.presentation.activity_requested', detail: { activity_name: 'shy', category: 'emotion', intensity: 'strong', source: 'mock' } },
        },
      ],
    });
    const driver = new MockDriver({ scenario });
    const emissions: string[] = [];
    driver.onEvent((ev) => {
      if (ev.name === 'avatar.fixture.presentation.activity_requested') emissions.push(String(ev.detail['activity_name']));
    });
    await driver.start();
  ...
  it('loops when loop=true and duration_ms finite', async () => {
    const scenario = makeScenario({
      ...
      loop: true,
      events: [
        { kind: 'time', at_ms: 500, type: 'avatar.fixture.presentation.activity_requested', detail: { activity_name: 'a', category: 'state', intensity: null, source: 'mock' } },
      ],
    });
    const driver = new MockDriver({ scenario });
    const count = { n: 0 };
    driver.onEvent((ev) => {
      if (ev.name === 'avatar.fixture.presentation.activity_requested') count.n += 1;
    });
    await driver.start();
  ...
  it('stops emitting after stop() is called', async () => {
    const scenario = makeScenario({
      events: [
        { kind: 'time', at_ms: 1000, type: 'avatar.fixture.presentation.activity_requested', detail: { activity_name: 'x', category: 'state', intensity: null, source: 'mock' } },
      ],
    });
    const driver = new MockDriver({ scenario });
    const count = { n: 0 };
