import { hasAvatarHostRuntime, invokeAvatarHostCommand } from './avatar-host-bridge.js';
...
export async function beginManualDragWindow(): Promise<AvatarManualDragWindowOrigin | null> {
  if (!hasAvatarHostRuntime()) return null;
...
export async function getCursorClientPosition(): Promise<AvatarCursorClientPosition> {
  return invokeAvatarHostCommand<AvatarCursorClientPosition>('nimi_avatar_get_cursor_client_position', undefined, {
    fallback: {
      screenX: 0,
      screenY: 0,
      clientX: 0,
