...
//
// VRM emote state machine. Translates emotion ontology ids (loaded as a
...
export interface VrmEmoteState {
  ...
  tick(input: { vrm: VRM; deltaSec: number }): { skippedCount: number };
  reset(input: { vrm: VRM }): void;
...
export function createVrmEmoteState(input: CreateVrmEmoteStateInputs): VrmEmoteState {
  ...
  function safeSetExpression(vrm: VRM, name: string, weight: number): void {
  ...
  function tick(tickInput: { vrm: VRM; deltaSec: number }): { skippedCount: number } {
    ...
    // Advance transient overlays. They decay startWeight → 0 over
  ...
  function reset(resetInput: { vrm: VRM }): void {
