...
export const ALPHA_MASK_THRESHOLD = 10 / 255;

/** Byte-equivalent of {@link ALPHA_MASK_THRESHOLD}. Used inside the
 *  probe because `VrmRenderTarget.probeAlphaAtClient` returns a byte
 *  value [0, 255]. */
export const ALPHA_MASK_THRESHOLD_BYTE = 10;
...
export type CreateVrmHitRegionInputs = {
   ...
   *  `getCachedDeviceTier()`; falls back to 'C' (conservative) when
...
/** Resolve effective device tier with conservative fallback semantics.
 *  Override → cached detection → 'C'. */
...
export function createVrmHitRegion(
  ...
  const isOpaqueAtClientPoint = (
    ...
    const effectiveThreshold = threshold ?? ALPHA_MASK_THRESHOLD;
