...
//
// VRM BackendAudioConsumer — wires AudioPipeline source nodes into a
// `wlipsync` AudioWorklet so per-frame viseme weights become available
// to the VRM lipsync driver. Mirrors `live2d/live2d-audio-consumer.ts`
// line-for-line (the wLipSyncNode itself is backend-agnostic — what
// differs is who reads its `snapshot()` output: Live2D's lipsync driver
// writes to ParamMouthOpenY; VRM's writes to expression-manager presets).
//
// createWLipSyncNode is lazy and scoped per AudioContext (audio worklet
// processor registration is per-context; reusing across distinct
// AudioContexts is unsafe).
//
// Spec: backend-branch-contract.md §BackendAudioConsumer;
//       design-05 §"wLipSync 集成"; vrm-backend-contract.md §F.7.
  ...
  return {
    async attachAudioSource(source, audioContext) {
      ...
      try {
        ...
        console.warn('[avatar:vrm:lipsync] source.connect(wlipsync) failed', err);
        deps.onSilent?.();
      }
    },
