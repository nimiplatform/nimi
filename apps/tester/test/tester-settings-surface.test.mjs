...
test('tester settings consumes SDK Realm data sync DX surface', () => {
  ...
  assert.match(realmPersonaCore, /worldCoreControllerCreateSourceMaterializationPacket/);
...
test('tester settings consumes SDK Nimi App bridge projection parser', () => {
  ...
  assert.match(settings, /permissions\.list\(scopeRef\)/);
  assert.match(settings, /permissions\.request\(scopeRef/);
  assert.match(settings, /permissions\.revoke\(scopeRef/);
  assert.match(settings, /requestState/);
  assert.match(settings, /revokeState/);
  assert.doesNotMatch(settings, /new PermissionClient\(transport\)/);
...
test('tester settings consumes SDK Runtime agent presentation profile projection', () => { const settings = readSettingsSurface(); const helper = read('src/tester/tester-runtime-agent-presentation-profile.ts'); [/createTesterRuntimeAgentPresentationProfileProjection/, /Runtime agent presentation profile projection/, /runtimeAgentPresentationProfileProjection\.defaultVoiceReference/].forEach((pattern) => assert.match(settings, pattern)); [/buildNimiSetRuntimeAgentPresentationProfileRequest/, /normalizeNimiRuntimeAgentPresentationDefaultVoiceReference/, /from '@nimiplatform\/sdk\/runtime'/].forEach((pattern) => assert.match(helper, pattern)); [/buildSetRuntimeAgentPresentationProfileRequest/, new RegExp(['parse', 'Runtime', 'Local', 'Agent', 'Identity'].join('')), /normalizeRuntimeAgentPresentationDefaultVoiceReference/].forEach((pattern) => assert.doesNotMatch(helper, pattern)); });
