...
test('history persistence compensation removes every managed artifact and reports cleanup failures', async () => {
  ...
  const outcome = await persistTesterRunHistoryWithArtifactCompensation(
    result,
    async () => {
      persistenceCalls += 1;
...
test('failed compensation paths remain directly retryable without a new persistence owner', async () => {
  ...
  const first = await cleanupTesterManagedArtifactPaths(
  ...
  locked = false;
  const retry = await cleanupTesterManagedArtifactPaths(first.remainingCleanupPaths, removeAsset);
...
test('a later successful run does not erase a surviving artifact cleanup retry', () => {
  const survivor = settleTesterHistorySaveIssueAfterPersistedRun({
  ...
  assert.equal(settleTesterHistorySaveIssueAfterPersistedRun({
...
test('multi-artifact deletion uses canonical run history and converges after a partial asset failure', async () => {
  ...
  const first = await deleteTesterManagedHistoryRecord(state.port, 'run-video', true);
  ...
  const second = await deleteTesterManagedHistoryRecord(state.port, 'run-video', true);
...
test('multi-artifact deletion retains canonical run history when image-history cleanup fails', async () => {
  ...
  const result = await deleteTesterManagedHistoryRecord(state.port, 'run-1', true);
