...
function invalidStorageValue(path: string, detail: string): never {
  throw new Error(`Tester standard storage value ${detail} at ${path}.`);
}
...
export function normalizeTesterStandardStorageJsonValue(value: unknown): JsonValue {
...
export async function readTesterStandardStorageJson(relativePath: string): Promise<JsonValue | undefined> {
  try {
    const result = await getTesterLocalAppClient().storage.readJson(relativePath);
    return result.value;
  } catch (error) {
...
export async function writeTesterStandardStorageJson(relativePath: string, value: unknown): Promise<void> {
  await getTesterLocalAppClient().storage.writeJson(
    relativePath,
    normalizeTesterStandardStorageJsonValue(value),
  );
}
