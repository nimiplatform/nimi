...
test('agent host submit driver converges completed submit to authoritative bundle, hidden footer, and cleared submitting state', () => {
...
test('agent host submit driver preserves sealed first-beat, restores composer text, and ignores late refresh after tail cancel', () => {
  const threadId = 'thread-1';
  const harness = createAgentHostHarness({
    threadId,
    initialBundle: baseUserBundle(),
...
test('agent host submit driver does not create a pending second text message for follow-up planning', () => {
  const threadId = 'thread-1';
  const harness = createAgentHostHarness({
    threadId,
    initialBundle: baseUserBundle(),
...
test('agent host submit driver restores composer text and clears submitting state when runtime fails before first-beat', () => {
  const threadId = 'thread-1';
  const harness = createAgentHostHarness({
    threadId,
    initialBundle: baseUserBundle(),
