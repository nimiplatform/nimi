...
import {
  ...
  type LocalAgentTerminationDeliverer,
} from '../src/shell/renderer/infra/local-agent-courier/termination-courier.js';
 ...
 * intent operations the courier actually calls; this alias names that exact
 * call shape so the structural double is cast once, at one boundary.
 */
type RealmCourierApiCaller = <T>(task: (realm: Realm) => Promise<T>, fallbackMessage?: string) => Promise<T>;

/**
 * R-SOC-008 / T6.1.2-W2 — desktop reconciliation courier.
 *
 * The courier is pure transport: it pulls the viewer's OPEN
 * LocalAgentTerminationIntents, delivers runtime.agent.terminateAgent to the
 ...
 * on the real backend transition (W1): an `ackMyLocalAgentTerminationIntent`
 * with `terminated` removes the intent from the OPEN list; with
      ...
      services: {
        MeService: {
          ...
          ackMyLocalAgentTerminationIntent: async (
            ...
            return { id: intentId, status: body.outcome === 'terminated' ? 'ACKED' : 'OPEN' };
          },
        },
      },
    } as unknown as Realm;
...
describe('R-SOC-008 T6.1.2-W2: courier pull + deliver + ack on success', () => {
  test('pulls OPEN intents, delivers terminate, and acks terminated', async () => {
    const backend = createBackendDouble([makeIntent('intent-a'), makeIntent('intent-b')]);
    const delivered: string[] = [];
    const deliverer: LocalAgentTerminationDeliverer = async (intent) => {
      delivered.push(intent.localAgentRef);
    };

    const result = await runLocalAgentTerminationCourierPass({
    ...
    assert.ok(backend.ackCalls.every((call) => call.outcome === 'terminated'));
    assert.equal(backend.openIntents.length, 0, 'acked intents leave the OPEN list');
  });

  test('empty pull is a no-op — no terminate, no ack', async () => {
    ...
    assert.deepEqual(result, { pulled: 0, terminated: 0, substrateFailed: 0, deferred: 0 });
    assert.equal(deliveries, 0);
    assert.equal(backend.ackCalls.length, 0);
  });
});

describe('R-SOC-008 T6.1.2-W2: absent-ref no-op converges', () => {
  test('K-AGCORE-141 absent-ref typed no-op (deliverer resolves) → acks terminated', async () => {
    // A never-materialized LocalAgent: runtime.agent.terminateAgent returns a
    // typed success no-op, so the deliverer simply resolves.
    const backend = createBackendDouble([makeIntent('intent-absent')]);
    const deliverer: LocalAgentTerminationDeliverer = async () => {
      /* typed no-op success */
    };
    ...
    assert.equal(result.deferred, 0);
    assert.equal(backend.ackCalls[0]?.outcome, 'terminated');
    assert.equal(backend.openIntents.length, 0, 'never-materialized agent converges');
  });
});

describe('R-SOC-008 T6.1.2-W2: transport/offline failure leaves the intent OPEN', () => {
...
describe('R-SOC-008 T6.1.2-W2: typed substrate failure is acked, never synthesized as success', () => {
  test('K-AGCORE-141 fail-closed substrate failure → acks substrate_failure (not terminated)', async () => {
    const backend = createBackendDouble([makeIntent('intent-substrate')]);
    const deliverer: LocalAgentTerminationDeliverer = async () => {
      throw createNimiError({
        message: 'projection delete failed: disk error',
...
describe('R-SOC-008 T6.1.2-W2: statelessness — re-pulls each pass, holds no queue', () => {
  test('a second pass re-pulls from the backend; converged intents do not reappear', async () => {
    ...
    const first = await runLocalAgentTerminationCourierPass({
      callApi: backend.callApi,
      emitCourierError: noopEmit,
      getCurrentUser,
      deliverer,
    ...
    // intents were acked terminated last pass, so the backend returns none.
    const second = await runLocalAgentTerminationCourierPass({
      callApi: backend.callApi,
      emitCourierError: noopEmit,
      getCurrentUser,
      deliverer,
  ...
  test('an intent left OPEN by an offline pass is re-delivered on the next pass', async () => {
    ...
    const offlinePass = await runLocalAgentTerminationCourierPass({
      callApi: backend.callApi,
      emitCourierError: noopEmit,
      getCurrentUser,
      deliverer,
    ...
    // converges it. This is the long-offline convergence guarantee.
    runtimeUp = true;
    const onlinePass = await runLocalAgentTerminationCourierPass({
      callApi: backend.callApi,
      emitCourierError: noopEmit,
      getCurrentUser,
      deliverer,
    ...
    assert.equal(onlinePass.terminated, 1);
    assert.equal(backend.openIntents.length, 0, 'converged on the device coming online');
  });
});

describe('R-SOC-008 T6.1.2-W2: the courier owns no decision', () => {
  ...
  test('a re-delivered intent (lost ack) is safe — backend ack stays idempotent', async () => {
    ...
    // K-AGCORE-141 makes the second terminate a typed no-op; the backend ack is
    // idempotent. The courier re-acks without error.
    const intent = makeIntent('intent-lost-ack');
    const backend = createBackendDouble([intent]);
    let firstAck = true;
    const callApiWithLostAck: RealmCourierApiCaller = async <T>(
      ...
      const realm = {
        services: {
          MeService: {
            listMyLocalAgentTerminationIntents: async () => ({ items: [{ ...intent }] }),
            ackMyLocalAgentTerminationIntent: async (
              intentId: string,
              body: { outcome: string },
            ) => {
              if (firstAck) {
                firstAck = false;
                // Ack POST is lost on the wire after the runtime delete.
                throw createNimiError({
                  message: 'realm unavailable',
                  reasonCode: ReasonCode.REALM_UNAVAILABLE,
                  actionHint: 'retry_when_online',
    ...
    let deliveries = 0;
    const deliverer: LocalAgentTerminationDeliverer = async () => {
      deliveries += 1; // second delivery is a K-AGCORE-141 no-op
    };

    // Pass 1: terminate succeeds, ack POST is lost → intent left OPEN (deferred).
    const pass1 = await runLocalAgentTerminationCourierPass({
    ...
    const pass2 = await runLocalAgentTerminationCourierPass({
      callApi: callApiWithLostAck,
      emitCourierError: noopEmit,
      getCurrentUser,
      deliverer,
    });
    assert.equal(pass2.terminated, 1, 're-pull re-delivers and re-acks');
    assert.equal(deliveries, 2, 'terminate delivered twice; second is the K-AGCORE-141 no-op');
    void backend;
  });
});

describe('R-SOC-008 T6.1.2-W2: courier wiring constants', () => {
  test('periodic tick is registered under a stable key with a ~60s interval', () => {
    assert.equal(COURIER_POLLING_KEY, 'local-agent-termination-courier');
