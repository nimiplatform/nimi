...
test('renderer evidence: continuing from the Local AI phase records the install level and starts Runtime materialization', () => {
  ...
  assert.ok(
    materializationStartIndex > installLevelIndex,
    'Local AI continue must start materialization after Product Control persists the install level',
  );
  assert.doesNotMatch(productControlWorkflowSource, /syncFirstRunRuntimeDataRootConfig|syncRuntimeStorageConfig/);
