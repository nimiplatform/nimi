...
test('createResolveRuntimeBinding reads source/model/connectorId from RuntimeFields', async () => {
    ...
    await assert.rejects(() => resolve({ targetId: 'test-target' }), /RUNTIME_ROUTE_BINDING_REQUIRED/);
});
...
test('createResolveRuntimeBinding connector binding takes priority over fields.connectorId', async () => {
    ...
    const result = await resolve({ targetId: 'test-target', binding });
...
test('createResolveRuntimeBinding no longer falls back to fields.localProviderModel', async () => {
    ...
    await assert.rejects(() => resolve({ targetId: 'test-target', binding }), /RUNTIME_ROUTE_BINDING_MODEL_REQUIRED/);
});
