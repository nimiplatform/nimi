...
test('notification panel unread count consumes SDK Realm projection, not Desktop dataSync parser', () => {
  ...
  assert.match(notificationPanelSource, /queryFn:\s*async \(\) => loadNimiRealmNotificationUnreadCount\(bindings\.sdk\.realm\(\)\)/);
  assert.doesNotMatch(notificationPanelSource, /desktop-nimi-client-session/);
