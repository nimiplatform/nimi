...
test('send gift modal loads dynamic gift catalog and sends selected gift ids', () => {
  ...
  assert.match(modalSource, /createRealmCommerceGiftService/);
  assert.match(modalSource, /generated: sdk\.realm\(\)\.generated/);
...
test('send gift modal uses explicit receiverIsSource instead of handle-prefix inference', () => {
  ...
  assert.match(modalSource, /kind=\{props\.receiverIsSource === true \? 'source' : 'human'\}/);
