...
import { collectDesktopReleaseSyncViolations, readJson } from './lib/desktop-release-sync.mjs';
...
function main() {
  ...
  const violations = collectDesktopReleaseSyncViolations(desktopRoot, expected);
  ...
  process.stdout.write(`[check-desktop-release-sync] version=${expected}\n`);
}
