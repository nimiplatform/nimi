...
fn set_state(app: Option<&AppHandle>, update: impl FnOnce(&mut DesktopUpdateState)) -> DesktopUpdateState {
    ...
    store_pending_update(PendingUpdatePayload {
        ...
        install: Box::new(move |payload| {
            update.install(payload).map_err(|error| {
                format!("DESKTOP_UPDATER_INSTALL_FAILED: {error}")
            })
        }),
    });
    Ok(DesktopUpdateCheckResult {
...
fn install_pending_downloaded_update(app: Option<&AppHandle>) -> Result<DesktopUpdateState, String> {
    ...
    let pending = take_pending_update().ok_or_else(|| {
        set_error_state(
            app,
            "DESKTOP_UPDATER_DOWNLOAD_REQUIRED: download an update before installing it".to_string(),
        )
    })?;
...
mod tests {
    ...
    fn with_release_version(run: impl FnOnce()) {
        with_env(
            &[
                ...
                ("NIMI_DESKTOP_UPDATER_ENDPOINT", Some("https://updates.example.com/latest.json")),
            ],
            || {
                reset_test_state();
    ...
    fn configured_updater_pubkey_prefers_runtime_env() {
        with_env(&[("NIMI_DESKTOP_UPDATER_PUBLIC_KEY", Some("runtime-pubkey"))], || {
            assert_eq!(
    ...
    fn install_requires_downloaded_payload() {
        with_release_version(|| {
            clear_pending_update();
            let error = install_pending_downloaded_update(None).err().unwrap_or_default();
