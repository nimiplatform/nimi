...
fn normalize_required_local_agent_handoff_value(
    ...
    if !normalized.starts_with(LOCAL_AGENT_REF_PREFIX) {
        return Err(structured_avatar_handoff_error(
            "DESKTOP_AVATAR_HANDOFF_INVALID",
...
fn spawn_avatar_handoff_binary(path: PathBuf, uri: &str) -> Result<(), String> {
    ...
    command.arg(uri);
    spawn_avatar_handoff_process(command)
...
fn spawn_avatar_handoff_process(mut command: Command) -> Result<(), String> {
...
/// `set_standard_shell_ui_host_hooks`.
