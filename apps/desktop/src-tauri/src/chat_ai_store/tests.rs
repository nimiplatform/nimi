...
fn chat_ai_db_path(storage_root: &std::path::Path) -> PathBuf {
...
#[test]
fn chat_ai_db_path_stays_under_runtime_app_storage_root() {
    let storage_root = temp_home("db-path");
    let path = super::db::db_path(storage_root.to_str().expect("storage root")).expect("db path");
    assert_eq!(
        path,
        chat_ai_db_path(&storage_root.canonicalize().expect("canonical root"))
    );
}

#[test]
fn chat_ai_open_db_initializes_schema_idempotently() {
    let storage_root = temp_home("schema");
    let conn = open_test_conn(&storage_root);
...
#[test]
fn chat_ai_schema_rejects_legacy_thread_route_columns() {
    let storage_root = temp_home("legacy-route-columns");
    let path = chat_ai_db_path(&storage_root);
    fs::create_dir_all(path.parent().expect("parent")).expect("create parent");
    let conn = Connection::open(&path).expect("open");
    ...
    let error = super::schema::init_schema(&conn).expect_err("legacy schema rejected");
    assert!(error.contains("CHAT_AI_SCHEMA_MISMATCH"));
    assert!(error.contains(
        "forbidden_legacy_columns=route_kind,connector_id,provider,model_id,route_binding_json"
    ));
}

#[test]
fn chat_ai_store_round_trip_thread_message_and_draft() {
    let storage_root = temp_home("roundtrip");
    let conn = open_test_conn(&storage_root);
...
fn chat_ai_store_allows_empty_pending_assistant_placeholder_content() {
    let storage_root = temp_home("empty-placeholder");
    let conn = open_test_conn(&storage_root);
...
fn chat_ai_store_rejects_missing_thread_duplicate_id_and_invalid_json() {
    let storage_root = temp_home("errors");
    let conn = open_test_conn(&storage_root);
    ...
    let bundle_error = get_thread_bundle(&conn, "thread-ai-dup").expect_err("bad json should fail");
    assert!(bundle_error.contains("invalid JSON"));
}

#[test]
fn chat_ai_draft_put_overwrites_and_delete_clears() {
    let storage_root = temp_home("draft");
    let conn = open_test_conn(&storage_root);
