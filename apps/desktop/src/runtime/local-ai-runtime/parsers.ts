...
export function assertLifecycleWriteAllowed(command: string, caller: LocalAiRuntimeWriteOptions['caller']): void {
...
export async function invokeLocalAiCommand<T>(command: string, args: Record<string, unknown> = {}): Promise<T> {
  if (!hasTauriInvoke()) {
