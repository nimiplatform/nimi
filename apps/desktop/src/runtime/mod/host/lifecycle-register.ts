...
export async function registerRuntimeModState(input: {
  ...
  if (input.sourceType === 'codegen') {
    ...
    const missingConsent = decision.requiresConsent.filter(
      (capability) => !anyCapabilityMatches(grants, capability),
    );
  ...
  const nextDefaultPrivateExecutionModId =
    input.mod.isDefaultPrivateExecution || input.defaultPrivateExecutionModId === input.mod.modId
      ? input.mod.modId
