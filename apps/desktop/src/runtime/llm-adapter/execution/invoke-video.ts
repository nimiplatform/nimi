...
export async function invokeModVideo(input: InvokeModVideoInput): Promise<InvokeModVideoOutput> {
  ...
  try {
    ...
    const generated = await runtime.media.video.generate({
      ...
      firstFrameUri: String(input.firstFrameUri || '').trim() || undefined,
      lastFrameUri: String(input.lastFrameUri || '').trim() || undefined,
      cameraMotion: String(input.cameraMotion || '').trim() || undefined,
