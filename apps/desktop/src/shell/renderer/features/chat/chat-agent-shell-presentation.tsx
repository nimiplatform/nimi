...
> {
  ...
  const emptyStateAgent = null;
  const emptyStateSuggestions = input.activeTarget ? (
    <AgentIntroduction
      key={input.activeTarget.agentHandle}
      introduction={input.emptyStateCharacterPresence ?? null}
      unavailable={input.introductionUnavailable}
      onRetry={input.onRetryIntroduction}
      displayName={resolvedAgentDisplayName}
      avatarUrl={characterData.avatarUrl || null}
      locale={i18n.resolvedLanguage ?? i18n.language}
      questionsEnabled={Boolean(surfaceState.composer && !surfaceState.composer.disabled && input.onComposerPrefillRequest)}
      onPrefill={input.onComposerPrefillRequest}
    />
  ) : null;
  const emptyStateHeaderLabels = input.activeTarget
    ? { emptyEyebrow: '', emptyTitle: '', emptyDescription: '' }
    : {
      emptyEyebrow: resolvedAgentDisplayName,
      emptyTitle: input.t('Chat.agentTranscriptEmptyTitle', { defaultValue: 'Start a conversation' }),
      emptyDescription: input.t('Chat.agentTranscriptEmpty', { defaultValue: 'Ask a question, share an idea, or tell this agent what you want to explore.' }),
    };
