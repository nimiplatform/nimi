...
export default function ChatAgentAvatarVrmViewport({
  ...
  return (
    <div
      className="relative flex h-full w-full items-center justify-center overflow-hidden bg-[radial-gradient(circle_at_30%_20%,rgba(255,255,255,0.98),rgba(224,231,255,0.88)_45%,rgba(186,230,253,0.7)_68%,rgba(14,165,233,0.16))]"
    ...
    >
      {input.posterUrl ? (
        <img
          src={input.posterUrl}
          alt={input.label}
          className="absolute inset-0 h-full w-full object-cover opacity-20 saturate-150"
        />
      ) : null}
      <span className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.58),transparent_54%)]" />
      <div className="absolute inset-[6%] overflow-hidden rounded-[46%] border border-white/70 bg-white/18 shadow-[0_30px_80px_rgba(14,165,233,0.14)] backdrop-blur-[2px]">
        <Canvas
