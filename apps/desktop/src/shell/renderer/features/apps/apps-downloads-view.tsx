...
export function isAppDownloadJob(job: AppPackageJob): boolean {
  return job.sourceClass === AppPackageSourceClass.VERIFIED && [AppPackageJobKind.INSTALL, AppPackageJobKind.UPDATE].includes(job.kind);
}
...
export function AppsDownloadsView({ downloads, entries, onViewApp, onRetry }: {
  ...
  const queued = jobs.filter((job) => job.phase === AppPackageJobPhase.QUEUED).sort((a, b) => a.queuePosition - b.queuePosition);
  const paused = jobs.filter((job) => job.phase === AppPackageJobPhase.PAUSED);
  const failed = jobs.filter((job) => failedDownloadNeedsAttention(job, jobs));
  const processing = jobs.filter((job) => !packageJobIsTerminal(job) && ![AppPackageJobPhase.QUEUED, AppPackageJobPhase.DOWNLOADING, AppPackageJobPhase.PAUSED].includes(job.phase));
  ...
  const detail = selected ?? active[0] ?? processing[0] ?? failed[0] ?? queued[0] ?? paused[0] ?? recent[0];
  ...
  const actions = (job: AppPackageJob) => <div className="flex flex-wrap gap-2">
    {[AppPackageJobPhase.QUEUED, AppPackageJobPhase.DOWNLOADING].includes(job.phase) ? <Button size="sm" disabled={blocked} leadingIcon={<Pause className="h-4 w-4" aria-hidden="true" />}
    ...
    {failedDownloadNeedsAttention(job, jobs) ? <Button size="sm" disabled={blocked} onClick={() => onRetry(job)}>{t('Apps.downloads.retry')}</Button> : null}
    ...
    return <li key={id} data-testid={`app-download-job-${id}`} className="flex flex-wrap items-center gap-2 border-b border-[var(--nimi-border-subtle)] px-3 py-2 last:border-b-0"
      ...
      <Button tone="ghost" size="sm" className="min-w-0 flex-1 justify-start px-1 text-left"
        ...
        <span className="flex min-w-0 flex-col items-start"><span className="max-w-full truncate font-semibold">{jobName(job)}</span>
          <span className="whitespace-normal text-xs font-normal text-[var(--nimi-text-secondary)]">{job.previousVersion ? `${job.previousVersion} → ` : ''}{job.targetVersion} · {t(`Apps.downloads.phase.${appDownloadPhase(job)}`)}</span></span>
      </Button>
  ...
  const group = (label: string, groupJobs: readonly AppPackageJob[], order = false) => groupJobs.length ? <section>
    <h3 className="mb-2 flex items-center gap-2 text-sm font-semibold text-[var(--nimi-text-primary)]">{t(`Apps.downloads.${label}`)}<span className="text-xs font-normal tabular-nums text-[var(--nimi-text-secondary)]">{groupJobs.length}</span></h3>
    <Surface tone="card" padding="none" className="overflow-hidden rounded-lg"><ul>{groupJobs.map((job) => row(job, order))}</ul></Surface>
  </section> : null;
  const matchingEntry = detail ? entries.find((entry) => entry.identity.appId === detail.appId && entry.identity.sourceClass === 'verified') : undefined;
  ...
  return <div className="flex min-h-0 flex-1 flex-col" data-testid="apps-downloads-view">
      ...
      <div className="grid min-w-0 gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(260px,.8fr)]">
        <div className={`min-w-0 space-y-4 ${selected ? 'hidden xl:block' : ''}`}>
          ...
          {recent.length ? <section><Button tone="ghost" size="sm" aria-expanded={recentOpen} onClick={() => setRecentOpen(!recentOpen)}>{t('Apps.downloads.recent', { count: recent.length })}</Button>
            {recentOpen ? <Surface tone="card" padding="none" className="mt-2 overflow-hidden rounded-lg"><ul>{recent.map((job) => row(job))}</ul></Surface> : null}</section> : null}
        </div>
        {detail ? <Surface as="aside" tone="card" padding="none" className={`min-w-0 self-start rounded-lg p-4 ${selected ? '' : 'hidden xl:block'}`} data-testid="apps-download-detail">
          ...
          {[AppPackageJobPhase.DOWNLOADING, AppPackageJobPhase.QUEUED, AppPackageJobPhase.PAUSED].includes(detail.phase) ? <div className="mb-3">{metrics(detail)}</div> : null}
          {actions(detail)}
          <div className="mt-4 space-y-2 text-xs leading-5 text-[var(--nimi-text-secondary)]">
            {detail.phase === AppPackageJobPhase.QUEUED ? <p>{t('Apps.downloads.position', { position: detail.queuePosition || '—' })}</p> : null}
            ...
            {detail.phase === AppPackageJobPhase.VERIFYING ? <p>{t('Apps.downloads.verifyingHint')}</p> : null}
            ...
            {detail.phase === AppPackageJobPhase.CANCELED ? <p>{t('Apps.downloads.canceledHint')}</p> : null}
    ...
    <ConfirmDialog open={cancelJob !== null} title={t('Apps.downloads.cancelTitle')} confirmTone="danger"
      message={cancelJob ? t('Apps.downloads.cancelMessage', { app: jobName(cancelJob), size: formatBytes(Number(cancelJob.bytesCompleted)) }) : ''}
      confirmLabel={t('Apps.downloads.confirmCancel')} cancelLabel={t('Apps.downloads.keepTask')} loading={busy}
