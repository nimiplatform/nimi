 ...
 * these sub-areas. `D-DEV-003` fixes what the surface carries: mod / developer
 * source directory management with auto-reload, developer-mode technical
 * diagnostics, and the `nimi.tester` developer-only app reference. It MUST NOT
 * host ordinary-user product functionality (`D-DEV-003` MUST NOT).
 */
...
export const DEVELOPER_TOOLS_SECTION_LABEL_KEY: Record<DeveloperToolsSectionId, string> = {
  'mod-sources': 'DeveloperTools.sectionModSources',
...
export function resolveDeveloperToolsSection(value: unknown): DeveloperToolsSectionId {
  return isDeveloperToolsSectionId(value) ? value : 'mod-sources';
}
