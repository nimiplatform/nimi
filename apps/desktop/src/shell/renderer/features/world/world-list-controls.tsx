...
export type FilterId = 'all' | 'main' | 'sub' | 'archived';
export type SortId = 'active' | 'recent' | 'alpha' | 'inhabitants';
export type ViewMode = 'grid' | 'list';

const FROZEN_STATUS = 'FROZEN';

export function isArchivedWorld(world: WorldListItem): boolean {
  return world.status === FROZEN_STATUS || Boolean(world.freezeReason);
}

type WorldCatalogToolbarProps = {
  view: ViewMode;
...
export function WorldCatalogToolbar({
  view,
  setView,
  sort,
  setSort,
...
export function WorldCatalogSidebar({
  worlds,
  filter,
  setFilter,
  counts,
