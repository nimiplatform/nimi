...
export function XianxiaWorldTemplate(props: XianxiaWorldTemplateProps) {
  ...
  const scoreMeta = [
    { key: 'scoreA', label: 'Activity Score', short: t('WorldDetail.scoresShort'), desc: t('WorldDetail.scoresDesc') },
    { key: 'scoreC', label: 'Consensus Score', short: t('WorldDetail.scoresShort'), desc: t('WorldDetail.scoresDesc') },
    { key: 'scoreE', label: 'Engagement Score', short: t('WorldDetail.scoresShort'), desc: t('WorldDetail.scoresDesc') },
    { key: 'scoreQ', label: 'Quality Score', short: t('WorldDetail.scoresShort'), desc: t('WorldDetail.scoresDesc') },
  ...
  const formatDate = (d: string) => new Date(d).toLocaleDateString('zh-CN', { month: '2-digit', day: '2-digit' });
      ...
      <div className="relative z-10 w-[min(1400px,calc(100vw-48px))] mx-auto my-6 mb-16 flex flex-col gap-5">
          ...
          <div className="absolute inset-0 flex flex-col justify-between p-8">
            ...
            <div className="flex items-end gap-6">
              ...
              <div className="flex-1 pb-2">
                <h1 className="text-[clamp(28px,4vw,48px)] leading-tight font-serif tracking-wide text-white drop-shadow-lg"
        ...
        <div className="grid grid-cols-[1fr_1.2fr_1fr] gap-5">
          ...
          <section className="relative overflow-hidden rounded-[20px] border border-[#4ECCA3]/15 bg-[#0f1612]/60 backdrop-blur-sm p-5">
            ...
            {/* ID Badge */}
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#4ECCA3]/10 border border-[#4ECCA3]/20 text-xs text-[#4ECCA3] mb-4">
              <span className="w-1.5 h-1.5 rounded-full bg-[#4ECCA3]" />
              {displayValue(world.id)}
            </div>

            {/* World Name */}
            <h3 className="text-xl font-bold text-[#e8f5ee] mb-3">{displayValue(world.name)}</h3>

            {/* Description */}
            <div className="mb-4">
              <div className="text-xs text-[#e8f5ee]/40 mb-1">Description</div>
              <p className="text-sm text-[#e8f5ee]/70 leading-relaxed">{displayValue(world.description)}</p>
            </div>

            {/* Narrative */}
            <div className="mb-4">
              <div className="text-xs text-[#e8f5ee]/40 mb-1">World Narrative</div>
              <p className="text-sm text-[#e8f5ee]/70 leading-relaxed">{displayValue(world.narrative || world.description)}</p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                ...
                <div className="text-sm text-[#e8f5ee]">{world.createdAt ? formatDateTime(world.createdAt) : 'N/A'}</div>
              </div>
              <div>
                <div className="text-xs text-[#e8f5ee]/40 mb-1">Hyper</div>
                <div className="text-sm text-[#e8f5ee]">{displayValue(world.creatorId)}</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="p-3 rounded-xl bg-[#0a0f0c]/60 border border-[#4ECCA3]/10">
                <div className="text-xs text-[#e8f5ee]/40 mb-1">Agent Count</div>
                <div className="text-lg font-bold text-[#4ECCA3]">{world.agentCount !== undefined ? world.agentCount : 'N/A'}</div>
              </div>
              <div className="p-3 rounded-xl bg-[#0a0f0c]/60 border border-[#4ECCA3]/10">
                <div className="text-xs text-[#e8f5ee]/40 mb-1">Time Flow Ratio</div>
                <div className="text-lg font-bold text-[#4ECCA3]">{world.timeFlowRatio ? `${world.timeFlowRatio.toFixed(1)}x` : 'N/A'}</div>
              </div>
            </div>
            ...
            <div className="relative w-full aspect-square max-w-[320px] mx-auto">
              <svg viewBox="0 0 300 300" className="w-full h-full">
                ...
                {Array.from({ length: levels }, (_, idx) => {
                  ...
                  return <polygon key={idx} points={pts} fill="none" stroke="rgba(78,204,163,0.1)" strokeWidth="1" />;
                })}
                
                {/* Axis lines */}
                {metrics.map((_, i) => {
                  const p = polarPoint(cx, cy, radius, i * angleStep);
                  return <line key={i} x1={cx} y1={cy} x2={p.x} y2={p.y} stroke="rgba(78,204,163,0.15)" strokeWidth="1" />;
                })}
                
                {/* Data area */}
                <polygon points={polygonPoints} fill="url(#scoreFill)" stroke="url(#scoreStroke)" strokeWidth="2" />
                
                {/* Data points */}
                {metrics.map((m, i) => {
                  const p = polarPoint(cx, cy, radius * (m.value / 100), i * angleStep);
                  return <circle key={i} cx={p.x} cy={p.y} r="4" fill="#4ECCA3" stroke="#0f1612" strokeWidth="2" />;
                })}
              ...
              {metrics.map((m, i) => {
                ...
                const value = m.value !== undefined && m.value !== null ? Number(m.value).toFixed(0) : '-';
          ...
          <section className="relative overflow-hidden rounded-[20px] border border-[#4ECCA3]/15 bg-[#0f1612]/60 backdrop-blur-sm p-5">
            ...
            <div className="relative flex flex-col gap-4">
              ...
              {props.events.length > 0 ? props.events.slice(0, 5).map((event, idx) => (
                <div key={event.id} className="relative pl-8">
                  ...
                  <div className="p-3 rounded-xl bg-[#0a0f0c]/40 border border-[#4ECCA3]/10">
                    <h4 className="text-sm font-bold text-[#e8f5ee] mb-1">{displayValue(event.title)}</h4>
                    <p className="text-xs text-[#e8f5ee]/50 leading-relaxed line-clamp-3">{displayValue(event.description)}</p>
                  </div>
                </div>
              )) : (
        ...
        <section className="relative overflow-hidden rounded-[20px] border border-[#4ECCA3]/15 bg-[#0f1612]/60 backdrop-blur-sm p-5">
          ...
          <div className="grid grid-cols-3 gap-4">
            {props.agents.length > 0 ? props.agents.slice(0, 6).map((agent) => (
              <article key={agent.id} className="relative p-4 rounded-xl bg-[#0a0f0c]/60 border border-[#4ECCA3]/10 overflow-hidden">
                {/* Avatar */}
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <h4 className="text-base font-bold text-[#e8f5ee] truncate">{displayValue(agent.name)}</h4>
                      <div className="w-2 h-2 rounded-full bg-[#4ECCA3] flex-shrink-0" title="Online" />
                    </div>
                    <div className="text-xs text-[#4ECCA3] truncate">{displayValue(agent.handle)}</div>
                  </div>
                </div>

                {/* Bio */}
                <p className="text-xs text-[#e8f5ee]/60 leading-relaxed mb-3 line-clamp-2">{displayValue(agent.bio, t('WorldDetail.xianxia.noData.bio'))}</p>
              ...
              <div className="col-span-3 py-16 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-[#e8f5ee]/60 text-sm">{t('WorldDetail.xianxia.agents.noData')}</p>
              </div>
            )}
          </div>
        </section>
