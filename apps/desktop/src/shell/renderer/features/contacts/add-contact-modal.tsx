...
export function AddContactModal(props: AddContactModalProps) {
  ...
  const handleIdentifierChange = (next: string) => {
    setIdentifier(next);
    // Typing a new query invalidates the prior result + errors.
  ...
  const blockClose = adding || searching;
  const titleNode = (
    <div className="flex items-center justify-between gap-4">
      <h2 className="text-base font-semibold text-[color:var(--nimi-text-primary)]">
  ...
  return (
    <OverlayShell
      open={props.open}
      kind="dialog"
      onClose={blockClose ? undefined : props.onClose}
      closeOnBackdrop={!blockClose}
      title={titleNode}
      footer={footerNode}
