...
export function RemoveFriendConfirmDialog({
  ...
  contact: Pick<ContactRecord, 'displayName'>;
  pending?: boolean;
  onConfirm: () => void;
  onCancel: () => void;
}) {
  return (
    <OverlayShell
      ...
      dataTestId={E2E_IDS.contactsRemoveFriendConfirmDialog}
    ...
    >
      <p className="text-sm text-gray-500">
          {i18n.t('Contacts.removeFriendConfirmMessagePrefix', { defaultValue: 'Remove' })}{' '}
          <span className="font-medium text-gray-700">{contact.displayName}</span>
          ? {i18n.t('Contacts.removeFriendConfirmMessageSuffix', { defaultValue: 'This will remove them from your friends list.' })}
      </p>
    </OverlayShell>
  );
}
