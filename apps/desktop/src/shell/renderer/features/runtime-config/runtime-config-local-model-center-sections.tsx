        ...
        >
          ...
          {props.showImportMenu ? (
            <div className="absolute right-0 top-full z-20 mt-1 w-56 overflow-hidden rounded-xl border border-[var(--nimi-border-subtle)] bg-[var(--nimi-surface-overlay)] shadow-[var(--nimi-elevation-floating)]">
              <button type="button" onClick={() => { void props.onImportFile().catch(() => undefined); }} className="w-full px-3 py-2.5 text-left text-xs transition-colors hover:bg-[color-mix(in_srgb,var(--nimi-text-primary)_6%,transparent)]">
              ...
              <button type="button" onClick={() => { void props.onImportDirectory().catch(() => undefined); }} className="w-full border-t border-[var(--nimi-border-subtle)] px-3 py-2.5 text-left text-xs transition-colors hover:bg-[color-mix(in_srgb,var(--nimi-text-primary)_6%,transparent)]">
                <div className="font-medium text-[var(--nimi-text-primary)]">
                  {i18n.t('runtimeConfig.localModelCenter.importModelDirectory', { defaultValue: 'Import Model Directory' })}
                </div>
                <div className="mt-0.5 text-[var(--nimi-text-muted)]">
                  {i18n.t('runtimeConfig.localModelCenter.importModelDirectoryDescription', { defaultValue: 'Import all regular files as one content-addressed ModelAsset.' })}
                </div>
              </button>
            </div>
          ) : null}
