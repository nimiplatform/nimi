...
export type RuntimeCatalogVideoGeneration = { modes: string[]; inputRoles: Array<{ key: string; values: string[] }>; limits: Record<string, unknown>; optionSupports: string[]; optionConstraints: Record<string, unknown>; outputs: { videoUrl: boolean; lastFrameUrl: boolean } };
...
export function jsonToProtoStruct(value: Record<string, unknown>): ProtoStruct { return { fields: Object.fromEntries(Object.entries(value || {}).map(([key, item]) => [key, jsonToProtoValue(item)])) }; }
function jsonToProtoValue(value: unknown): ProtoValue { if (value === null || value === undefined) return { kind: { oneofKind: 'nullValue', nullValue: 0 } }; if (Array.isArray(value)) return { kind: { oneofKind: 'listValue', listValue: { values: value.map(jsonToProtoValue) } } }; if (typeof value === 'number') return { kind: { oneofKind: 'numberValue', numberValue: value } }; if (typeof value === 'boolean') return { kind: { oneofKind: 'boolValue', boolValue: value } }; if (typeof value === 'string') return { kind: { oneofKind: 'stringValue', stringValue: value } }; return { kind: { oneofKind: 'structValue', structValue: jsonToProtoStruct(value as Record<string, unknown>) } }; }
export function protoStructToJson(value?: ProtoStruct): Record<string, unknown> { const output: Record<string, unknown> = {}; for (const [key, item] of Object.entries(value?.fields || {})) output[key] = protoValueToJson(item); return output; }
function protoValueToJson(value?: ProtoValue): unknown { switch (value?.kind.oneofKind) { case 'boolValue': return value.kind.boolValue; case 'numberValue': return value.kind.numberValue; case 'stringValue': return value.kind.stringValue; case 'structValue': return protoStructToJson(value.kind.structValue); case 'listValue': return (value.kind.listValue?.values || []).map(protoValueToJson); default: return null; } }
