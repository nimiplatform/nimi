          ...
          {repairedUses.length > 0 ? (
            <InlineAlert tone="success" data-testid="runtime-profile-share-repaired">
              {t('runtimeConfig.profiles.share.repairedBanner', {
                defaultValue: '{{uses}} can be shared now and joined the list. Your earlier choices and name are unchanged.',
        ...
        <Surface tone="card" className="space-y-4 p-4 lg:sticky lg:top-4" data-testid="runtime-profile-share-summary">
          <div className="space-y-1.5">
            <label className="block text-sm font-semibold text-[var(--nimi-text-primary)]" htmlFor="runtime-profile-export-name">
              {t('runtimeConfig.profiles.share.titleLabel', { defaultValue: 'Name (optional)' })}
            </label>
            <input
          ...
          {included.length > 0 ? (
            <div className="space-y-2">
              <p className="text-sm font-semibold text-[var(--nimi-text-primary)]">
                {t('runtimeConfig.profiles.share.includesTitle', { defaultValue: 'Sharing {{count}} use(s)', count: included.length })}
              </p>
              {included.map((loadout) => (
                <SummaryLine key={loadout.loadoutId}>
                  {useLabel(loadout.capabilityContract)} · {modelName(loadout)}
                </SummaryLine>
              ))}
            </div>
          ) : shareableCount > 0 ? (
            <p className="text-xs text-[var(--nimi-text-muted)]">
              {t('runtimeConfig.profiles.share.includesNone', { defaultValue: 'Tick the uses you want to share' })}
            </p>
          ) : null}
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-center text-xs text-[var(--nimi-text-muted)]">
              {t('runtimeConfig.profiles.share.exportHint', {
                ...
                action: t('runtimeConfig.profiles.importAction', { defaultValue: 'Import a profile' }),
              })}
            </p>
          </div>
          ...
          {exportFailure ? (
            <InlineAlert tone="danger" data-testid="runtime-profile-share-export-failed">
              ...
              <p className="mt-1 text-xs">{t('runtimeConfig.profiles.share.exportFailedBody', { defaultValue: 'You can try again.' })}</p>
              <details className="mt-2 text-xs">
                <summary className="cursor-pointer font-semibold">{t('runtimeConfig.profiles.technicalDetails', { defaultValue: 'Technical details' })}</summary>
...
function SummaryLine(props: { readonly children: ReactNode }) {
  return (
    <div className="flex items-start gap-2 text-xs leading-relaxed text-[var(--nimi-text-secondary)]">
        ...
        <div className="space-y-1 px-4 py-4">
          <p className="text-sm font-semibold text-[var(--nimi-text-primary)]">
            ...
            <span className="ml-1 text-xs font-normal text-[var(--nimi-text-muted)]">
              · {t('runtimeConfig.profiles.share.doneTitleTag', { defaultValue: 'name, written inside the file' })}
            </span>
          </p>
          <p className="text-xs text-[var(--nimi-text-muted)]">
