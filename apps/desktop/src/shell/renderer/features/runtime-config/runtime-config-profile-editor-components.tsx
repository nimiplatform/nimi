...
export type CapabilitySectionDef = {
  key: string;
  label: string;
  subtitle: string;
...
export const CAPABILITY_SECTIONS: CapabilitySectionDef[] = [
  ...
  { key: 'voice_clone', label: 'Voice clone workflow', subtitle: 'Custom voice generation', sdkCapability: 'voice_workflow.voice_clone', group: 'voice', iconColor: 'text-pink-600', iconBg: 'bg-pink-50', icon: CAP_ICONS.voiceClone, hasConfigPage: false },
  { key: 'voice_design', label: 'Voice design workflow', subtitle: 'Advanced audio parameters', sdkCapability: 'voice_workflow.voice_design', group: 'voice', iconColor: 'text-rose-600', iconBg: 'bg-rose-50', icon: CAP_ICONS.voiceDesign, hasConfigPage: false },
...
export const FIELD_BASE = 'w-full rounded-xl border border-[var(--nimi-border-subtle)] bg-white px-3 text-[13px] text-[var(--nimi-text-primary)] outline-none transition-colors hover:border-[var(--nimi-border-strong)] focus:border-[var(--nimi-field-focus)] focus:ring-2 focus:ring-mint-100';

export function EditorFieldLabel(props: { label: string }) {
...
export function TagsEditor(props: { tags: string[]; onChange: (tags: string[]) => void }) {
  const { t } = useTranslation();
...
export function CapabilityConfigPage(props: {
  capKey: string;
  capLabel: string;
  profileTitle: string;
  binding: RuntimeRouteBinding | null | undefined;
