...
export async function startSocialOauth(provider: SocialOauthProvider): Promise<SocialOauthResult> {
  const config = resolveSocialOauthConfig(provider);
  if (!config.enabled) {
  ...
  const listenTask = desktopBridge.oauthListenForCode({
    redirectUri: callbackUrl,
  });

  try {
    const launchResult = await desktopBridge.openExternalUrl(authorizeUrl);
    ...
    const exchange = await desktopBridge.oauthTokenExchange({
