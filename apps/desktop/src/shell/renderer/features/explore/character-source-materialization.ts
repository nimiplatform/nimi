...
function translate(t: TFunction, key: string, defaultValue: string): string {
  const translated = t(key, { defaultValue });
  return typeof translated === 'string' && translated.trim() ? translated : defaultValue;
}

export function characterSourceMaterializationMessage(t: TFunction): string {
  return translate(
    t,
...
export function characterSourceAmbiguousMessage(t: TFunction): string {
  return translate(
    t,
...
export function characterRuntimeUnavailableMessage(t: TFunction): string {
  return translate(
    t,
...
export function characterRuntimePendingMessage(t: TFunction): string {
  return translate(
    t,
...
export function characterSourceMaterializationRejectedMessage(t: TFunction): string {
  return translate(
    t,
...
export function characterSourceMaterializationFailureMessage(
  error: unknown,
  t: TFunction,
): string {
  ...
  if (normalized.includes('binding mismatch')
    ...
    return characterSourceMaterializationRejectedMessage(t);
  }
...
export function describeCharacterPrimaryAction(
  state: CharacterSourceState,
  t: TFunction,
): CharacterPrimaryActionLabel {
  if (state === 'source_materialization_unavailable') {
    return { state, action: state, label: translate(t, 'Explore.characterSourceUnavailable', 'Unavailable'), disabled: true, hint: characterSourceMaterializationMessage(t) };
  }
  if (state === 'runtime_agent_inventory_unavailable') {
    return { state, action: 'partner_runtime_unavailable', label: translate(t, 'Explore.characterRuntimeUnavailableLabel', 'Runtime unavailable'), disabled: true, hint: characterRuntimeUnavailableMessage(t) };
  }
  if (state === 'runtime_agent_inventory_pending') {
    return { state, action: 'partner_runtime_pending', label: translate(t, 'Explore.characterRuntimePendingLabel', 'Checking partner'), disabled: true, hint: characterRuntimePendingMessage(t) };
  }
  if (state === 'local_agent_ambiguous') {
    return { state, action: 'partner_ambiguous', label: translate(t, 'Explore.characterSourceAmbiguousLabel', 'Open from partners'), disabled: true, hint: characterSourceAmbiguousMessage(t) };
  }
  if (state === 'local_agent_available') {
    return { state, action: 'open_partner', label: translate(t, 'Explore.characterSourceOpenLocalAgent', 'Open partner'), disabled: false };
  }
  return { state, action: 'become_partner', label: translate(t, 'Explore.characterSourceMaterialize', 'Become my partner'), disabled: false };
}
