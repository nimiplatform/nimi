...
async function requestObject(spec: Record<string, unknown>): Promise<Record<string, unknown>> {
  const payload = await withRuntimeOpenApiContext(() => openApiRequest<unknown>(OpenAPI, spec as never));
  return toObjectOr(payload, {});
}

async function requestObjectOrNull(spec: Record<string, unknown>): Promise<Record<string, unknown> | null> {
  try {
    const payload = await withRuntimeOpenApiContext(() => openApiRequest<unknown>(OpenAPI, spec as never));
