...
function parseChatAiMessagePart(value: unknown, errorPrefix: string): ChatAiMessagePart {
  ...
  return {
    type: 'text',
    text: parseString(record.text, 'text', errorPrefix),
  };
}
...
export function parseChatAiMessageRecord(value: unknown): ChatAiMessageRecord {
  ...
  return {
    ...
    contentText: parseString(record.contentText, 'contentText', 'chat_ai message record'),
...
export function parseChatAiDraftRecord(value: unknown): ChatAiDraftRecord {
  ...
  return {
    ...
    text: parseString(record.text, 'text', 'chat_ai draft record'),
...
export function parseChatAiCreateMessageInput(value: unknown): ChatAiCreateMessageInput {
  ...
  return {
    ...
    contentText: parseString(record.contentText, 'contentText', 'chat_ai create_message payload'),
    content: parseChatAiMessageContent(record.content),
    error: record.error == null ? null : parseChatAiMessageError(record.error),
...
export function parseChatAiUpdateMessageInput(value: unknown): ChatAiUpdateMessageInput {
  ...
  return {
    ...
    contentText: parseString(record.contentText, 'contentText', 'chat_ai update_message payload'),
    content: parseChatAiMessageContent(record.content),
    error: record.error == null ? null : parseChatAiMessageError(record.error),
    traceId: parseNullableStringValue(record.traceId),
...
export function parseChatAiPutDraftInput(value: unknown): ChatAiPutDraftInput {
  ...
  return {
    ...
    text: parseString(record.text, 'text', 'chat_ai put_draft payload'),
    attachments: Array.isArray(record.attachments)
...
function parseRequiredString(value: unknown, fieldName: string, errorPrefix: string): string {
  if (typeof value !== 'string') {
    throw new Error(`${errorPrefix}: ${fieldName} must be a string`);
  }
  const normalized = value.trim();
  if (!normalized) {
    throw new Error(`${errorPrefix}: ${fieldName} must not be empty`);
  }
  return normalized;
}
...
function parseOptionalString(value: unknown): string | undefined {
  ...
  if (typeof value !== 'string') {
    throw new Error('chat_ai optional string field must be a string');
  }
  return value.trim() || undefined;
}

function parseString(value: unknown, fieldName: string, errorPrefix: string): string {
  if (typeof value !== 'string') {
    throw new Error(`${errorPrefix}: ${fieldName} must be a string`);
  }
  return value;
