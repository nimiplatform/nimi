...
6. `tab-page` UI mod route host 必须提供 `h-full + min-h-0 + overflow-hidden` 的全高容器，保证 mod 内部滚动面只在 mod 自己的 shell 内发生，而不是泄漏到 host 页面。
...
2. 运行 `pnpm -C apps/desktop run smoke:mods` 时，只校验 runtime mods 目录中的已安装包，并验证 manifest 声明的 `entry`/`styles[]` 资产完整性。
3. 运行 `pnpm -C apps/desktop run smoke:mod:installed` 时，可验证任意单个已安装 mod 的 manifest、entry 与声明样式资产完整性。
...
| `UI runtime mod manifest styles missing` | UI mod 未声明 `styles[]` | 在 mod manifest 中补齐 `styles` 并重新构建安装包 |
