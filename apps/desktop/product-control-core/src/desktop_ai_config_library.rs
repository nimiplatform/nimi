...
//!
//! This Desktop Electron host module owns first-run built-in `AIConfig`
...
pub fn runtime_capability_bindings_from_execution_evidence_ref(
    ...
    let trim = |value: &str| value.trim().to_string();
    ...
    if trim(&evidence.runtime_baseline_ref).is_empty() {
    ...
    for proof in &evidence.selected_baseline_capability_proof {
        ...
        let capability = match scenario {
            ScenarioType::TextGenerate => "text.generate",
            ScenarioType::SpeechTranscribe => "audio.transcribe",
            ScenarioType::SpeechSynthesize => "audio.synthesize",
            _ => {
                return Err(format!(
        ...
        if trim(&proof.local_route_target).is_empty() {
            return Err(format!(
