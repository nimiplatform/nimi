...
fn derive_and_commit_library_projection(
    ...
    let normalized_account = validate_account_id(account_id)?;
    let _library_dir = account_profile_library_dir(data_root, &normalized_account)?;
    ...
    Ok(AccountProfileLibraryProjection {
        library_ref: format!("account-profile-library:{normalized_account}"),
