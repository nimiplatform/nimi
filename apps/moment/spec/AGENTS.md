...
8. Future integrations with neighboring apps must remain explicit downstream or upstream hooks. They must not collapse `Moment` into a wrapper around another product.
...
- Retired workspace products live in Git history only and must not be referenced as active upstream/downstream products.
