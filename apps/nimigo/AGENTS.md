# NimiGo App Instructions

## Scope
- `apps/nimigo/**` is a Nimi root-workspace App. Root AGENTS.md, dependency installation, lockfile and Nimi-coding guidance apply.
- Product code lives in `src/product/`. Electron and renderer startup code is maintained in this repository, not by an independent scaffold sync.
...
## Verification
- From the repository root, run `pnpm --filter @nimiplatform/nimigo typecheck` and `pnpm --filter @nimiplatform/nimigo test` for affected behavior.
