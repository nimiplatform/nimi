...
/** App business state only. Runtime Conversation and Agent identity are never copied here. */
export class GoStore {
  private writes = Promise.resolve();
  private workspaceId: string | null = null;
  constructor(private client: NimiLocalAppClient) {}
  async load(): Promise<{ workspace: Workspace; works: Work[] }> {
    ...
    catch (error) { if (!isMissing(error)) throw error; workspace = createWorkspace(); await this.client.storage.writeJson(`${root}/workspace.json`, workspace as unknown as JsonValue); }
  ...
  async readText(path: string): Promise<string> {
    ...
    const chunks: Uint8Array[] = []; for await (const chunk of result.body) chunks.push(chunk);
    return new TextDecoder().decode(await new Blob(chunks.map(c => new Uint8Array(c))).arrayBuffer());
  }
}
