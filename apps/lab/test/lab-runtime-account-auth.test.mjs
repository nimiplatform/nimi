...
async function importLabRuntime() {
  return import(pathToFileURL(path.join(buildModule(), 'lab/lab-runtime.js')).href);
}
...
test('Lab Electron lifecycle has no protected-session termination coupling', () => {
...
test('Lab presents typed unavailable posture without terminating its App carrier', async () => {
...
test('Lab names a signed-out account without reporting carrier distrust', async () => {
  const previousElectronTest = globalThis.__NIMI_ELECTRON_TEST__;
  globalThis.__NIMI_ELECTRON_TEST__ = {
...
test('Lab preserves a bound identity session without treating it as App Access', async () => {
  const previousElectronTest = globalThis.__NIMI_ELECTRON_TEST__;
  const calls = [];
  globalThis.__NIMI_ELECTRON_TEST__ = {
    async invoke(command, payload) {
...
test('Lab app-private storage reaches typed ingress and preserves owner-unavailable', async () => {
  const previousElectronTest = globalThis.__NIMI_ELECTRON_TEST__;
  const calls = [];
  globalThis.__NIMI_ELECTRON_TEST__ = {
    async invoke(command, payload) {
...
test('Lab reports typed owner unavailability after protected ingress', async () => {
  const previousElectronTest = globalThis.__NIMI_ELECTRON_TEST__;
  const calls = [];
  globalThis.__NIMI_ELECTRON_TEST__ = {
    async invoke(command, payload) {
...
test('Lab adopts every returned video artifact including the requested last frame', async () => {
  const { runLabCapability } = await importLabRuntime();
  const adoptionCalls = [];
  const client = fakeLocalAppClient({
    async adoptArtifact(input) {
...
test('Lab forwards the user cancellation signal to the video ScenarioJob runner', async () => {
  const { runLabCapability } = await importLabRuntime();
...
test('Lab preserves caller-local operation-aborted without fabricating a Runtime cancellation', async () => {
  const { runLabCapability } = await importLabRuntime();
  const client = fakeLocalAppClient();
  const result = await runLabCapability({
    capabilityId: 'video.generate',
...
test('Lab adopts metadata-only video without reading the source artifact body', async () => {
  const { runLabCapability } = await importLabRuntime();
  let readCalls = 0;
  const client = fakeLocalAppClient({
    async readArtifact() {
...
test('Lab reports non-success when adoption fails and never falls back to artifact read', async () => {
  const { runLabCapability } = await importLabRuntime();
  let readCalls = 0;
  const client = fakeLocalAppClient({
...
test('Lab text.generate projects the protected Local App candidate happy path without sampling fallbacks', async () => {
  const { runLabCapability } = await importLabRuntime();
  const calls = [];
  const client = fakeLocalAppClient({
  ...
  const result = await runLabCapability({ capabilityId: 'text.generate', prompt: 'hello' }, readyRuntimeDependencies(client));
...
test('Lab chat.stream runs the Kit streaming face and forwards accumulated partials', async () => {
  const { runLabCapability } = await importLabRuntime();
  const streamInputs = [];
  const partials = [];
  const client = fakeLocalAppClient({
...
test('Lab voice.create submits reference audio, waits for the Job, and verifies the VoiceAsset catalog projection', async () => {
  const { runLabCapability } = await importLabRuntime();
...
test('Lab voice.create submits a text description through the same canonical contract', async () => {
  const { runLabCapability } = await importLabRuntime();
...
test('Lab speech.bundle runs the Kit voice catalog over the Local App list client', async () => {
  const { runLabCapability } = await importLabRuntime();
  const calls = [];
  const client = fakeLocalAppClient({
    async listVoiceAssets(input) {
...
for (const failureCase of TYPED_FAILURE_CASES) {
  test(`Lab ${failureCase.capabilityId} preserves a typed failure projection`, async () => {
    const { runLabCapability } = await importLabRuntime();
    ...
    const result = await runLabCapability({ capabilityId: failureCase.capabilityId, prompt: failureCase.prompt, parameters: failureCase.parameters }, dependencies);
    assert.equal(result.ok, false);
