...
export type Recipe = {
  ...
  wide?: boolean;
  stage: ReactNode | ((context: KitRecipeExampleContext) => ReactNode);
  snippet: string | ((context: KitRecipeExampleContext) => string);
  props: Array<{ name: string; desc: string }>;
  accessChecks?: string[];
  tokenFootprint?: Array<{ token: string; role: string; source: string }>;
};

export type KitRecipeExampleContext = {
...
export const RECIPES: Recipe[] = [
  ...
  // Inputs
  {
    ...
    stage: ({ appId }) => (
      <div className="kit-fields-recipe grid w-full min-w-0 max-w-sm gap-3">
        <FieldShell label="App identity">
          <TextField defaultValue={appId} leading={<Search size={14} />} />
        </FieldShell>
    ...
    snippet: ({ appId }) => `<div className="kit-fields-recipe grid w-full min-w-0 max-w-sm gap-3">
  <FieldShell label="App identity">
    <TextField defaultValue="${appId}" leading={<Search size={14} />} />
  </FieldShell>
  <FieldShell label="Capability">
