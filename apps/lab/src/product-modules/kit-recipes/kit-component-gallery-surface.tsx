...
import {
  ...
  TYPE_ROLES,
  type CategoryId,
  type KitRecipeExampleContext,
  type Recipe,
} from './kit-component-gallery-recipes.js';
import type { KitRecipesCopyText } from './gallery.js';
...
function recipeCopyText(recipe: Recipe, context: KitRecipeExampleContext): string {
  const snippet = typeof recipe.snippet === 'function' ? recipe.snippet(context) : recipe.snippet;
  return `${recipeImportBlock(recipe)}\n\n${snippet}`;
}

function RecipePreview({ recipe, context, compact = false }: { recipe: Recipe; context: KitRecipeExampleContext; compact?: boolean }) {
  const stage = typeof recipe.stage === 'function' ? recipe.stage(context) : recipe.stage;
  return (
    <div
      className={[
...
function RecipeTile({
  ...
  recipe: Recipe;
  context: KitRecipeExampleContext;
  active: boolean;
  onInspect: () => void;
}) {
  return (
    ...
    >
      ...
      <RecipePreview recipe={recipe} context={context} compact />
    </AppCardSurface>
  );
}
  ...
  return (
    <div ref={panelRef} className="min-w-0">
      ...
      >
        ...
        <section className="grid min-w-0 gap-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <IconButton
              ...
              onClick={() => void copyText(importBlock)}
            />
          </div>
        ...
        <Button tone="primary" fullWidth leadingIcon={<Copy size={15} aria-hidden="true" />} onClick={() => void copyText(recipeCopyText(recipe, context))}>
...
export function RecipeWorkspace({ recipes, copyText, exampleAppId }: { recipes: Recipe[]; copyText: KitRecipesCopyText; exampleAppId: string }) {
