...
import { forgetMusicRecovery, readMusicRecovery, type MusicRecoveryEntry } from './music-recovery.js';

export function MusicRecoveryPanel({ disabled }: { readonly disabled: boolean }) {
  ...
  async function refresh() { setEntries(await readMusicRecovery(host.sdk.storage)); }
  useEffect(() => {
    let active = true;
    const load = () => void readMusicRecovery(host.sdk.storage).then((value) => { if (active) setEntries(value); })
  ...
  async function recover(entry: MusicRecoveryEntry) {
    ...
    try {
      const next = await host.sdk.runCapability({ capabilityId: 'music.generate', prompt: '', signal: controller.signal,
  ...
  return <section className="space-y-3 border-t pt-4" aria-label={t('Music.recoveryTitle')}>
    ...
    <div className="max-h-48 space-y-2 overflow-y-auto">
      {[...entries].reverse().map((entry) => <div key={entry.clientSubmissionId} className="flex flex-wrap items-center gap-2">
        ...
        <Button disabled={disabled || busy} onClick={() => void forgetMusicRecovery(host.sdk.storage, entry.clientSubmissionId).then(refresh)
    ...
    {result?.ok && result.output.kind === 'artifacts' ? <div className="space-y-3">
      ...
      {result.output.artifacts.filter((artifact) => artifact.relativePath !== generatedScorePath).map((artifact) => <ArtifactMediaResult key={artifact.relativePath} artifact={artifact} fallbackLabel={artifact.relativePath} />)}
    </div> : null}
  </section>;
}
