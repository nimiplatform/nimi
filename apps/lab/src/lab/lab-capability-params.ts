...
export type LabCapabilityParamRoute = 'local' | 'cloud';
export type LabCapabilityParamRouteState =
  | { readonly kind: 'supported' }
  | { readonly kind: 'unsupported' }
  | { readonly kind: 'fixed'; readonly value: string | number };

export type LabCapabilityParamPresentation = {
...
type ParamRouteMatrixEntry = Readonly<Record<LabCapabilityParamRoute, LabCapabilityParamRouteState>>;
...
/**
 * Lab presentation matrix for the effective Local App route, not a
 ...
 */
export const LAB_CAPABILITY_PARAM_ROUTE_MATRIX = {
  ...
  // camera/watermark/draft/tier/expiry remain typed rejects; Cloud keeps the full lab surface.
  'video.generate': {
  ...
  'speech.bundle': {},
  'world.generate': {},
} as const satisfies Record<LabCapabilityId, CapabilityParamRouteMatrix>;

export function getLabCapabilityParamPresentation(
  capabilityId: LabCapabilityId,
  source: LabRunTargetSource,
): readonly LabCapabilityParamPresentation[] {
  const matrix = LAB_CAPABILITY_PARAM_ROUTE_MATRIX[capabilityId] as CapabilityParamRouteMatrix;
...
export function projectLabCapabilityParamsForRoute<
