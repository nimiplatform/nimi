...
import { nonEmptyEmbeddingInputs, summarizeLabCapabilityParameters, type LabEmbeddingParameters, type LabSpeechTranscribeParameters, type LabVoiceCreateParameters } from '../lab-capability-parameters.js';
import { projectLabCapabilityParamsForRoute } from '../lab-capability-params.js';
import { getLabRunIntentLabel, restoreLabCapabilityRunResult, type LabRunConfigSnapshot, type LabRunHistory, type LabRunHistoryRecord } from '../lab-history.js';
...
function TextStudioShell({
  ...
  const parameterStore = useContext(LabCapabilityParameterContext);
  ...
  const attachmentAdapter = useMemo(
    () => createBrowserDataUrlAttachmentAdapter({ idPrefix: 'lab-attachment' }),
    [],
  );
  ...
  const currentResult = activeRun
    ? activeRun.result ?? (activeRun.record ? restoreLabCapabilityRunResult(activeRun.record) : null)
    : lastResult?.capabilityId === capability.id ? lastResult : null;
  const headerResult = hasActiveRun ? currentResult : null;
  const runTarget = useLabRunTargetSummary(capability, runtime);
  ...
  const effectiveCapabilityParameters = useMemo(() => projectLabCapabilityParamsForRoute(
    capability.id,
    runTarget.source,
    capabilityParameters,
  ), [capability.id, capabilityParameters, runTarget.source]);
  const parameterSummary = summarizeLabCapabilityParameters(capability.id, effectiveCapabilityParameters);
  const hasAlternativeInput = capability.id === 'text.embed'
    ? nonEmptyEmbeddingInputs(capabilityParameters as LabEmbeddingParameters).length > 0
    : capability.id === 'audio.transcribe'
      ? Boolean((capabilityParameters as LabSpeechTranscribeParameters).audioFile)
      : capability.id === 'voice.create'
        ? Boolean((capabilityParameters as LabVoiceCreateParameters).referenceAudioFile)
      : false;
  ...
  useEffect(() => {
    abortControllerRef.current?.abort('lab-capability-changed');
  ...
  async function run(nextPrompt = prompt, nextContext = context) {
    ...
    setCancelRequested(false);
    try {
      let result: LabCapabilityRunResult;
      try {
        if (isWorldTour) {
  ...
  function selectHistoryRun(record: LabRunHistoryRecord) {
  ...
  function useHistoryRunAsDraft(record: LabRunHistoryRecord) {
  ...
  function handleCancel() {
    ...
    abortControllerRef.current?.abort('lab-user-canceled');
  }
          ...
          <main className="studio__stage">
            {hasActiveRun && activeRun ? (
              <TextStudioResultState
                ...
                activeRun={activeRun}
                admission={admission}
                intentLabel={activeRun.record ? getLabRunIntentLabel(activeRun.record) : runTarget.intentLabel}
                running={running}
...
export function SectionAITesting({
  ...
  return (
    <div
      ...
      data-testid="nimi-lab-section-ai-testing"
