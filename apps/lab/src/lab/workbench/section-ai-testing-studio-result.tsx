...
import { formatLabRunTimestamp } from '../lab-history.js';
import type { LabCapabilityRunResult } from '../lab-runtime.js';
import { nonSuccessReasonUserAction, nonSuccessReasonUserMessage } from '../lab-non-success.js';
...
function ReadyBody({ result }: { result: LabCapabilityRunResult & { ok: true } }) {
...
// Runtime details disclosure - preserves the developer-lab diagnostic surface
...
function studioResultIntentLabel(result: LabCapabilityRunResult | null, capability: LabCapability, preferredLabel: string | undefined, translate: (key: string) => string): string {
...
export function StudioResult({
  ...
  const runTimeLabel = createdAt
    ? formatLabRunTimestamp(createdAt, new Date(rendererHost.clock.now()))
