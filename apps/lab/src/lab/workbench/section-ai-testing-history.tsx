...
import { labCapabilities, type LabCapabilityId } from '../lab-capabilities.js';
import type { LabImageHistoryRecord } from '../lab-image-history.js';
import type { LabHistoryPanelScope } from '../lab-preferences.js';
import {
  flattenLabRunHistory,
  formatLabRunHistoryTimestamp,
  getLabRunMetricSummary,
  getLabRunIntentLabel,
  getLabRunIntentSource,
  getLabRunPromptSummary,
  getLabRunResultSummary,
  getLabRunStatusTone,
  type LabFlatRunRecord,
  type LabRunHistory,
  type LabRunHistoryRecord,
} from '../lab-history.js';
import { capabilityIcons } from './capability-icons.js';
import { LabHistoryActionsContext, LabHistoryLoadContext, LabHistoryPanelContext } from './workbench-context.js';
import { useLabRendererHost } from '../../renderer/context.js';

type HistoryStatusFilter = 'all' | LabRunHistoryRecord['status'];
...
const runtimeHistoryCapabilities = Object.freeze(
  labCapabilities.filter((item) => item.execution === 'runtime-sdk' || item.execution === 'standalone-tauri'),
);
...
// Local run history recovered from the desktop lab history panel. Reads only
// the app-owned history store; it does not claim Runtime or Realm truth.
function historyToneForRun(record: LabRunHistoryRecord): 'success' | 'warning' | 'info' | 'neutral' {
  const tone = getLabRunStatusTone(record.status);
...
function historyTitleForRun(record: LabRunHistoryRecord): string {
  const prompt = getLabRunPromptSummary(record).trim();
  return prompt || getLabRunResultSummary(record);
}

function historyFailureReasonForRun(record: LabRunHistoryRecord): string {
  const result = record.result;
  if (result && result.ok === false) {
    return result.reason || result.summary || record.message;
  }
  return getLabRunResultSummary(record);
}
...
function matchesHistoryStatus(record: LabRunHistoryRecord, status: HistoryStatusFilter): boolean {
  if (status === 'all') return true;
  return record.status === status;
}

function isFailureHistoryStatus(record: LabRunHistoryRecord): boolean {
  return record.status === 'failed' || record.status === 'unavailable';
}

function matchesHistorySearch(record: LabRunHistoryRecord, query: string): boolean {
  if (!query) return true;
  const haystacks = [
    record.prompt,
    record.message,
    record.result?.summary ?? '',
    getLabRunIntentLabel(record),
  ];
  return haystacks.some((value) => value.toLowerCase().includes(query));
}

function matchesMediaSearch(record: LabImageHistoryRecord, query: string): boolean {
  if (!query) return true;
  return [record.title, record.capabilityLabel ?? '', record.artifactLabel ?? '', record.message ?? '']
    .some((value) => value.toLowerCase().includes(query));
}

function matchesHistoryEnvironment(record: LabRunHistoryRecord, environment: HistoryEnvironmentFilter): boolean {
  if (environment === 'all') return true;
  const source = getLabRunIntentSource(record);
...
function matchesHistoryActivity(record: Pick<LabRunHistoryRecord, 'createdAt'>, activity: HistoryActivityFilter, now: Date): boolean {
...
export function CapabilityRunHistory({
  ...
  onSelectRun: (record: LabRunHistoryRecord) => void;
  collapsed: boolean;
  currentCapabilityId: LabCapabilityId;
}) {
  const rendererHost = useLabRendererHost();
  const { t, i18n } = useTranslation();
  const historyLoad = useContext(LabHistoryLoadContext);
  const historyActions = useContext(LabHistoryActionsContext);
  const historyPanel = useContext(LabHistoryPanelContext);
  const locale = i18n.language.startsWith('zh') ? 'zh-CN' : 'en-US';
  const [filterOpen, setFilterOpen] = useState(false);
  const [activeMenu, setActiveMenu] = useState<HistoryFilterMenuId | null>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [clearArmed, setClearArmed] = useState(false);
  const scope: LabHistoryPanelScope = historyPanel?.scope ?? 'capability';
  const hideFailures = historyPanel?.hideFailures ?? false;
  const setScope = (next: LabHistoryPanelScope) => {
    setClearArmed(false);
    historyPanel?.setScope(next);
  };
  const setHideFailures = (next: boolean) => historyPanel?.setHideFailures(next);
  const [statusFilter, setStatusFilter] = useState<HistoryStatusFilter>('all');
  const [capabilityFilter, setCapabilityFilter] = useState<LabCapabilityId | 'all'>('all');
  ...
  const records = useMemo(() => flattenLabRunHistory(history)
  ...
  function handleSelectMediaRecord(record: LabImageHistoryRecord) {
    const linkageId = record.runId || record.id;
    const runRecord = flattenLabRunHistory(history).find((entry) => entry.id === linkageId);
    if (!runRecord) {
  ...
  function handleRemoveRecord(record: LabRunHistoryRecord) {
    void historyActions?.removeRecord(record.id);
  }

  function handleRemoveMediaRecord(record: LabImageHistoryRecord) {
  ...
  function historySourceLabelForRun(record: LabRunHistoryRecord): string {
    const source = getLabRunIntentSource(record);
  ...
  function historySubtitleForRun(record: LabRunHistoryRecord): string {
    const intent = getLabRunIntentLabel(record);
  ...
  function historyLabelForRun(record: LabFlatRunRecord): string {
    const prompt = historyTitleForRun(record);
    const intent = getLabRunIntentLabel(record);
    const source = getLabRunIntentSource(record);
    const metrics = getLabRunMetricSummary(record);
    return [
      ...
      historyCapabilityLabel(record.capabilityId as LabCapabilityId),
      formatLabRunHistoryTimestamp(record.createdAt, now),
      metrics,
    ].filter(Boolean).join(' / ');
  }
  ...
  function historyCapabilityLabel(id: LabCapabilityId | 'all'): string {
  ...
  const groups = useMemo(() => {
    ...
    const grouped = new Map<string, { id: string; label: string; records: LabFlatRunRecord[] }>();
    for (const record of records) {
      const label = groupBy === 'date' ? historyDateGroupLabel(record.createdAt) : historyCapabilityLabel(record.capabilityId as LabCapabilityId);
                      ...
                      <span className="studio-recent__copy">
                        <span className="studio-recent__summary">
                          <span className="studio-recent__title">
                            ...
                            <time dateTime={record.createdAt}>{formatLabRunHistoryTimestamp(record.createdAt, now)}</time>
                          </span>
                        </span>
