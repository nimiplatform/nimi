...
import { formatLabRunTimestamp, getLabRunConfigParamRows, getLabRunIntentLabel, getLabRunPromptControlFacts, getLabRunResultTags, getLabRunStatusTone, type LabRunConfigParamRow, type LabRunHistoryRecord, type LabRunHistoryResultSnapshot, type LabRunPromptControlFact } from '../lab-history.js';
import { nonSuccessReasonTitle, nonSuccessReasonUserAction, nonSuccessReasonUserMessage } from '../lab-non-success.js';
...
// projection in lab-history.ts stays locale-agnostic.
const RUN_STATUS_LABEL_KEY: Record<LabRunHistoryRecord['status'], string> = {
...
function TextStudioPromptControlFacts({ facts }: { facts: readonly LabRunPromptControlFact[] }) {
...
function TextStudioPromptSettings({ activeRun }: { activeRun: TextStudioActiveRun }) {
  ...
  const facts = runConfig ? getLabRunPromptControlFacts(runConfig) : [];
...
function groupParamRows(rows: readonly LabRunConfigParamRow[]): Array<{ group: string; rows: LabRunConfigParamRow[] }> {
  const groups: Array<{ group: string; rows: LabRunConfigParamRow[] }> = [];
...
function TextStudioRequestSettings({ record }: { record: LabRunHistoryRecord }) {
  const { t } = useTranslation();
  ...
  const paramRows = getLabRunConfigParamRows(runConfig);
...
function hasTextStudioRequestSettings(record: LabRunHistoryRecord): boolean {
  const runConfig = record.runConfig;
  if (!runConfig) return false;
  return getLabRunConfigParamRows(runConfig).length > 0 || Boolean(runConfig.target.paramsSummary.join(' / '));
}

function historyRecordPlainText(record: LabRunHistoryRecord): string {
...
function historyNonSuccessDiagnosticsText(snapshot: Extract<LabRunHistoryResultSnapshot, { ok: false }>): string {
  return [
...
function historyResultToneClass(record: LabRunHistoryRecord): string {
...
function TextStudioHistoryRecordResult({
  ...
  onRegenerate: () => void;
  onUseAsDraft: (record: LabRunHistoryRecord) => void;
}) {
  const rendererHost = useLabRendererHost();
  const { t } = useTranslation();
  const snapshot = record.result;
  const blocked = snapshot && !snapshot.ok ? snapshot : null;
  const tags = blocked ? [nonSuccessReasonTitle(blocked.reason)] : getLabRunResultTags(record);
  const intentLabel = getLabRunIntentLabel(record);
    ...
    <div className="studio-history-result" role="status">
      <div className="studio-history-result__head">
        <div className="studio-history-result__line">
          <StatusBadge tone={getLabRunStatusTone(record.status)} shape="dot">
          ...
          <span className="studio-history-result__title-stack">
            <time dateTime={record.createdAt}>{t('StudioShell.runLabel')} / {formatLabRunTimestamp(record.createdAt, new Date(rendererHost.clock.now()))}</time>
          </span>
        </div>
...
function TextStudioHistorySnapshotBody({ snapshot }: { snapshot: Extract<LabRunHistoryResultSnapshot, { ok: true }> }) {
  const { t } = useTranslation();
