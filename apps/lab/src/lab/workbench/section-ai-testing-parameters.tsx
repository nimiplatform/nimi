...
import {
  MAX_LAB_ARTIFACT_UPLOAD_BYTES,
  MAX_LAB_AUDIO_UPLOAD_BYTES,
  MAX_LAB_VOICE_REFERENCE_AUDIO_BYTES,
  type LabCapabilityParameterState,
  type LabEmbeddingParameters,
  type LabImageGenerationParameters,
  type LabSpeechSynthesizeParameters,
  type LabSpeechTranscribeParameters,
  type LabTextGenerationParameters,
  type LabVideoGenerationParameters,
  type LabVoiceCreateParameters,
} from '../lab-capability-parameters.js';
import { getLabCapabilityParamPresentation, projectLabCapabilityParamsForRoute } from '../lab-capability-params.js';
import type { LabRunTargetSource } from '../lab-run-target.js';

const LAB_IMAGE_MIME_TYPES = ['image/png', 'image/jpeg', 'image/webp', 'image/gif'] as const;
type LabImageMimeType = typeof LAB_IMAGE_MIME_TYPES[number];

function isLabImageMimeType(value: string): value is LabImageMimeType {
  return LAB_IMAGE_MIME_TYPES.includes(value as LabImageMimeType);
}

type ParameterPanelProps<TCapabilityId extends LabCapabilityId = LabCapabilityId> = {
  capabilityId: TCapabilityId;
  source: LabRunTargetSource;
  parameters: LabCapabilityParameterState[TCapabilityId];
  disabled: boolean;
  onChange: (parameters: LabCapabilityParameterState[TCapabilityId]) => void;
};
...
function RouteAwareParameterFields({
  ...
  const presentation = new Map(
    getLabCapabilityParamPresentation(capabilityId, source).map((item) => [item.field, item]),
  );
...
function TextGenerationFields(props: ParameterPanelProps<'text.generate' | 'chat.stream'>) {
  const { t } = useTranslation();
  const parameters = props.parameters as LabTextGenerationParameters;
  const update = props.onChange as (next: LabTextGenerationParameters) => void;
  const numberField = (
    field: keyof LabTextGenerationParameters,
    label: string,
...
function EmbeddingFields(props: ParameterPanelProps<'text.embed'>) {
  const { t } = useTranslation();
  const parameters = props.parameters as LabEmbeddingParameters;
...
function ImageFields(props: ParameterPanelProps<'image.generate'>) {
  ...
  const { t } = useTranslation();
  const parameters = props.parameters as LabImageGenerationParameters;
  const update = props.onChange as (next: LabImageGenerationParameters) => void;
  ...
  async function selectReferenceImage(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    if (file.size === 0 || file.size > MAX_LAB_ARTIFACT_UPLOAD_BYTES) {
      setUploadError(t('Studio.parameters.imageFileTooLarge'));
      return;
    }
    if (!isLabImageMimeType(file.type)) {
  ...
  const updateReferenceUrl = (next: LabImageGenerationParameters) => {
  ...
  const textField = (field: keyof LabImageGenerationParameters, label: string, placeholder?: string): RouteAwareParameterField => ({
    field,
    label,
...
function VideoFields(props: ParameterPanelProps<'video.generate'>) {
  const rendererHost = useLabRendererHost();
  const { t } = useTranslation();
  const parameters = props.parameters as LabVideoGenerationParameters;
  const update = props.onChange as (next: LabVideoGenerationParameters) => void;
  const inputRef = useRef<HTMLInputElement>(null);
  const [uploading, setUploading] = useState(false);
  const [uploadError, setUploadError] = useState('');
  const [uploadedFileName, setUploadedFileName] = useState('');
  ...
  const textField = (field: keyof LabVideoGenerationParameters, label: string, placeholder?: string): RouteAwareParameterField => ({
    field,
    label,
    render: (routeDisabled) => <TextParameter current={parameters} field={field} label={label} placeholder={placeholder} onChange={update} disabled={props.disabled || routeDisabled} />,
  });
  const numberField = (field: keyof LabVideoGenerationParameters, label: string, min?: number): RouteAwareParameterField => ({
    field,
    label,
  ...
  const booleanField = (field: keyof LabVideoGenerationParameters, label: string): RouteAwareParameterField => ({
    field,
    label,
      ...
      render: (routeDisabled) => (
        <ParameterField label={modeLabel}>
          <SelectField
            ...
            onValueChange={(value) => update({ ...parameters, mode: value as LabVideoGenerationParameters['mode'] })}
          />
        </ParameterField>
      ),
    },
...
function SpeechSynthesizeFields(props: ParameterPanelProps<'audio.synthesize'>) {
  const rendererHost = useLabRendererHost();
  const { t } = useTranslation();
  const parameters = props.parameters as LabSpeechSynthesizeParameters;
  const update = props.onChange as (next: LabSpeechSynthesizeParameters) => void;
  const effectiveVoiceKind = projectLabCapabilityParamsForRoute(
    'audio.synthesize',
    props.source,
    parameters,
  ).voiceKind;
  ...
  const textField = (field: keyof LabSpeechSynthesizeParameters, label: string): RouteAwareParameterField => ({
    field,
    label,
  ...
  const numberField = (field: keyof LabSpeechSynthesizeParameters, label: string, min?: number): RouteAwareParameterField => ({
    field,
    label,
      ...
      render: (routeDisabled) => (
        <ParameterField label={timingModeLabel}>
          <SelectField
            ...
            onValueChange={(timingMode) => update({ ...parameters, timingMode: timingMode as LabSpeechSynthesizeParameters['timingMode'] })}
          />
        </ParameterField>
      ),
    },
...
function SpeechTranscribeFields(props: ParameterPanelProps<'audio.transcribe'>) {
  const { t } = useTranslation();
  const parameters = props.parameters as LabSpeechTranscribeParameters;
  const update = props.onChange as (next: LabSpeechTranscribeParameters) => void;
  const inputRef = useRef<HTMLInputElement>(null);
  ...
  async function selectFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    event.currentTarget.value = '';
    if (!file) return;
    if (file.size > MAX_LAB_AUDIO_UPLOAD_BYTES) {
  ...
  const textField = (field: keyof LabSpeechTranscribeParameters, label: string): RouteAwareParameterField => ({
    field,
    label,
    render: (routeDisabled) => <TextParameter current={parameters} field={field} label={label} onChange={update} disabled={props.disabled || routeDisabled} />,
  });
  const booleanField = (field: keyof LabSpeechTranscribeParameters, label: string): RouteAwareParameterField => ({
    field,
    label,
    render: (routeDisabled) => <BooleanParameter current={parameters} field={field} label={label} onChange={update} disabled={props.disabled || routeDisabled} />,
  });
...
function VoiceCreateFields(props: ParameterPanelProps<'voice.create'>) {
  const { t } = useTranslation();
  const parameters = props.parameters as LabVoiceCreateParameters;
  const update = props.onChange as (next: LabVoiceCreateParameters) => void;
  const inputRef = useRef<HTMLInputElement>(null);
  const [fileError, setFileError] = useState('');
  ...
  async function selectFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    event.currentTarget.value = '';
    if (!file) return;
    if (file.size === 0 || file.size > MAX_LAB_VOICE_REFERENCE_AUDIO_BYTES) {
  ...
  const textField = (field: keyof LabVoiceCreateParameters, label: string, placeholder?: string): RouteAwareParameterField => ({
    field,
    label,
    render: (routeDisabled) => <TextParameter current={parameters} field={field} label={label} placeholder={placeholder} onChange={update} disabled={props.disabled || routeDisabled} />,
  });
      ...
      render: (routeDisabled) => (
        <ParameterField label={sourceLabel}>
          <SelectField
            ...
            onValueChange={(value) => update({ ...parameters, creationSource: value as LabVoiceCreateParameters['creationSource'] })}
          />
        </ParameterField>
      ),
    },
