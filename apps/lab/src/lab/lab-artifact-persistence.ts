...
export type LabPersistableArtifactResult = LabArtifactPersistenceCandidate & {
  ok: true;
  output: {
    kind: 'artifacts';
    artifactCount: number;
...
export function shouldPersistLabArtifactRecord(
...
export function settleLabHistorySaveIssueAfterPersistedRun<T extends { id: string }>(
  issue: {
    message: string;
    records: readonly T[];
...
export async function persistLabRunHistoryWithArtifactCompensation<T>(
  result: LabArtifactPersistenceCandidate,
  persist: () => Promise<T>,
  removeAsset: (relativePath: string) => Promise<unknown>,
): Promise<
...
> {
  try {
    ...
    const cleanup = await cleanupLabManagedArtifacts(result, removeAsset);
