...
import { getLabCapability, type LabCapability, type LabCapabilityId } from './lab-capabilities.js';
import {
  MAX_LAB_AUDIO_UPLOAD_BYTES,
  MAX_LAB_VOICE_REFERENCE_AUDIO_BYTES,
  nonEmptyEmbeddingInputs,
  type LabCapabilityParameters,
...
import {
  ...
  type LabNonSuccess,
  type LabNonSuccessDiagnostics,
  type LabNonSuccessReason,
} from './lab-non-success.js';
...
export type LabTypedOutput =
  ...
  | { kind: 'artifacts'; jobId: string; jobState: string; artifactCount: number; artifacts: LabManagedArtifact[]; firstArtifact?: LabManagedArtifact }
...
export type LabManagedArtifact = {
  relativePath: string;
  mediaType?: string;
  sizeBytes: number;
  sha256: string;
...
export type LabTypedSuccess = {
  ok: true;
  capabilityId: LabCapabilityId;
  capabilityLabel: string;
  message: string;
  output: LabTypedOutput;
  trace?: LabTrace;
};

export type LabRuntimeInspection =
  | {
      status: 'simulated';
      mode: 'simulated';
      detail: string;
...
export type LabCapabilityRunInput = {
  capabilityId: LabCapabilityId;
  prompt: string;
  scenarioId?: string;
...
export type LabCapabilityRunResult = LabTypedSuccess | LabNonSuccess;

export async function inspectRuntimeConnection(): Promise<LabRuntimeInspection> {
...
export type LabRuntimeDependencies = {
  ...
  readonly runners?: Partial<LabRuntimeRunnerSet>;
};

const LAB_RUNTIME_RUNNERS: LabRuntimeRunnerSet = Object.freeze({
...
export async function runLabCapability(
  input: LabCapabilityRunInput,
  dependencies: LabRuntimeDependencies = {},
): Promise<LabCapabilityRunResult> {
  ...
  const embeddingInputs = capability.id === 'text.embed'
    ? nonEmptyEmbeddingInputs(input.parameters as LabEmbeddingParameters | undefined)
    : [];
  const transcribeParameters = capability.id === 'audio.transcribe'
    ? input.parameters as LabSpeechTranscribeParameters | undefined
    : undefined;
  const voiceCreateParameters = capability.id === 'voice.create'
    ? input.parameters as LabVoiceCreateParameters | undefined
    : undefined;
  ...
  if (transcribeParameters?.audioFile && transcribeParameters.audioFile.sizeBytes > MAX_LAB_AUDIO_UPLOAD_BYTES) {
  ...
  const client = (dependencies.getLocalAppClient ?? getLabLocalAppClient)();
  const runners = { ...LAB_RUNTIME_RUNNERS, ...dependencies.runners };
  const scenarioId = input.scenarioId?.trim() || `lab:${capability.id}`;

  try {
    switch (capability.id) {
      case 'text.generate': {
        const parameters = input.parameters as LabTextGenerationParameters | undefined;
      ...
      case 'chat.stream': {
        const result = await runners.aiConsume({
          ...
          ...(input.parameters ? { parameters: textGenerationParameters(input.parameters as LabTextGenerationParameters) } : {}),
          scenarioId,
      ...
      case 'image.generate': {
        const parameters = input.parameters as LabImageGenerationParameters | undefined;
        const result = await runners.imageGenerate({
          runtime: { ai: createLabScenarioJobClient(client, dependencies) },
          appId,
          prompt,
          ...
          scenarioId,
          surfaceId: LAB_RUNTIME_SURFACE_ID,
          ...(input.signal ? { signal: input.signal, abortReason: 'lab-user-canceled' } : {}),
        });
        return await projectArtifactRunnerResult(capability, result, client);
      }
      case 'video.generate': {
        const parameters = input.parameters as LabVideoGenerationParameters | undefined;
      ...
      case 'audio.synthesize': {
        const parameters = input.parameters as LabSpeechSynthesizeParameters | undefined;
      ...
      case 'voice.create': {
        ...
        if (creationSource === 'reference-audio') {
          ...
          if (!audio || audio.sizeBytes === 0 || audio.sizeBytes > MAX_LAB_VOICE_REFERENCE_AUDIO_BYTES || !audio.mimeType.startsWith('audio/')) {
            return capabilityNonSuccess(
              capability,
              'input-invalid',
        ...
        const terminalResult = await runNimiRuntimeScenarioJob({
          ai: createLabScenarioJobClient(client, dependencies),
          request: {
            head: undefined,
            ...
            labels: { scenarioId, surfaceId: LAB_RUNTIME_SURFACE_ID },
            extensions: [],
          },
          ...(input.signal ? { signal: input.signal, abortReason: 'lab-user-canceled' } : {}),
        });
    ...
    return capabilityNonSuccess(
      capability,
      labNonSuccessReason(runtimeScenarioJobNonSuccessReasonFromError(error)),
      labRuntimeErrorMessage(error),
      labNonSuccessDiagnostics(error),
    );
  }
}

function createLabScenarioJobClient(
...
async function projectArtifactRunnerResult(
  ...
  if (result.ok === false) return projectRunnerNonSuccess(capability, result);
  const artifacts: LabManagedArtifact[] = [];
  ...
  try {
    ...
    for (const relativePath of [...adoptedPaths].reverse()) {
      try {
        ...
        cleanupFailures.push(`${relativePath}: ${labRuntimeErrorMessage(cleanupError)}`);
      }
    }
    if (cleanupFailures.length > 0) {
      throw new Error(`${labRuntimeErrorMessage(error)} Managed artifact cleanup also failed: ${cleanupFailures.join('; ')}`);
    }
    throw error;
  }
  return {
...
async function managedAssetPath(capabilityId: LabCapabilityId, jobId: string, artifactIndex: number): Promise<string> {
...
function projectRunnerNonSuccess(
  ...
  return capabilityNonSuccess(
    capability,
    labNonSuccessReason(result.reason),
    result.message,
    labNonSuccessDiagnostics(result.error),
  );
}

function labNonSuccessReason(reason: string): LabNonSuccessReason {
...
function textGenerationParameters(parameters: LabTextGenerationParameters | undefined): LabTextGenerationParameters {
...
function videoGenerationOptions(parameters: LabVideoGenerationParameters | undefined) {
...
function speechVoiceReference(parameters: LabSpeechSynthesizeParameters | undefined) {
...
function labNonSuccessDiagnostics(error: unknown): LabNonSuccessDiagnostics | undefined {
...
function labRuntimeErrorMessage(error: unknown): string {
