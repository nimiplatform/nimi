...
mod tests {
    ...
    #[test]
    fn lab_consumes_shared_platform_catalog_from_kit() {
        let speech_profile =
    ...
    #[test]
    fn lab_consumes_shared_platform_projection_materializers_from_kit() {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("time")
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("nimi-lab-platform-projection-{unique}"));
    ...
    #[test]
    fn lab_consumes_shared_governed_config_repair_framework() {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("time")
            .as_nanos();
        let dir = std::env::temp_dir().join(format!("nimi-lab-governed-config-{unique}"));
        ...
        match outcome {
            nimi_shell_tauri::capabilities::config::ConfigReadOutcome::Repair {
                ...
                assert!(reason.contains("newer than the supported version"));
                assert!(reason.contains("~/.nimi/lab/probe.json"));
            }
    ...
    #[test]
    fn lab_consumes_shared_runtime_bridge_unary_codec_helpers() {
        let request =
    ...
    #[test]
    fn lab_consumes_shared_nimi_data_directory_primitives() {
        let unique = SystemTime::now()
            .duration_since(UNIX_EPOCH)
            .expect("time")
            .as_nanos();
        let data_root = std::env::temp_dir().join(format!("nimi-lab-data-root-{unique}"));
