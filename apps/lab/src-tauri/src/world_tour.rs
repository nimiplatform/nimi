...
/// Resolves the lab cache root from the Runtime-attested managed storage
...
/// Resolves the lab temp root from the Runtime-attested managed storage
...
fn world_tour_cache_root(cache_root: &str) -> Result<PathBuf, String> {
    let root = canonical_storage_root(cache_root, "lab cache root")?.join("world-tour");
...
fn resolve_manifest_path(cache_root: &str, input: &str) -> Result<PathBuf, String> {
    ...
    if !canonical.starts_with(&root) {
        return Err(format!(
            "world-tour manifest escapes lab cache: {}",
            canonical.display()
        ));
    }
    Ok(canonical)
