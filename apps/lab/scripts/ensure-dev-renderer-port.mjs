...
function isLabRendererProcess(commandLine) {
  const normalized = normalizeForMatch(commandLine);
  const normalizedLabRoot = normalizeForMatch(labRoot);
  return normalized.includes(normalizedLabRoot)
    && normalized.includes('vite')
    && normalized.includes('--port 1468');
}
...
function ensureRendererPortAvailable() {
  ...
  for (const pid of pids) {
    ...
    if (!isLabRendererProcess(commandLine)) {
      throw new Error(
        `Port ${rendererPort} is already in use by PID ${pid}. ` +
        'It is not a recognized lab renderer process, so cleanup was skipped.',
      );
    }

    console.log(`[dev-renderer-port] Stopping stale lab renderer on port ${rendererPort} (PID ${pid}).`);
    process.kill(pid);
  }
}
