...
export function buildWorkbenchWorldPackage(input: {
  ...
  const pkg = {
    ...
    projection: {
      inputs: [
        ...worldRules.map((rule) => ({
          id: deterministicId('projection', `${worldId}-${rule.ruleKey}`),
        ...
        ...agentBlueprints.map((agent) => ({
          id: deterministicId('projection', `${worldId}-${agent.id}`),
    ...
    evidence: {
      sourceChunkIds: [input.workspaceId],
      truthBindings: [],
    },
    governance: {
      packageId: deterministicId('forge-package', `${worldId}-${packageVersion}`),
      packageVersion,
      sourceTitle: worldName,
      sourceMode: 'forge-official' as const,
      generatedBy: 'world-agent-package-factory' as const,
    ...
    compat: {
      ...
      agentProfiles: agentBlueprints.map((agent) => ({
        id: agent.id,
        name: agent.name,
