      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Surface tone="panel" padding="md" className="rounded-xl">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="rounded bg-[color-mix(in_srgb,var(--nimi-surface-panel)_60%,transparent)] px-2 py-1 text-xs text-[var(--nimi-text-secondary)]">{worldRules.length}</span>
          </div>
          <div className="space-y-3">
            {worldRules.length === 0 ? (
              <Surface tone="card" material="glass-thin" padding="sm" className="text-sm text-[var(--nimi-text-muted)]">
                No world rules derived yet.
              </Surface>
            ) : worldRules.map((rule) => (
              <Surface key={String(rule.ruleKey)} tone="card" material="glass-thin" padding="sm">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-sm font-medium text-[var(--nimi-text-primary)]">{String(rule.title || 'Untitled Rule')}</span>
                  <span className="rounded bg-[color-mix(in_srgb,var(--nimi-surface-panel)_60%,transparent)] px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-[var(--nimi-text-secondary)]">{String(rule.domain || 'UNKNOWN')}</span>
                  <span className="rounded bg-[color-mix(in_srgb,var(--nimi-surface-panel)_60%,transparent)] px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-[var(--nimi-text-secondary)]">{String(rule.scope || 'WORLD')}</span>
                </div>
        ...
        <Surface tone="panel" padding="md" className="rounded-xl">
          <div className="mb-3 flex items-center justify-between">
            ...
            <span className="rounded bg-[color-mix(in_srgb,var(--nimi-surface-panel)_60%,transparent)] px-2 py-1 text-xs text-[var(--nimi-text-secondary)]">{agentRules.length}</span>
          </div>
          <div className="space-y-3">
            {agentRules.length === 0 ? (
              <Surface tone="card" material="glass-thin" padding="sm" className="text-sm text-[var(--nimi-text-muted)]">
              ...
              <Surface key={item.characterName} tone="card" material="glass-thin" padding="sm">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-sm font-medium text-[var(--nimi-text-primary)]">{String(item.payload.title || item.characterName)}</span>
                  <span className="rounded bg-[color-mix(in_srgb,var(--nimi-surface-panel)_60%,transparent)] px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-[var(--nimi-text-secondary)]">{String(item.payload.layer || 'DNA')}</span>
                  <span className="rounded bg-[color-mix(in_srgb,var(--nimi-surface-panel)_60%,transparent)] px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-[var(--nimi-text-secondary)]">{String(item.payload.scope || 'SELF')}</span>
                </div>
