...
function assertPackageManifestCurrent(packageJson, versions) {
  ...
  if (scripts.publish !== undefined && scripts.publish !== 'nimi-app publish') {
    throw new Error('package.json scripts.publish must use the canonical nimi-app publish owner');
  }
}
...
function normalizePackageManifest(packageJson, descriptor, versions) {
  ...
  normalized.scripts.publish = 'nimi-app publish';
...
function selectBuildOwner(profile, requestedTarget) {
  ...
  return { target, command, payloadPath };
}
...
export function buildAppProject(cwd, options = {}, runners = {}) {
  ...
  if (options.production === true) {
    ...
    runners.signWindowsTarget(targetDir, selected.payloadPath);
  }
  return emitResult({
    ok: true,
    command: 'build',
