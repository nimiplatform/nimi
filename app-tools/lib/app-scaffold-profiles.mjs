...
function renderDefaultProductArea(identity) {
  return [
    ...
    "      setArtifactStatus(projection.appHost.operationAllowed ? 'selected-operation-granted' : 'zero-grant-session');",
    '    });',
    '    return () => { active = false; };',
    '  }, []);',
    '',
    ...
    '          <span>{artifactStatus || \'Checking the protected local-app session…\'}</span>',
    '        </InlineAlert>',
    '      </Surface>',
    '      <DemoSurfaces />',
    '    </main>',
