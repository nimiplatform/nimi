...
export function buildDefaultStarterFiles(identity) {
  return [
    ...
    {
      path: 'scripts/clean-electron-production.mjs',
      content: DEFAULT_ELECTRON_PRODUCTION_CLEANER,
      mutationClass: 'scaffold-managed glue',
    },
    {
      path: 'scripts/package-electron-production.mjs',
      content: renderDefaultElectronProductionPackager(identity),
      mutationClass: 'scaffold-managed glue',
    },
    {
...
const DEFAULT_ELECTRON_PRODUCTION_CLEANER = [
...
function renderDefaultElectronProductionPackager(identity) {
  return [
    ...
    '    arch: NATIVE_ARCH,',
    `    appBundleId: ${JSON.stringify(identity.tauriIdentifier)},`,
...
function renderGeneratedAIStudioHostAdapter(resolution) {
  ...
  return [
    ...
    '    if (result.ok && result.output.kind === \'artifacts\' && (result.output.musicGeneration || result.output.musicTranscription || result.output.voiceConversion)) {',
    ...
    '    assets: getNimiLocalAppClient().storage.assets,',
    '    storage: getNimiLocalAppClient().storage,',
    '    aiConfig: {',
    '      getSnapshot: () => getNimiLocalAppClient().aiConfig.get(),',
    ...
    '  // Absent when the App-owned ai-studio-core predates commit reporting.',
