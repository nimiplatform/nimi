...
function renderGeneratedAIStudioHostAdapter(resolution) {
  ...
  return [
    ...
    '  const verifiedHistory: Record<string, StudioRunHistoryRecord[]> = {};',
    '  const mediaHistory: StudioMediaHistoryRecord[] = [];',
    '  for (const [capabilityId, records] of Object.entries(runHistory)) {',
    '    const verifiedRecords: StudioRunHistoryRecord[] = [];',
    '    for (const record of records) {',
    '      const result = record.result;',
    ...
    '            capabilityLabel: generatedAIStudioRegistrations.find((entry) => entry.descriptor.id === record.capabilityId)?.descriptor.label ?? record.capabilityId,',
    '            title: artifact.displayName || artifact.relativePath,',
    '            status,',
    ...
    '        }',
    '      }',
    '      verifiedRecords.push(projectedRecord);',
    '    }',
    '    verifiedHistory[capabilityId] = verifiedRecords;',
    '  }',
    '  return { runHistory: verifiedHistory, mediaHistory };',
    '}',
    '',
    ...
    '});',
    '',
    'type PromptDraftStore = { readonly version: 1; readonly drafts: Readonly<Record<string, string>> };',
    "const emptyPromptDraftStore: PromptDraftStore = Object.freeze({ version: 1, drafts: Object.freeze({}) });",
    'let promptDraftStore = readPromptDraftStore();',
    '',
    'function promptDraftId(key: { readonly surfaceId: string; readonly capabilityId: string; readonly scenarioId: string }): string {',
    "  return [key.surfaceId, key.capabilityId, key.scenarioId].join(':');",
    '}',
    '',
    'function readPromptDraftStore(): PromptDraftStore {',
    '  try {',
    '    const raw = globalThis.localStorage?.getItem(PROMPT_DRAFT_STORAGE_KEY);',
    '    if (!raw) return emptyPromptDraftStore;',
    '    const parsed: unknown = JSON.parse(raw);',
    "    if (!isObject(parsed) || parsed.version !== 1 || !isObject(parsed.drafts)) return emptyPromptDraftStore;",
    '    const drafts = Object.fromEntries(Object.entries(parsed.drafts).filter((entry): entry is [string, string] => (',
    "      Boolean(entry[0]) && typeof entry[1] === 'string'",
    '    )));',
    '    return { version: 1, drafts };',
    '  } catch {',
    '    return emptyPromptDraftStore;',
    '  }',
    '}',
    '',
    ...
    '  const id = promptDraftId(key);',
    '  const drafts = { ...promptDraftStore.drafts };',
    '  if (enabled && prompt) drafts[id] = prompt;',
    '  else delete drafts[id];',
    '  promptDraftStore = { version: 1, drafts };',
    ...
    '  app: {',
    '    projection: {',
    '      promptDraft: (key, enabled) => ({ prompt: enabled ? promptDraftStore.drafts[promptDraftId(key)] ?? null : null }),',
