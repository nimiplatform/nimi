...
// Candidate is integrated into the active Simulator checker only in WP6.
// @nimi-authority: rule.nimi.platform.app-ecosystem.p-scaf-019c
export async function validateSimulatorCanonicalKitExportsCandidate(input) {
  ...
  for (const requirement of requirements) {
    ...
    const module = await import(`${pathToFileURL(target).href}?simulator-candidate=${encodeURIComponent(subpath)}`);
