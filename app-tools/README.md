...
and `sync`; production publication remains fail-closed until the publisher
GitHub and registry pull-request owner paths are implemented.
...
The intended production chain is singular but not yet enabled:
...
Repository administration must enable a protected `v*` tag ruleset and GitHub immutable releases before production. A fine-grained `GITHUB_REPOSITORY_ADMIN_TOKEN` secret with repository Administration read permission lets the managed tag workflow verify those settings; it cannot enable or change them. Manual workflow dispatch runs only the non-production build/package path. On Windows, the future tag-only production build imports the publisher PFX from `WINDOWS_CERTIFICATE_BASE64`/`WINDOWS_CERTIFICATE_PASSWORD`, signs the exact declared Host, and removes the temporary certificate; production pack only re-verifies Authenticode and never signs. Tag publication, registry submission, the shared installed carrier, and installed launch are not implemented, so the production path remains fail-closed.
