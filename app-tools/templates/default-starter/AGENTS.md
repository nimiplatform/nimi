...
- The default app-owned area is `src/shell/routes/product-area.tsx` plus future product-specific screens, view models, tests, and bounded native helpers the app intentionally adds.
- Keep reusable Electron, Tauri, Runtime, SDK, and Kit behavior in platform packages or scaffold-managed glue. App-owned shell additions must remain product-specific and must not become a parallel platform truth surface.
